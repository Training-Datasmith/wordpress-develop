...
class Theme_Upgrader extends WP_Upgrader
{
     ...
     */
    public function hide_activate_preview_actions(array $actions): array
    {
    ...
    public function bulk_upgrade($themes, $args = [])
    {
        ...
        $connected = $this->fs_connect([ WP_CONTENT_DIR ]);
        if (! $connected) {
            $this->skin->footer();
            return false;
        }
     ...
     */
    public function current_before($response, array $theme)
    {
     ...
     * @return bool|WP_Error The original `$response` parameter or WP_Error.
     */
    public function current_after($response, array $theme)
    {
        if (is_wp_error($response)) {
            return $response;
        }
     ...
     * @return bool
     */
    public function delete_old_theme($removed, $local_destination, $remote_destination, array $theme)
    {
        global $wp_filesystem;
