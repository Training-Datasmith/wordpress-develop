...
class WP_Automatic_Updater
{
     ...
     */
    public function should_update($type, $item, $context): bool
    {
     ...
     *              false otherwise.
     */
    protected function send_core_update_notification_email($item): bool
    {
    ...
    public function update($type, $item)
    {
        ...
        if ('plugin' === $type) {
            ...
            if ($was_active && !is_wp_error($upgrade_result)) {
                ...
                if ($this->has_fatal_error()) {
                    ...
                    $temp_backup = [['dir' => 'plugins', 'slug' => $item->slug, 'src' => WP_PLUGIN_DIR]];
                    $backup_restored = $upgrader->restore_temp_backup($temp_backup);
                    if (is_wp_error($backup_restored)) {
                        $upgrade_result->add('plugin_update_fatal_error_rollback_failed', sprintf(
                        ...
                        $upgrade_result->add('plugin_update_fatal_error_rollback_successful', sprintf(
                            /* translators: %s: The plugin's slug. */
        ...
        $this->update_results[$type][] = (object) ['item' => $item, 'result' => $upgrade_result, 'name' => $item_name, 'messages' => $skin->get_upgrade_messages()];
    ...
    protected function after_core_update($update_result)
    {
        ...
        if ($critical) {
            $critical_data = ['attempted' => $core_update->current, 'current' => $wp_version, 'error_code' => $error_code, 'error_data' => $result->get_error_data(), 'timestamp' => time(), 'critical' => true];
        ...
        update_site_option('auto_core_update_failed', ['attempted' => $core_update->current, 'current' => $wp_version, 'error_code' => $error_code, 'error_data' => $result->get_error_data(), 'timestamp' => time(), 'retry' => in_array($error_code, $transient_failures, true)]);
        if ($send) {
    ...
    {
        update_site_option('auto_core_update_notified', ['type' => $type, 'email' => get_site_option('admin_email'), 'version' => $core_update->current, 'timestamp' => time()]);
     ...
     */
    protected function after_plugin_theme_update(array $update_results)
    {
        $successful_updates = [];
        $failed_updates = [];
    ...
    protected function send_debug_email()
    {
        ...
        foreach (['plugin', 'theme', 'translation'] as $type) {
            ...
            if ($success_items) {
                $messages = ['plugin' => __('The following plugins were successfully updated:'), 'theme' => __('The following themes were successfully updated:'), 'translation' => __('The following translations were successfully updated:')];
                $body[] = $messages[$type];
            ...
            if ($success_items !== $this->update_results[$type]) {
                // Failed updates.
                $messages = ['plugin' => __('The following plugins failed to update:'), 'theme' => __('The following themes failed to update:'), 'translation' => __('The following translations failed to update:')];
                $body[] = $messages[$type];
...
=========='));
        ...
        $email = ['to' => get_site_option('admin_email'), 'subject' => $subject, 'body' => implode("\n", $body), 'headers' => ''];
        /**
