...
class Custom_Background
{
    ...
    {
        $page = add_theme_page(_x('Background', 'custom background'), _x('Background', 'custom background'), 'edit_theme_options', 'custom-background', [$this, 'admin_page']);
        if (!$page) {
            return;
        }
    ...
    {
        get_current_screen()->add_help_tab(['id' => 'overview', 'title' => __('Overview'), 'content' => '<p>' . __('You can customize the look of your site without touching any of your theme&#8217;s code by using a custom background. Your background can be an image or a color.') . '</p>' . '<p>' . __('To use a background image, simply upload it or choose an image that has already been uploaded to your Media Library by clicking the &#8220;Choose Image&#8221; button. You can display a single instance of your image, or tile it to fill the screen. You can have your background fixed in place, so your site content moves on top of it, or you can have it scroll with your site.') . '</p>' . '<p>' . __('You can also choose a background color by clicking the Select Color button and either typing in a legitimate HTML hex value, e.g. &#8220;#ff0000&#8221; for red, or by choosing a color using the color picker.') . '</p>' . '<p>' . __('Do not forget to click on the Save Changes button when you are finished.') . '</p>']);
...
<input name="background-preset" type="hidden" value="custom">
            ...
            $background_position_options = [['left top' => ['label' => __('Top Left'), 'icon' => 'dashicons dashicons-arrow-left-alt'], 'center top' => ['label' => __('Top'), 'icon' => 'dashicons dashicons-arrow-up-alt'], 'right top' => ['label' => __('Top Right'), 'icon' => 'dashicons dashicons-arrow-right-alt']], ['left center' => ['label' => __('Left'), 'icon' => 'dashicons dashicons-arrow-left-alt'], 'center center' => ['label' => __('Center'), 'icon' => 'background-position-center-icon'], 'right center' => ['label' => __('Right'), 'icon' => 'dashicons dashicons-arrow-right-alt']], ['left bottom' => ['label' => __('Bottom Left'), 'icon' => 'dashicons dashicons-arrow-left-alt'], 'center bottom' => ['label' => __('Bottom'), 'icon' => 'dashicons dashicons-arrow-down-alt'], 'right bottom' => ['label' => __('Bottom Right'), 'icon' => 'dashicons dashicons-arrow-right-alt']]];
            ?>
<tr>
<th scope="row"><?php 
            echo $background_position_title;
            ?></th>
...
<div class="background-position-control">
	...
	<div class="button-group">
                ...
                foreach ($group as $value => $input) {
                    ?>
		<label>
			<input class="ui-helper-hidden-accessible" name="background-position" type="radio" value="<?php 
                    echo esc_attr($value);
                    ?>"<?php 
...
<?php 
			...
			<?php 
            $image_size_title = __('Image Size');
            ?>
<tr>
<th scope="row"><label for="background-size"><?php 
            echo $image_size_title;
            ?></label></th>
<td><fieldset><legend class="screen-reader-text"><span><?php 
            echo $image_size_title;
            ?></span></legend>
...
<option value="cover"<?php 
			...
			<?php 
            $background_repeat_title = _x('Repeat', 'Background Repeat');
            ?>
<tr>
<th scope="row"><?php 
            echo $background_repeat_title;
            ?></th>
<td><fieldset><legend class="screen-reader-text"><span><?php 
            echo $background_repeat_title;
            ?></span></legend>
...
<label><input type="checkbox" name="background-repeat" value="repeat"<?php 
			...
			<?php 
            $background_scroll_title = _x('Scroll', 'Background Scroll');
            ?>
<tr>
<th scope="row"><?php 
            echo $background_scroll_title;
            ?></th>
<td><fieldset><legend class="screen-reader-text"><span><?php 
            echo $background_scroll_title;
            ?></span></legend>
...
<?php 
		...
		<?php 
        $background_color_title = __('Background Color');
        ?>
<tr>
<th scope="row"><?php 
        echo $background_color_title;
        ?></th>
<td><fieldset><legend class="screen-reader-text"><span><?php 
        echo $background_color_title;
        ?></span></legend>
...
<input type="text" name="background-color" id="background-color" value="#<?php 
    ...
    public function handle_upload(): void
    {
        ...
        $attachment = ['post_title' => $filename, 'post_content' => $url, 'post_mime_type' => $type, 'guid' => $url, 'context' => 'custom-background'];
        // Save the data.
        ...
        apply_filters('wp_create_file_in_uploads', $file, $id);
        // For replication.
        $this->updated = true;
    }
    /**
     ...
     * @since 4.1.0
     */
    public function ajax_background_add(): void
    {
     ...
     * @deprecated 3.5.0
     */
    public function wp_set_background_image(): void
    {
        ...
        $sizes = array_keys(
            ...
            apply_filters('image_size_names_choose', ['thumbnail' => __('Thumbnail'), 'medium' => __('Medium'), 'large' => __('Large'), 'full' => __('Full Size')])
        );
        $size = 'thumbnail';
