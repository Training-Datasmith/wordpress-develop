 ...
 */
function wp_get_plugin_file_editable_extensions($plugin): array
{
    $default_types = ['bash', 'conf', 'css', 'diff', 'htm', 'html', 'http', 'inc', 'include', 'js', 'mjs', 'json', 'jsx', 'less', 'md', 'patch', 'php', 'php3', 'php4', 'php5', 'php7', 'phps', 'phtml', 'sass', 'scss', 'sh', 'sql', 'svg', 'text', 'txt', 'xml', 'yaml', 'yml'];
    /**
 ...
 * @return string[] Array of editable file extensions.
 */
function wp_get_theme_file_editable_extensions($theme): array
{
    $default_types = ['bash', 'conf', 'css', 'diff', 'htm', 'html', 'http', 'inc', 'include', 'js', 'mjs', 'json', 'jsx', 'less', 'md', 'patch', 'php', 'php3', 'php4', 'php5', 'php7', 'phps', 'phtml', 'sass', 'scss', 'sh', 'sql', 'svg', 'text', 'txt', 'xml', 'yaml', 'yml'];
    /**
 ...
 * @since 4.9.0
 */
function wp_print_file_editor_templates(): void
{
    ?>
 ...
 */
function wp_edit_theme_plugin_file(array $args)
{
    if (empty($args['file'])) {
    ...
    if ($is_active && 'php' === $extension) {
        $scrape_key = md5(random_int(0, mt_getrandmax()));
        $transient = 'scrape_key_' . $scrape_key;
        $scrape_nonce = (string) random_int(0, mt_getrandmax());
        // It shouldn't take more than 60 seconds to make the two loopback requests.
        set_transient($transient, $scrape_nonce, 60);
        $cookies = wp_unslash($_COOKIE);
        $scrape_params = ['wp_scrape_key' => $scrape_key, 'wp_scrape_nonce' => $scrape_nonce];
        ...
        if ($plugin) {
            ...
            $url = add_query_arg(['theme' => $stylesheet, 'file' => $file], admin_url('theme-editor.php'));
        } else {
            $url = admin_url();
        }
        ...
        $loopback_request_failure = ['code' => 'loopback_request_failed', 'message' => __('Unable to communicate back with site to check for fatal errors, so the PHP change was reverted. You will need to upload your PHP file change by some other means, such as by using SFTP.')];
...
function _wp_handle_upload(&$file, $overrides, $time, $action)
{
    ...
    if (!function_exists('wp_handle_upload_error')) {
        function wp_handle_upload_error(&$file, $message): array
        {
    ...
    if (isset($overrides['upload_error_strings'])) {
        ...
        $upload_error_strings = [false, sprintf(
     ...
     */
    return apply_filters('wp_handle_upload', ['file' => $new_file, 'url' => $url, 'type' => $type], 'wp_handle_sideload' === $action ? 'sideload' : 'upload');
}
/**
...
function download_url($url, $timeout = 300, $signature_verification = false)
{
    ...
    if ($signature_verification) {
        ...
        if (!$signature) {
            ...
            if ($signature_url) {
                $signature_request = wp_safe_remote_get($signature_url, ['limit_response_size' => 10 * KB_IN_BYTES]);
...
function verify_file_signature($filename, $signatures, $filename_for_errors = false)
{
    ...
    if (!extension_loaded('sodium') && !Paragon_Ie_sodium_compat::polyfill_is_fast()) {
        ...
        if (method_exists('ParagonIE_Sodium_Compat', 'runtime_speed_test')) {
            ...
            $old_fast_mult = Paragon_Ie_sodium_compat::$fast_mult;
            Paragon_Ie_sodium_compat::$fast_mult = true;
            $sodium_compat_is_fast = Paragon_Ie_sodium_compat::runtime_speed_test(100, 10);
            Paragon_Ie_sodium_compat::$fast_mult = $old_fast_mult;
            // phpcs:enable
        }
        /*
        ...
        if (!$sodium_compat_is_fast) {
            return new WP_Error('signature_verification_unsupported', sprintf(
                ...
                __('The authenticity of %s could not be verified as signature verification is unavailable on this system.'),
                '<span class="code">' . esc_html($filename_for_errors) . '</span>'
            ), ['php' => PHP_VERSION, 'sodium' => defined('SODIUM_LIBRARY_VERSION') ? SODIUM_LIBRARY_VERSION : (defined('ParagonIE_Sodium_Compat::VERSION_STRING') ? Paragon_Ie_sodium_compat::VERSION_STRING : false), 'polyfill_is_fast' => false, 'max_execution_time' => ini_get('max_execution_time')]);
        }
    }
    if (!$signatures) {
    ...
    return new WP_Error(
        ...
        ['filename' => $filename_for_errors, 'keys' => $trusted_keys, 'signatures' => $signatures, 'hash' => bin2hex($file_hash), 'skipped_key' => $skipped_key, 'skipped_sig' => $skipped_signature, 'php' => PHP_VERSION, 'sodium' => defined('SODIUM_LIBRARY_VERSION') ? SODIUM_LIBRARY_VERSION : (defined('ParagonIE_Sodium_Compat::VERSION_STRING') ? Paragon_Ie_sodium_compat::VERSION_STRING : false)]
    );
}
/**
...
function wp_zip_file_is_valid($file)
{
    ...
    if (class_exists('ZipArchive', false) && apply_filters('unzip_file_use_ziparchive', true)) {
        ...
        $archive_is_valid = $archive->open($file, Zip_Archive::CHECKCONS);
    ...
    return is_array($archive->properties());
}
/**
 ...
 * @return true|WP_Error True on success, WP_Error on failure.
 */
function _unzip_file_ziparchive($file, string $to, $needed_dirs = [])
{
    global $wp_filesystem;
    $z = new Zip_Archive();
 ...
 * @return true|WP_Error True on success, WP_Error on failure.
 */
function _unzip_file_pclzip($file, string $to, $needed_dirs = [])
{
    global $wp_filesystem;
    mbstring_binary_safe_encoding();
    require_once ABSPATH . 'wp-admin/includes/class-pclzip.php';
    ...
    if (!is_array($archive_files)) {
        return new WP_Error('incompatible_archive', __('Incompatible Archive.'), $archive->error_info(true));
    }
    ...
    foreach ($archive_files as $archive_file) {
        if (str_starts_with($archive_file['filename'], '__MACOSX/')) {
            // Skip the OS X-created __MACOSX directory.
            continue;
        }
...
function request_filesystem_credentials($form_post, $type = '', $error = false, $context = '', $extra_fields = null, $allow_relaxed_file_ownership = false)
{
    ...
    $ftp_constants = ['hostname' => 'FTP_HOST', 'username' => 'FTP_USER', 'password' => 'FTP_PASS', 'public_key' => 'FTP_PUBKEY', 'private_key' => 'FTP_PRIKEY'];
    /*
    ...
    if ($error) {
        ...
        wp_admin_notice($error_string, ['id' => 'message', 'additional_classes' => ['error']]);
    }
    $types = [];
...
<p id="auth-keys-desc"><?php 
    ...
    ?></button>
		<?php 
    submit_button(__('Proceed'), 'primary', 'upgrade', false);
    ?>
	</p>
</div>
</form>
 ...
 * @since 4.2.0
 */
function wp_print_request_filesystem_credentials_modal(): void
{
 ...
 */
function wp_opcache_invalidate_directory($dir): void
{
    global $wp_filesystem;
     ...
     */
    $invalidate_directory = static function ($dirlist, $path) use (&$invalidate_directory): void {
