 ...
 *                              Default false.
 */
function wp_image_editor($post_id, $msg = false): void
{
	...
	<div class="imgedit-panel-content wp-clearfix">
		<div class="imgedit-tools">
			...
			<img id="image-preview-<?php echo $post_id; ?>" onload="imageEdit.imgLoaded('<?php echo $post_id; ?>')"
				src="<?php echo esc_url(admin_url('admin-ajax.php', 'relative')) . '?action=imgedit-preview&amp;_ajax_nonce=' . $nonce . '&amp;postid=' . $post_id . '&amp;rand=' . random_int(1, 99999); ?>" alt="" />
			</div>
		</div>
 ...
 */
function wp_restore_image($post_id): \stdClass
{
    $meta             = wp_get_attachment_metadata($post_id);
 ...
 * @param int $post_id Attachment post ID.
 */
function wp_save_image($post_id): \stdClass
{
