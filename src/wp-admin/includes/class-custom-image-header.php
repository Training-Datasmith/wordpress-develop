...
class Custom_Image_Header
{
    ...
    {
        $page = add_theme_page(_x('Header', 'custom image header'), _x('Header', 'custom image header'), 'edit_theme_options', 'custom-header', [$this, 'admin_page']);
        if (!$page) {
            return;
        }
     ...
     * @since 2.1.0
     */
    public function js_includes(): void
    {
        $step = $this->step();
        if (1 === $step || 3 === $step) {
            wp_enqueue_media();
     ...
     * @since 2.7.0
     */
    public function css_includes(): void
    {
        $step = $this->step();
     ...
     * @since 2.6.0
     */
    public function take_action(): void
    {
     ...
     */
    public function process_default_headers(): void
    {
     ...
     */
    public function show_header_selector($type = 'default'): void
    {
        if ('default' === $type) {
...
<h1><?php 
        ...
        if (current_user_can('customize')) {
            ...
            wp_admin_notice($message, ['type' => 'info', 'additional_classes' => ['hide-if-no-customize']]);
        }
        if (!empty($this->updated)) {
            ...
            wp_admin_notice($updated_message, ['id' => 'message', 'additional_classes' => ['updated']]);
        }
        ?>
...
<td>
	...
	<p>
		...
		<input type="hidden" name="action" value="save" />
            ...
            ?>
			<?php 
            submit_button(_x('Upload', 'verb'), '', 'submit', false);
            ?>
	</p>
			<?php 
            $modal_update_href = add_query_arg(['page' => 'custom-header', 'step' => 2, '_wpnonce-custom-header-upload' => wp_create_nonce('custom-header-upload')], admin_url('themes.php'));
            ?>
	<p>
...
<td>
            ...
            if (current_theme_supports('custom-header', 'uploads')) {
                ?>
	<p><?php 
                _e('If you do not want to upload your own image, you can use one of these cool headers, or show a random one.');
                ?></p>
	<?php 
            } else {
                ?>
...
<td>
    ...
    public function step_2()
    {
        ...
        if (!current_theme_supports('custom-header', 'uploads')) {
            wp_die('<h1>' . __('An error occurred while processing your header image.') . '</h1>' . '<p>' . __('The active theme does not support uploading a custom header image. Please ensure your theme supports custom headers and try again.') . '</p>', 403);
        }
        ...
        if (!current_theme_supports('custom-header', 'flex-height') && !current_theme_supports('custom-header', 'flex-width') && (int) get_theme_support('custom-header', 'width') === $width && (int) get_theme_support('custom-header', 'height') === $height) {
             ...
             */
            $file = apply_filters('wp_create_file_in_uploads', $file, $attachment_id);
            // For replication.
...
<form method="post" action="<?php 
	...
	<p class="submit">
        ...
        ?>
		<?php 
        if (1 === $oitar && (current_theme_supports('custom-header', 'flex-height') || current_theme_supports('custom-header', 'flex-width'))) {
     ...
     * @since 3.4.0
     */
    public function step_2_manage_upload(): array
    {
        ...
        $attachment = ['post_title' => $filename, 'post_content' => $url, 'post_mime_type' => $type, 'guid' => $url, 'context' => 'custom-header'];
        // Save the data.
    ...
    public function step_3()
    {
        ...
        if (!empty($_POST['skip-cropping']) && !current_theme_supports('custom-header', 'flex-height') && !current_theme_supports('custom-header', 'flex-width')) {
            wp_die('<h1>' . __('An error occurred while processing your header image.') . '</h1>' . '<p>' . __('The active theme does not support a flexible sized header image.') . '</p>', 403);
        }
        if ($_POST['oitar'] > 1) {
        ...
        $dimensions = $this->get_header_dimensions(['height' => $_POST['height'], 'width' => $_POST['width']]);
     ...
     */
    final public function set_header_image($choice): void
    {
        if (is_array($choice) || is_object($choice)) {
            ...
            $header_image_data = (object) ['attachment_id' => $choice['attachment_id'], 'url' => $choice['url'], 'thumbnail_url' => $choice['url'], 'height' => $choice['height'], 'width' => $choice['width']];
     ...
     * @since 3.4.0
     */
    final public function remove_header_image(): void
    {
     ...
     * @since 3.4.0
     */
    final public function reset_header_image(): void
    {
        $this->process_default_headers();
     ...
     */
    final public function get_header_dimensions(array $dimensions): array
    {
        $max_width = 0;
     ...
     */
    final public function create_attachment_object($cropped, $parent_attachment_id): array
    {
        ...
        return ['ID' => $parent_attachment_id, 'post_title' => wp_basename($cropped), 'post_mime_type' => $image_type, 'guid' => $url, 'context' => 'custom-header', 'post_parent' => $parent_attachment_id];
    }
    /**
     ...
     */
    final public function insert_attachment(array $attachment, $cropped)
    {
     ...
     * @since 3.9.0
     */
    public function ajax_header_crop(): void
    {
        ...
        $dimensions = $this->get_header_dimensions(['height' => $crop_details['height'], 'width' => $crop_details['width']]);
     ...
     * @since 3.9.0
     */
    public function ajax_header_add(): void
    {
     ...
     * @since 3.9.0
     */
    public function ajax_header_remove(): void
    {
     ...
     */
    public function customize_set_last_used($wp_customize): void
    {
    ...
    public function get_default_header_images()
    {
        ...
        foreach ($this->default_headers as $header) {
        ...
        $header_images = [];
        $header_images['default'] = ['url' => $default, 'thumbnail_url' => $default, 'description' => 'Default'];
     ...
     */
    public function get_previous_crop(array $attachment)
    {
