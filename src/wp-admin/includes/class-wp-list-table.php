...
class WP_List_Table
{
    ...
    {
        $args = wp_parse_args($args, ['plural' => '', 'singular' => '', 'ajax' => false, 'screen' => null]);
        ...
        if (empty($this->modes)) {
            $this->modes = ['list' => __('Compact view'), 'excerpt' => __('Extended view')];
        }
    }
    /**
...
<p class="search-box">
    ...
    protected function row_actions($actions, $always_visible = false): string
    {
        ...
        $output .= '</div>';
        return $output . ('<button type="button" class="toggle-row"><span class="screen-reader-text">' . __('Show more details') . '</span></button>');
    }
    /**
    ...
    protected function comments_bubble($post_id, $pending_comments)
    {
        ...
        if (!current_user_can($edit_post_cap, $post_id) && (post_password_required($post_id) || !current_user_can('read_post', $post_id))) {
        ...
        if (!$approved_comments && !$pending_comments) {
            ...
            printf('<a href="%s" class="post-com-count post-com-count-approved">' . '<span class="comment-count-approved" aria-hidden="true">%s</span>' . '<span class="screen-reader-text">%s</span>' . '</a>', esc_url(add_query_arg(['p' => $post_id, 'comment_status' => 'approved'], admin_url('edit-comments.php'))), $approved_comments_number, $pending_comments ? $approved_phrase : $approved_only_phrase);
        } else {
        ...
        if ($pending_comments) {
            printf('<a href="%s" class="post-com-count post-com-count-pending">' . '<span class="comment-count-pending" aria-hidden="true">%s</span>' . '<span class="screen-reader-text">%s</span>' . '</a>', esc_url(add_query_arg(['p' => $post_id, 'comment_status' => 'moderated'], admin_url('edit-comments.php'))), $pending_comments_number, $pending_phrase);
        } else {
    ...
    public function print_column_headers($with_id = true): void
    {
        ...
        if (!empty($columns['cb'])) {
			...
			<label for="cb-select-all-' . $cb_counter . '">' . '<span class="screen-reader-text">' . __('Select All') . '</span>' . '</label>';
            ++$cb_counter;
        }
     ...
     * @since 6.3.0
     */
    public function print_table_description(): void
    {
...
<table class="wp-list-table <?php 
    ...
    {
        $args = ['class' => get_class($this), 'screen' => ['id' => $this->screen->id, 'base' => $this->screen->base]];
