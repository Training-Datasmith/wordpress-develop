 ...
 * @return int Post ID.
 */
function edit_post($post_data = null): int
{
    global $wpdb;
    ...
    if ('attachment' === $post_data['post_type']) {
        ...
        $attachment_data = $post_data['attachments'][ $post_id ] ?? [];
 ...
 * }
 */
function bulk_edit_posts($post_data = null): array
{
    global $wpdb;
 ...
 */
function post_exists($title, $content = '', $date = '', $type = '', $status = ''): int
{
    global $wpdb;
...
function _fix_attachment_links($post)
{
    ...
    foreach ($link_matches[1] as $key => $value) {
        if (! strpos($value, '?attachment_id=')) {
            continue;
        }
        if (! strpos($value, 'wp-att-')) {
            continue;
        }
        if (! preg_match('/href=(["\'])[^"\']*\?attachment_id=(\d+)[^"\']*\\1/', $value, $url_match)) {
            continue;
        }
        if (! preg_match('/rel=["\'][^"\']*wp-att-(\d+)/', $value, $rel_match)) {
            continue;
        }
        ...
        if (! str_contains($url_match[0], $site_url)) {
            continue;
        }
 ...
 */
function get_available_post_statuses($type = 'post'): array
{
 ...
 * }
 */
function wp_edit_attachments_query($q = false): array
{
 ...
 */
function postbox_classes($box_id, string $screen_id): string
{
 ...
 * @since 2.8.5
 */
function _admin_notice_post_locked(): void
{
    $post = get_post();
 ...
 */
function wp_autosave_post_revisioned_meta_fields(array $new_autosave): void
{
    /*
 ...
 */
function wp_autosave(array $post_data)
{
    // Back-compat.
 ...
 */
function redirect_post($post_id = 0): void
{
 ...
 */
function taxonomy_meta_box_sanitize_cb_checkboxes($taxonomy, $terms): array
{
 ...
 */
function taxonomy_meta_box_sanitize_cb_input($taxonomy, $terms): array
{
    /*
 ...
 */
function get_block_editor_server_block_settings(): array
{
 ...
 */
function the_block_editor_meta_boxes(): void
{
 ...
 */
function the_block_editor_meta_box_post_form_hidden_fields($post): void
{
    $form_extra = '';
    if ('auto-draft' === $post->post_status) {
 ...
 */
function _disable_content_editor_for_navigation_post_type($post): void
{
 ...
 * @param WP_Post $post An instance of WP_Post class.
 */
function _enable_content_editor_for_navigation_post_type($post): void
{
    $post_type = get_post_type($post);
    if ('wp_navigation' !== $post_type) {
        return;
