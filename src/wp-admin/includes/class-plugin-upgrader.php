...
class Plugin_Upgrader extends WP_Upgrader
{
    ...
    public function bulk_upgrade($plugins, $args = [])
    {
        ...
        $connected = $this->fs_connect([ WP_CONTENT_DIR, WP_PLUGIN_DIR ]);
        if (! $connected) {
            $this->skin->footer();
            return false;
        }
     ...
     * @return bool|WP_Error The original `$response` parameter or WP_Error.
     */
    public function active_before($response, array $plugin)
    {
     ...
     * @return bool|WP_Error The original `$response` parameter or WP_Error.
     */
    public function active_after($response, array $plugin)
    {
        if (is_wp_error($response)) {
            return $response;
        }
