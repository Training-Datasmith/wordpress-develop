...
if (!function_exists('wp_install_defaults')) {
     ...
     */
    function wp_install_defaults($user_id): void
    {
...
Commenter avatars come from <a href="%s">Gravatar</a>.'),
        ...
        $wpdb->insert($wpdb->comments, ['comment_post_ID' => 1, 'comment_author' => $first_comment_author, 'comment_author_email' => $first_comment_email, 'comment_author_url' => $first_comment_url, 'comment_date' => $now, 'comment_date_gmt' => $now_gmt, 'comment_content' => $first_comment, 'comment_type' => 'comment']);
        // First page.
        if (is_multisite()) {
        ...
        if (!empty($privacy_policy_content)) {
            ...
            $wpdb->insert($wpdb->postmeta, ['post_id' => 3, 'meta_key' => '_wp_page_template', 'meta_value' => 'default']);
            update_option('wp_page_for_privacy_policy', 3);
        }
        // Set up default widgets for default theme.
        update_option('widget_block', [2 => ['content' => '<!-- wp:search /-->'], 3 => ['content' => '<!-- wp:group --><div class="wp-block-group"><!-- wp:heading --><h2>' . __('Recent Posts') . '</h2><!-- /wp:heading --><!-- wp:latest-posts /--></div><!-- /wp:group -->'], 4 => ['content' => '<!-- wp:group --><div class="wp-block-group"><!-- wp:heading --><h2>' . __('Recent Comments') . '</h2><!-- /wp:heading --><!-- wp:latest-comments {"displayAvatar":false,"displayDate":false,"displayExcerpt":false} /--></div><!-- /wp:group -->'], 5 => ['content' => '<!-- wp:group --><div class="wp-block-group"><!-- wp:heading --><h2>' . __('Archives') . '</h2><!-- /wp:heading --><!-- wp:archives /--></div><!-- /wp:group -->'], 6 => ['content' => '<!-- wp:group --><div class="wp-block-group"><!-- wp:heading --><h2>' . __('Categories') . '</h2><!-- /wp:heading --><!-- wp:categories /--></div><!-- /wp:group -->'], '_multiwidget' => 1]);
        update_option('sidebars_widgets', ['wp_inactive_widgets' => [], 'sidebar-1' => [0 => 'block-2', 1 => 'block-3', 2 => 'block-4'], 'sidebar-2' => [0 => 'block-5', 1 => 'block-6'], 'array_version' => 3]);
        if (!is_multisite()) {
 ...
 */
function wp_install_maybe_enable_pretty_permalinks(): bool
{
    global $wp_rewrite;
     ...
     */
    $permalink_structures = ['/%year%/%monthnum%/%day%/%postname%/', '/index.php/%year%/%monthnum%/%day%/%postname%/'];
...
'),
        ...
        $installed_email = ['to' => $email, 'subject' => __('New WordPress Site'), 'message' => $message, 'headers' => ''];
        /**
...
function upgrade_160(): void
{
    ...
    if ($wp_current_db_version > 2541 && $wp_current_db_version <= 3091) {
        ...
        foreach ($objects as $object) {
            $wpdb->update($wpdb->posts, ['post_status' => 'attachment', 'post_mime_type' => $object->post_type, 'post_type' => ''], ['ID' => $object->ID]);
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 */
function upgrade_230_options_table(): void
{
    global $wpdb;
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 */
function upgrade_230_old_tables(): void
{
    global $wpdb;
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 */
function upgrade_old_slugs(): void
{
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 */
function upgrade_430_fix_comments(): void
{
    global $wpdb;
    ...
    if (false === $content_length) {
        $content_length = ['type' => 'byte', 'length' => 65535];
    } elseif (!is_array($content_length)) {
        $length = (int) $content_length > 0 ? (int) $content_length : 65535;
        $content_length = ['type' => 'byte', 'length' => $length];
    }
 ...
 * @global int  $wp_current_db_version The old (current) database version.
 */
function upgrade_670(): void
{
    global $wp_current_db_version;
    if ($wp_current_db_version < 58975) {
        $options = ['recently_activated', '_wp_suggested_policy_text_has_changed', 'dashboard_widget_options', 'ftp_credentials', 'adminhash', 'nav_menu_options', 'wp_force_deactivated_plugins', 'delete_blog_hash', 'allowedthemes', 'recovery_keys', 'https_detection_errors', 'fresh_site'];
...
function upgrade_682(): void
{
    ...
    if ($wp_current_db_version < 60421) {
        ...
        $ping_sites_value = array_map(function ($url) {
            $url = trim($url);
            $url = sanitize_url($url);
            if (str_ends_with(trailingslashit($url), '://rpc.pingomatic.com/') || str_ends_with(trailingslashit($url), '://rpc.twingly.com/')) {
 ...
 * @global wpdb $wpdb                  WordPress database abstraction object.
 */
function upgrade_700(): void
{
    global $wp_current_db_version, $wpdb;
    // Migrate users with 'fresh' admin color to 'modern'.
    if ($wp_current_db_version < 61644) {
        $wpdb->update($wpdb->usermeta, ['meta_value' => 'modern'], ['meta_key' => 'admin_color', 'meta_value' => 'fresh']);
    }
}
/**
 ...
 */
function maybe_create_table($table_name, $create_ddl): bool
{
    global $wpdb;
 ...
 */
function drop_index($table, $index): bool
{
    global $wpdb;
    $wpdb->hide_errors();
 ...
 */
function add_clean_index($table, $index): bool
{
    global $wpdb;
 ...
 */
function maybe_add_column($table_name, $column_name, $create_ddl): bool
{
    global $wpdb;
 ...
 */
function get_alloptions_110(): \stdClass
{
    global $wpdb;
 ...
 */
function db_delta($queries = '', $execute = true): array
{
    global $wpdb;
    ...
    foreach ($cqueries as $table => $qry) {
                ...
                case 'spatial':
                    ...
                    foreach ($index_columns as $id => &$index_column) {
                        ...
                        preg_match('/
							...
							/x', $index_column, $index_column_matches);
        ...
        foreach ($tablefields as $tablefield) {
            ...
            $tablefield_type_without_parentheses = preg_replace('/' . '(.+)' . '\(\d*\)' . '(.*)' . '/', '$1$2', $tablefield_type_lowercased);
            ...
            if (array_key_exists($tablefield_field_lowercased, $cfields)) {
                ...
                $fieldtype_without_parentheses = preg_replace('/' . '(.+)' . '\(\d*\)' . '(.*)' . '/', '$1$2', $fieldtype_lowercased);
                // Get the type without attributes, e.g. `int`.
 ...
 */
function make_db_current($tables = 'all'): void
{
    $alterations = db_delta($tables);
    echo "<ol>\n";
 ...
 * @param string $tables Optional. Which set of tables to update. Default is 'all'.
 */
function make_db_current_silent($tables = 'all'): void
{
    db_delta($tables);
}
/**
 ...
 */
function make_site_theme_from_oldschool($theme_name, $template): bool
{
...
function make_site_theme_from_default($theme_name, $template)
{
    ...
    if ($stylelines) {
        $f = fopen("{$site_dir}/style.css", 'w');
        $headers = ['Theme Name:' => $theme_name, 'Theme URI:' => __get_option('url'), 'Description:' => 'Your theme.', 'Version:' => '1', 'Author:' => 'You'];
 ...
 */
function translate_level_to_role($level): string
{
    switch ($level) {
        case 10:
        case 9:
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 */
function wp_check_mysql_version(): void
{
    global $wpdb;
 ...
 * @since 2.2.0
 */
function maybe_disable_automattic_widgets(): void
{
 ...
 * @global wpdb $wpdb                  WordPress database abstraction object.
 */
function maybe_disable_link_manager(): void
{
    global $wp_current_db_version, $wpdb;
 ...
 * @global wpdb $wpdb                  WordPress database abstraction object.
 */
function pre_schema_upgrade(): void
{
    global $wp_current_db_version, $wpdb;
