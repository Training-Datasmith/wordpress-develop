...
final class WP_Screen
{
    ...
    public static function get($hook_name = '')
    {
        ...
        } else if (defined('WP_NETWORK_ADMIN') && WP_NETWORK_ADMIN) {
            $in_admin = 'network';
     ...
     */
    public function add_option($option, $args = []): void
    {
     ...
     * }
     */
    public function set_screen_reader_content($content = []): void
    {
        $defaults = ['heading_views' => __('Filter items list'), 'heading_pagination' => __('Items list navigation'), 'heading_list' => __('Items list')];
    ...
    public function render_screen_meta(): void
    {
         ...
         */
        self::$_old_compat_help = apply_filters_deprecated('contextual_help_list', [self::$_old_compat_help, $this], '3.3.0', 'get_current_screen()->add_help_tab(), get_current_screen()->remove_help_tab()');
         ...
         * @param WP_Screen $screen    Current WP_Screen instance.
         */
        $old_help = apply_filters_deprecated('contextual_help', [$old_help, $this->id, $this], '3.3.0', 'get_current_screen()->add_help_tab(), get_current_screen()->remove_help_tab()');
        ...
        if ($old_help) {
            $this->add_help_tab(['id' => 'old-contextual-help', 'title' => __('Overview'), 'content' => $old_help]);
        }
     ...
     * }
     */
    public function render_screen_options($options = []): void
    {
        $options = wp_parse_args($options, ['wrap' => true]);
        $wrapper_start = '';
        $wrapper_end = '';
        $form_start = '';
        $form_end = '';
    ...
    public function render_meta_boxes_preferences(): void
    {
        ...
        ?>
			<?php 
        _e('Expand or collapse the elements by clicking on their headings, and arrange them by dragging their headings or by clicking on the up and down arrows.');
        ?>
		</p>
