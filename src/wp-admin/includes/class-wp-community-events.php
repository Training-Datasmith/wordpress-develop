...
class WP_Community_Events
{
    ...
    public function get_events($location_search = '', $timezone = '')
    {
        ...
        if (is_wp_error($response)) {
            ...
            $response_error = new WP_Error('api-invalid-response', $response_body['error'] ?? __('Unknown API error.'));
        }
     ...
     */
    protected function get_request_args($search = '', $timezone = ''): array
    {
        $args = [
            'number' => 5,
    ...
    public static function get_unsafe_client_ip()
    {
        ...
        $address_headers = ['HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR'];
     ...
     */
    protected function coordinates_match(array $a, array $b)
    {
     ...
     */
    protected function get_events_transient_key(array $location)
    {
        $key = false;
     ...
     */
    protected function cache_events(array $events, $expiration = false)
    {
        $set = false;
     ...
     */
    protected function format_event_data_time(array $response_body): array
    {
     ...
     */
    protected function trim_events(array $events): array
    {
        $future_events = [];
        ...
        $future_wordcamps = array_filter($future_events, static fn(array $wordcamp) => 'wordcamp' === $wordcamp['type']);
