...
class WP_Upgrader
{
     ...
     * @since 2.8.0
     */
    public function generic_strings(): void
    {
     ...
     */
    protected function flatten_dirlist($nested_files, string $path = ''): array
    {
        $files = [];
     ...
     */
    public function clear_destination(string $remote_destination)
    {
        global $wp_filesystem;
    ...
    public function install_package($args = [])
    {
        ...
        if (!is_string($source) || '' === $source || trim($source) !== $source || (!is_string($destination) || '' === $destination || trim($destination) !== $destination)) {
         ...
         */
        if ($args['clear_working'] && (!$wp_filesystem->exists($remote_destination) || empty($wp_filesystem->dirlist($remote_destination)))) {
    ...
    public function run($options)
    {
        ...
        if (is_wp_error($download) && $download->get_error_data('softfail-filename')) {
            ...
            if ('signature_verification_no_signature' !== $download->get_error_code() || WP_DEBUG) {
                ...
                wp_version_check(['signature_failure_code' => $download->get_error_code(), 'signature_failure_data' => $download->get_error_data()]);
            }
        ...
        $result = $this->install_package(['source' => $working_dir, 'destination' => $options['destination'], 'clear_destination' => $options['clear_destination'], 'abort_if_destination_exists' => $options['abort_if_destination_exists'], 'clear_working' => $options['clear_working'], 'hook_extra' => $options['hook_extra']]);
        /**
     ...
     */
    public static function create_lock(string $lock_name, $release_timeout = null)
    {
        global $wpdb;
        if (!$release_timeout) {
     ...
     */
    public static function release_lock(string $lock_name)
    {
     ...
     */
    public function move_to_temp_backup_dir(array $args)
    {
        global $wp_filesystem;
