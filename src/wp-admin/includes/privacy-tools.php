...
function _wp_privacy_completed_request($request_id)
{
    ...
    return wp_update_post(['ID' => $request_id, 'post_status' => 'request-completed']);
}
/**
 ...
 * @access private
 */
function _wp_personal_data_handle_actions(): void
{
 ...
 * @access private
 */
function _wp_personal_data_cleanup_requests(): void
{
    /** This filter is documented in wp-includes/user.php */
    $expires = (int) apply_filters('user_request_key_expiration', DAY_IN_SECONDS);
    $requests_query = new WP_Query(['post_type' => 'user_request', 'posts_per_page' => -1, 'post_status' => 'request-pending', 'fields' => 'ids', 'date_query' => [['column' => 'post_modified_gmt', 'before' => $expires . ' seconds ago']]]);
    $request_ids = $requests_query->posts;
    foreach ($request_ids as $request_id) {
        wp_update_post(['ID' => $request_id, 'post_status' => 'request-failed', 'post_password' => '']);
    }
}
/**
 ...
 */
function wp_privacy_generate_personal_data_export_group_html(array $group_data, string $group_id = '', $groups_count = 1): string
{
    ...
    $group_html .= '<div>';
    foreach ((array) $group_data['items'] as $group_item_data) {
        $group_html .= '<table>';
        $group_html .= '<tbody>';
 ...
 */
function wp_privacy_generate_personal_data_export_file($request_id): void
{
    ...
    $about_group = [
        ...
        'items' => ['about-1' => [['name' => _x('Report generated for', 'email address'), 'value' => $email_address], ['name' => _x('For site', 'website name'), 'value' => get_bloginfo('name')], ['name' => _x('At URL', 'website URL'), 'value' => get_bloginfo('url')], ['name' => _x('On', 'date/time'), 'value' => current_time('mysql')]]],
    ];
    ...
    $zip = new Zip_Archive();
    if (true === $zip->open($archive_pathname, Zip_Archive::CREATE)) {
        if (!$zip->add_file($json_report_pathname, 'export.json')) {
            $error = __('Unable to archive the personal data export file (JSON format).');
        }
        if (!$zip->add_file($html_report_pathname, 'index.html')) {
...
function wp_privacy_send_personal_data_export_email($request_id)
{
    ...
    $email_data = ['request' => $request, 'expiration' => $expiration, 'expiration_date' => $expiration_date, 'message_recipient' => $request_email, 'export_file_url' => $export_file_url, 'sitename' => $site_name, 'siteurl' => $site_url];
...
function wp_privacy_process_personal_data_export_page($response, $exporter_index, $email_address, $page, $request_id, $send_as_email, $exporter_key)
{
    ...
    foreach ($export_data as $export_datum) {
        ...
        if (!array_key_exists($group_id, $groups)) {
            $groups[$group_id] = ['group_label' => $group_label, 'group_description' => $group_description, 'items' => []];
        }
