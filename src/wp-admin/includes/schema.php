 ...
 */
function wp_get_db_schema($scope = 'all', $blog_id = null): string
{
    global $wpdb;
    $charset_collate = $wpdb->get_charset_collate();
 ...
 */
function populate_options(array $options = []): void
{
    ...
    $fat_options = ['moderation_keys', 'recently_edited', 'disallowed_keys', 'uninstall_plugins', 'auto_plugin_theme_update_emails'];
 ...
 * @since 2.0.0
 */
function populate_roles_160(): void
{
    // Add roles.
 ...
 * @since 2.1.0
 */
function populate_roles_210(): void
{
 ...
 * @since 2.3.0
 */
function populate_roles_230(): void
{
    $role = get_role('administrator');
    if (!empty($role)) {
 ...
 * @since 2.5.0
 */
function populate_roles_250(): void
{
    $role = get_role('administrator');
    if (!empty($role)) {
 ...
 * @since 2.6.0
 */
function populate_roles_260(): void
{
    $role = get_role('administrator');
    if (!empty($role)) {
 ...
 * @since 2.7.0
 */
function populate_roles_270(): void
{
    $role = get_role('administrator');
    if (!empty($role)) {
 ...
 * @since 2.8.0
 */
function populate_roles_280(): void
{
    $role = get_role('administrator');
    if (!empty($role)) {
 ...
 * @since 3.0.0
 */
function populate_roles_300(): void
{
    $role = get_role('administrator');
    if (!empty($role)) {
...
if (!function_exists('install_network')) {
     ...
     * @since 3.0.0
     */
    function install_network(): void
    {
        if (!defined('WP_INSTALLING_NETWORK')) {
            define('WP_INSTALLING_NETWORK', true);
        }
        db_delta(wp_get_db_schema('global'));
    }
}
/**
 ...
 */
function populate_network($network_id = 1, string $domain = '', $email = '', $site_name = '', $path = '/', $subdomain_install = false)
{
    ...
    } else if ($network_id === (int) $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->site} WHERE id = %d", $network_id))) {
        $errors->add('siteid_exists', __('The network already exists.'));
    }
    if (!is_email($email)) {
    ...
    populate_network_meta($network_id, ['admin_email' => $email, 'site_name' => $site_name, 'subdomain_install' => $subdomain_install]);
    ...
    if (!is_multisite()) {
        ...
        $site_user_id = (int) $wpdb->get_var($wpdb->prepare("SELECT meta_value\n\t\t\t\tFROM {$wpdb->sitemeta}\n\t\t\t\tWHERE meta_key = %s AND site_id = %d", 'admin_user_id', $network_id));
        ...
        $wpdb->insert($wpdb->sitemeta, ['site_id' => $network_id, 'meta_key' => 'main_site', 'meta_value' => $current_site->blog_id]);
 ...
 */
function populate_network_meta($network_id, array $meta = []): void
{
    ...
    if (!is_multisite()) {
        ...
        $users = get_users(['fields' => ['user_login'], 'role' => 'administrator']);
        if ($users) {
 ...
 * @param array $meta    Optional. Custom meta $key => $value pairs to use. Default empty array.
 */
function populate_site_meta($site_id, array $meta = []): void
{
    global $wpdb;
    $site_id = (int) $site_id;
