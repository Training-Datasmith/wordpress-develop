...
{
    private static ?\WP_Site_Health $instance = null;
    private ?bool $is_acceptable_mysql_version = null;
    private ?bool $is_recommended_mysql_version = null;
    public $is_mariadb = false;
    private $mysql_server_version = '';
    private string $mysql_required_version = '5.5';
    private $mysql_recommended_version = '8.0';
    private string $mariadb_recommended_version = '10.6';
    /**
     * @var string
     */
     ...
     */
    public function show_site_health_tab($tab): void
    {
        if ('debug' === $tab) {
    ...
    public function enqueue_scripts(): void
    {
        ...
        $health_check_js_variables = ['screen' => $screen->id, 'nonce' => ['site_status' => wp_create_nonce('health-check-site-status'), 'site_status_result' => wp_create_nonce('health-check-site-status-result')], 'site_status' => ['direct' => [], 'async' => [], 'issues' => ['good' => 0, 'recommended' => 0, 'critical' => 0]]];
        ...
        if ('site-health' === $screen->id && (!isset($_GET['tab']) || empty($_GET['tab']))) {
            ...
            foreach ($tests['async'] as $test) {
                if (is_string($test['test'])) {
                    $health_check_js_variables['site_status']['async'][] = ['test' => $test['test'], 'has_rest' => $test['has_rest'] ?? false, 'completed' => false, 'headers' => $test['headers'] ?? []];
                }
            }
        }
     ...
     */
    private function prepare_sql_data(): void
    {
        global $wpdb;
     ...
     * @since 5.2.0
     */
    public function check_wp_version_check_exists(): void
    {
    ...
    {
        $result = ['label' => '', 'status' => '', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => '', 'actions' => '', 'test' => 'wordpress_version'];
    ...
    public function get_test_php_version(): array
    {
        ...
        $result = ['label' => sprintf(
            ...
            PHP_VERSION
        ), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('PHP is one of the programming languages used to build WordPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.')), 'actions' => sprintf(
     ...
     */
    private function test_php_extension_availability($extension_name = null, $function_name = null, $constant_name = null, $class_name = null): bool
    {
    ...
    {
        $result = ['label' => __('Required and recommended modules are installed'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p><p>%s</p>', __('PHP modules perform most of the tasks on the server that make your site run. Any changes to these must be made by your server administrator.'), sprintf(
        ...
        $modules = [
            'curl' => ['function' => 'curl_version', 'required' => false],
            'dom' => ['class' => 'DOMNode', 'required' => false],
            'exif' => ['function' => 'exif_read_data', 'required' => false],
            'fileinfo' => ['function' => 'finfo_file', 'required' => false],
            'hash' => ['function' => 'hash', 'required' => true],
            'imagick' => ['extension' => 'imagick', 'required' => false],
            'json' => ['function' => 'json_last_error', 'required' => true],
            'mbstring' => ['function' => 'mb_check_encoding', 'required' => false],
            'mysqli' => ['function' => 'mysqli_connect', 'required' => false],
            // Sodium was introduced in PHP 7.2, but the extension may not be enabled.
            'libsodium' => ['constant' => 'SODIUM_LIBRARY_VERSION', 'required' => false, 'php_bundled_version' => '7.2.0'],
            'openssl' => ['function' => 'openssl_encrypt', 'required' => false],
            'pcre' => ['function' => 'preg_match', 'required' => false],
            'mod_xml' => ['extension' => 'libxml', 'required' => false],
            'zip' => ['class' => 'ZipArchive', 'required' => false],
            'filter' => ['function' => 'filter_list', 'required' => false],
            'gd' => ['extension' => 'gd', 'required' => false, 'fallback_for' => 'imagick'],
            'iconv' => ['function' => 'iconv', 'required' => false],
            'intl' => ['extension' => 'intl', 'required' => false],
            'mcrypt' => ['extension' => 'mcrypt', 'required' => false, 'fallback_for' => 'libsodium'],
            'simplexml' => ['extension' => 'simplexml', 'required' => false, 'fallback_for' => 'mod_xml'],
            'xmlreader' => ['extension' => 'xmlreader', 'required' => false, 'fallback_for' => 'mod_xml'],
            'zlib' => ['extension' => 'zlib', 'required' => false, 'fallback_for' => 'zip'],
        ];
        /**
     ...
     * @return array The test results.
     */
    public function get_test_php_default_timezone(): array
    {
        $result = ['label' => __('PHP default timezone is valid'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('PHP default timezone was configured by WordPress on loading. This is necessary for correct calculations of dates and times.')), 'actions' => '', 'test' => 'php_default_timezone'];
    ...
    {
        $result = ['label' => __('No PHP sessions detected'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', sprintf(
     ...
     * @return array The test results.
     */
    public function get_test_sql_server(): array
    {
        if (!$this->mysql_server_version) {
            $this->prepare_sql_data();
        }
        $result = ['label' => __('SQL server is up to date'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('The SQL server is a required piece of software for the database WordPress uses to store all your site&#8217;s content and settings.')), 'actions' => sprintf(
            '<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
    ...
    {
        $result = ['label' => __('Can communicate with WordPress.org'), 'status' => '', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('Communicating with the WordPress servers is used to check for new versions, and to both install and update WordPress core, themes or plugins.')), 'actions' => '', 'test' => 'dotorg_communication'];
        $wp_dotorg = wp_remote_get('https://api.wordpress.org', ['timeout' => 10]);
    ...
    public function get_test_https_status(): array
    {
        ...
        $result = ['label' => __('Your website is using an active HTTPS connection'), 'status' => 'good', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('An HTTPS connection is a more secure way of browsing the web. Many services now have HTTPS as a requirement. HTTPS allows you to take advantage of new features that can increase site speed, improve search rankings, and gain the trust of your visitors by helping to protect their online privacy.')), 'actions' => sprintf(
            '<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
     ...
     * @return array The test result.
     */
    public function get_test_ssl_support(): array
    {
        $result = ['label' => '', 'status' => '', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('Securely communicating between servers are needed for transactions such as fetching files, conducting sales on store sites, and much more.')), 'actions' => '', 'test' => 'ssl_support'];
    ...
    {
        $result = ['label' => __('Background updates are working'), 'status' => 'good', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('Background updates ensure that WordPress can auto-update if a security update is released for the version you are currently using.')), 'actions' => '', 'test' => 'background_updates'];
     ...
     * @return array The test results.
     */
    public function get_test_plugin_theme_auto_updates(): array
    {
        $result = ['label' => __('Plugin and theme auto-updates appear to be configured correctly'), 'status' => 'good', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('Plugin and theme auto-updates ensure that the latest versions are always installed.')), 'actions' => '', 'test' => 'plugin_theme_auto_updates'];
     ...
     * @return array The test results.
     */
    public function get_test_available_updates_disk_space(): array
    {
        $available_space = function_exists('disk_free_space') ? @disk_free_space(WP_CONTENT_DIR) : false;
        $result = ['label' => __('Disk space available to safely perform updates'), 'status' => 'good', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf(
            /* translators: %s: Available disk space in MB or GB. */
            '<p>' . __('%s available disk space was detected, update routines can be performed safely.') . '</p>',
            size_format($available_space)
        ), 'actions' => '', 'test' => 'available_updates_disk_space'];
     ...
     * @return array The test results.
     */
    public function get_test_insecure_registration(): array
    {
        $users_can_register = get_option('users_can_register');
        $default_role = get_option('default_role');
        $result = ['label' => __('Open Registration with privileged default role'), 'status' => 'good', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => '<p>' . __('The combination of open registration setting and the default user role may lead to security issues.') . '</p>', 'actions' => '', 'test' => 'insecure_registration'];
        if ($users_can_register && in_array($default_role, ['editor', 'administrator'], true)) {
            $result['description'] = __('Registration is open to anyone, and the default role is set to a privileged role.');
            $result['status'] = 'critical';
            $result['actions'] = sprintf('<p><a href="%s">%s</a></p>', esc_url(admin_url('options-general.php')), __('Change these settings'));
        }
        return $result;
    }
    /**
     ...
     * @return array The test results.
     */
    public function get_test_update_temp_backup_writable(): array
    {
        global $wp_filesystem;
        $result = ['label' => __('Plugin and theme temporary backup directory is writable'), 'status' => 'good', 'badge' => ['label' => __('Security'), 'color' => 'blue'], 'description' => sprintf(
        ...
        ), 'actions' => '', 'test' => 'update_temp_backup_writable'];
    ...
    public function get_test_rest_availability(): array
    {
        ...
        if (is_wp_error($r)) {
            ...
            $result['description'] .= sprintf('<p>%s</p><p>%s<br>%s</p>', __('When testing the REST API, an error was encountered:'), sprintf(
            ...
            $result['description'] .= sprintf('<p>%s</p><p>%s<br>%s</p>', __('When testing the REST API, an unexpected result was returned:'), sprintf(
                // translators: %s: The REST API URL.
                __('REST API Endpoint: %s'),
                $url
            ), sprintf(
    ...
    {
        $result = ['label' => __('Files can be uploaded'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', sprintf(
    ...
    public function get_test_page_cache(): array
    {
        ...
        $result = ['badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => wp_kses_post($description), 'test' => 'page_cache', 'status' => 'good', 'label' => '', 'actions' => sprintf(
        ...
        if (empty($page_cache_detail['response_time'])) {
            ...
            if (empty($page_cache_detail['headers'])) {
                ...
                $headers_summary .= ' ' . sprintf(
                    ...
                    _n('There was %d client caching response header detected:', 'There were %d client caching response headers detected:', count($page_cache_detail['headers'])),
     ...
     * @return array The test result.
     */
    public function get_test_persistent_object_cache(): array
    {
        /**
        ...
        $result['description'] .= sprintf('<p>%s</p>', wp_kses($notes, ['a' => ['href' => true], 'code' => true, 'em' => true, 'strong' => true]));
        return $result;
    }
    /**
     ...
     * @return array The test results.
     */
    public function get_test_autoloaded_options(): array
    {
        ...
        $result = ['label' => __('Autoloaded options are acceptable'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf(
        ...
        ), 'actions' => '', 'test' => 'autoloaded_options'];
        /**
    ...
    public function get_test_search_engine_visibility(): array
    {
        ...
        if (!get_option('blog_public')) {
            ...
            $result['description'] = sprintf('<p>%s</p>', __('Your site is hidden from search engines. Consider enabling indexing if this is a public site.', 'default'));
        }
        return $result;
    }
    /**
    ...
    public function get_test_opcode_cache(): array
    {
        ...
        $result = ['label' => __('Opcode cache is enabled'), 'status' => 'good', 'badge' => ['label' => __('Performance'), 'color' => 'blue'], 'description' => sprintf('<p>%s</p>', __('Opcode cache improves PHP performance by storing precompiled script bytecode in memory, reducing the need for PHP to load and parse scripts on each request.')), 'actions' => sprintf(
            '<p><a href="%s" target="_blank">%s<span class="screen-reader-text"> %s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a></p>',
    ...
    {
        $tests = ['direct' => ['wordpress_version' => ['label' => __('WordPress Version'), 'test' => 'wordpress_version'], 'plugin_version' => ['label' => __('Plugin Versions'), 'test' => 'plugin_version'], 'theme_version' => ['label' => __('Theme Versions'), 'test' => 'theme_version'], 'php_version' => ['label' => __('PHP Version'), 'test' => 'php_version'], 'php_extensions' => ['label' => __('PHP Extensions'), 'test' => 'php_extensions'], 'php_default_timezone' => ['label' => __('PHP Default Timezone'), 'test' => 'php_default_timezone'], 'php_sessions' => ['label' => __('PHP Sessions'), 'test' => 'php_sessions'], 'sql_server' => ['label' => __('Database Server version'), 'test' => 'sql_server'], 'ssl_support' => ['label' => __('Secure communication'), 'test' => 'ssl_support'], 'scheduled_events' => ['label' => __('Scheduled events'), 'test' => 'scheduled_events'], 'http_requests' => ['label' => __('HTTP Requests'), 'test' => 'http_requests'], 'rest_availability' => ['label' => __('REST API availability'), 'test' => 'rest_availability', 'skip_cron' => true], 'debug_enabled' => ['label' => __('Debugging enabled'), 'test' => 'is_in_debug_mode'], 'file_uploads' => ['label' => __('File uploads'), 'test' => 'file_uploads'], 'plugin_theme_auto_updates' => ['label' => __('Plugin and theme auto-updates'), 'test' => 'plugin_theme_auto_updates'], 'update_temp_backup_writable' => ['label' => __('Plugin and theme temporary backup directory access'), 'test' => 'update_temp_backup_writable'], 'available_updates_disk_space' => ['label' => __('Available disk space'), 'test' => 'available_updates_disk_space'], 'autoloaded_options' => ['label' => __('Autoloaded options'), 'test' => 'autoloaded_options'], 'insecure_registration' => ['label' => __('Open Registration with privileged default role'), 'test' => 'insecure_registration'], 'search_engine_visibility' => ['label' => __('Search Engine Visibility'), 'test' => 'search_engine_visibility'], 'opcode_cache' => ['label' => __('Opcode cache'), 'test' => 'opcode_cache']], 'async' => ['dotorg_communication' => ['label' => __('Communication with WordPress.org'), 'test' => rest_url('wp-site-health/v1/tests/dotorg-communication'), 'has_rest' => true, 'async_direct_test' => [WP_Site_Health::get_instance(), 'get_test_dotorg_communication']], 'background_updates' => ['label' => __('Background updates'), 'test' => rest_url('wp-site-health/v1/tests/background-updates'), 'has_rest' => true, 'async_direct_test' => [WP_Site_Health::get_instance(), 'get_test_background_updates']], 'loopback_requests' => ['label' => __('Loopback request'), 'test' => rest_url('wp-site-health/v1/tests/loopback-requests'), 'has_rest' => true, 'async_direct_test' => [WP_Site_Health::get_instance(), 'get_test_loopback_requests']], 'https_status' => ['label' => __('HTTPS status'), 'test' => rest_url('wp-site-health/v1/tests/https-status'), 'has_rest' => true, 'async_direct_test' => [WP_Site_Health::get_instance(), 'get_test_https_status']]]];
        ...
        if (!wp_is_site_protected_by_basic_auth()) {
            $tests['async']['authorization_header'] = ['label' => __('Authorization header'), 'test' => rest_url('wp-site-health/v1/tests/authorization-header'), 'has_rest' => true, 'headers' => ['Authorization' => 'Basic ' . base64_encode('user:pwd')], 'skip_cron' => true];
        }
        // Only check for caches in production environments.
        if ('production' === wp_get_environment_type()) {
            $tests['async']['page_cache'] = ['label' => __('Page cache'), 'test' => rest_url('wp-site-health/v1/tests/page-cache'), 'has_rest' => true, 'async_direct_test' => [WP_Site_Health::get_instance(), 'get_test_page_cache']];
            $tests['direct']['persistent_object_cache'] = ['label' => __('Persistent object cache'), 'test' => 'persistent_object_cache'];
        }
        /**
        ...
        $tests = array_merge(['direct' => [], 'async' => []], $tests);
        return $tests;
    }
    /**
     ...
     * @since 5.2.0
     */
    private function wp_schedule_test_init(): void
    {
     ...
     * @since 5.2.0
     */
    private function get_cron_tasks(): void
    {
        ...
        foreach ($cron_tasks as $time => $cron) {
            foreach ($cron as $hook => $dings) {
                foreach ($dings as $sig => $data) {
                    $this->crons["{$hook}-{$sig}-{$time}"] = (object) ['hook' => $hook, 'time' => $time, 'sig' => $sig, 'args' => $data['args'], 'schedule' => $data['schedule'], 'interval' => $data['interval'] ?? null];
                }
            }
        }
    }
    ...
    {
        $mock_plugin = (object) ['id' => 'w.org/plugins/a-fake-plugin', 'slug' => 'a-fake-plugin', 'plugin' => 'a-fake-plugin/a-fake-plugin.php', 'new_version' => '9.9', 'url' => 'https://wordpress.org/plugins/a-fake-plugin/', 'package' => 'https://downloads.wordpress.org/plugin/a-fake-plugin.9.9.zip', 'icons' => ['2x' => 'https://ps.w.org/a-fake-plugin/assets/icon-256x256.png', '1x' => 'https://ps.w.org/a-fake-plugin/assets/icon-128x128.png'], 'banners' => ['2x' => 'https://ps.w.org/a-fake-plugin/assets/banner-1544x500.png', '1x' => 'https://ps.w.org/a-fake-plugin/assets/banner-772x250.png'], 'banners_rtl' => [], 'tested' => '5.5.0', 'requires_php' => '5.6.20', 'compatibility' => new stdClass()];
        $mock_theme = (object) ['theme' => 'a-fake-theme', 'new_version' => '9.9', 'url' => 'https://wordpress.org/themes/a-fake-theme/', 'package' => 'https://downloads.wordpress.org/theme/a-fake-theme.9.9.zip', 'requires' => '5.0.0', 'requires_php' => '5.6.20'];
        ...
        if (!$test_plugins_enabled && $ui_enabled_for_plugins || !$test_themes_enabled && $ui_enabled_for_themes) {
            return (object) ['status' => 'critical', 'message' => __('Auto-updates for plugins and/or themes appear to be disabled, but settings are still set to be displayed. This could cause auto-updates to not work as expected.')];
        }
        if (!$test_plugins_enabled && $plugin_filter_present && (!$test_themes_enabled && $theme_filter_present)) {
            return (object) ['status' => 'recommended', 'message' => __('Auto-updates for plugins and themes appear to be disabled. This will prevent your site from receiving new versions automatically when available.')];
        }
        if (!$test_plugins_enabled && $plugin_filter_present) {
            return (object) ['status' => 'recommended', 'message' => __('Auto-updates for plugins appear to be disabled. This will prevent your site from receiving new versions automatically when available.')];
        }
        if (!$test_themes_enabled && $theme_filter_present) {
            return (object) ['status' => 'recommended', 'message' => __('Auto-updates for themes appear to be disabled. This will prevent your site from receiving new versions automatically when available.')];
        }
        return (object) ['status' => 'good', 'message' => __('There appear to be no issues with plugin and theme auto-updates.')];
    }
    /**
    ...
    public function can_perform_loopback()
    {
        ...
        if (is_wp_error($r)) {
            return (object) ['status' => 'critical', 'message' => sprintf('%s<br>%s', __('The loopback request to your site failed, this means features relying on them are not currently working as expected.'), sprintf(
        ...
        if (200 !== wp_remote_retrieve_response_code($r)) {
            return (object) ['status' => 'recommended', 'message' => sprintf(
        ...
        return (object) ['status' => 'good', 'message' => __('The loopback request to your site completed successfully.')];
    }
    /**
     ...
     * @since 5.4.0
     */
    public function maybe_create_scheduled_event(): void
    {
    ...
    public function wp_cron_scheduled_check(): void
    {
        ...
        $tests = WP_Site_Health::get_tests();
        $results = [];
        $site_status = ['good' => 0, 'recommended' => 0, 'critical' => 0];
        // Don't run https test on development environments.
        if ($this->is_development_environment()) {
            unset($tests['async']['https_status']);
        }
        ...
        foreach ($tests['async'] as $test) {
            ...
            if (is_string($test['test'])) {
                ...
                if (isset($test['has_rest']) && $test['has_rest']) {
                    $result_fetch = wp_remote_get($test['test'], ['body' => ['_wpnonce' => wp_create_nonce('wp_rest')]]);
                } else {
                    $result_fetch = wp_remote_post(admin_url('admin-ajax.php'), ['body' => ['action' => $test['test'], '_wpnonce' => wp_create_nonce('health-check-site-status')]]);
                }
                ...
                if (is_array($result)) {
                    ...
                    $results[] = ['status' => 'recommended', 'label' => __('A test is unavailable')];
                }
            }
        }
    ...
    {
        $cache_hit_callback = static fn($header_value) => 1 === preg_match('/(^| |,)HIT(,| |$)/i', $header_value);
        $cache_headers = [
            // Standard HTTP caching headers.
            'cache-control' => static fn($header_value) => (bool) preg_match('/max-age=[1-9]/', $header_value),
            'expires' => static fn($header_value) => strtotime($header_value) > time(),
            'age' => static fn($header_value) => is_numeric($header_value) && $header_value > 0,
            'last-modified' => null,
            'etag' => null,
            'via' => null,
            /**
             ...
             */
            'x-cache-enabled' => static fn($header_value) => 'true' === strtolower($header_value),
            'x-cache-disabled' => static fn($header_value) => 'on' !== strtolower($header_value),
            /**
             * CloudFlare.
             *
             ...
             */
            'x-srcache-store-status' => static fn($header_value) => 'store' === strtolower($header_value),
             ...
             */
            'x-varnish' => static fn($header_value) => 1 === preg_match('/^\d+ \d+/', $header_value),
        ];
        /**
    ...
    private function check_for_page_caching()
    {
        ...
        return ['advanced_cache_present' => file_exists(WP_CONTENT_DIR . '/advanced-cache.php') && (defined('WP_CACHE') && WP_CACHE) && apply_filters('enable_loading_advanced_cache_dropin', true), 'page_caching_response_headers' => $page_caching_response_headers, 'response_timing' => $response_timing];
    }
    /**
    ...
    private function get_page_cache_detail()
    {
        ...
        return ['status' => $result, 'advanced_cache_present' => $page_cache_detail['advanced_cache_present'], 'headers' => $headers, 'response_time' => $page_speed];
    }
    /**
     ...
     */
    private function get_good_response_time_threshold(): int
    {
        /**
    ...
    public function should_suggest_persistent_object_cache()
    {
         ...
         */
        $thresholds = apply_filters('site_status_persistent_object_cache_thresholds', ['alloptions_count' => 500, 'alloptions_bytes' => 100000, 'comments_count' => 1000, 'options_count' => 1000, 'posts_count' => 1000, 'terms_count' => 1000, 'users_count' => 1000]);
        $alloptions = wp_load_alloptions();
        ...
        $threshold_map = ['comments_count' => $wpdb->comments, 'options_count' => $wpdb->options, 'posts_count' => $wpdb->posts, 'terms_count' => $wpdb->terms, 'users_count' => $wpdb->users];
    ...
    {
        $extensions = array_map('extension_loaded', ['APCu' => 'apcu', 'Redis' => 'redis', 'Relay' => 'relay', 'Memcache' => 'memcache', 'Memcached' => 'memcached']);
