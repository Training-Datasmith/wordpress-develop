...
foreach (get_taxonomies_for_attachments('objects') as $taxonomy) {
    ...
    $submenu['upload.php'][ $submenu_index++ ] = [ esc_attr($taxonomy->labels->menu_name), $taxonomy->cap->manage_terms, 'edit-tags.php?taxonomy=' . $taxonomy->name . '&amp;post_type=attachment' ];
}
...
foreach (array_merge($builtin, $post_types) as $post_type) {
    ...
    $menu[ $post_type_menu_position ] = [ esc_attr($post_type_obj->labels->menu_name), $post_type_obj->cap->edit_posts, $post_type_file, '', $menu_class, $post_type_menu_id, $menu_icon ];
    $submenu[ $post_type_file ][5]    = [ $post_type_obj->labels->all_items, $post_type_obj->cap->edit_posts, $post_type_file ];
    $submenu[ $post_type_file ][10]   = [ $post_type_obj->labels->add_new_item, $post_type_obj->cap->create_posts, $post_new_file ];
    ...
    foreach (get_taxonomies([], 'objects') as $taxonomy) {
        ...
        $submenu[ $post_type_file ][ $submenu_index++ ] = [ esc_attr($taxonomy->labels->menu_name), $taxonomy->cap->manage_terms, sprintf($edit_tags_file, $taxonomy->name) ];
    }
}
...
$menu[60] = [ __('Appearance'), $appearance_capability, 'themes.php', '', 'menu-top menu-icon-appearance', 'menu-appearance', 'dashicons-admin-appearance' ];
...
$submenu['themes.php'][5] = [ sprintf(__('Themes %s'), $count), $appearance_capability, 'themes.php' ];
...
$submenu['themes.php'][9] = [ __('Fonts'), 'edit_theme_options', 'font-library.php' ];
...
if (current_theme_supports('custom-header') && current_user_can('customize')) {
    ...
    $submenu['themes.php'][15] = [ _x('Header', 'custom image header'), $appearance_capability, esc_url($customize_header_url), '', 'hide-if-no-customize' ];
}

if (current_theme_supports('custom-background') && current_user_can('customize')) {
    $customize_background_url  = add_query_arg([ 'autofocus' => [ 'control' => 'background_image' ] ], $customize_url);
    $submenu['themes.php'][20] = [ _x('Background', 'custom background'), $appearance_capability, esc_url($customize_background_url), '', 'hide-if-no-customize' ];
}
 ...
 */
function _add_themes_utility_last(): void
{
    add_submenu_page(
 ...
 * @since 5.9.0
 */
function _add_plugin_file_editor_to_tools(): void
{
    if (! wp_is_block_theme()) {
        return;
    }
...
$submenu['options-general.php'][12] = [ __('Connectors'), 'manage_options', 'options-connectors.php' ];
