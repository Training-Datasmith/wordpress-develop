...
if ($key) {
    ...
    if (remove_query_arg(false) !== $redirect_url) {
        setcookie($activate_cookie, $key, ['expires' => 0, 'path' => $activate_path, 'domain' => COOKIE_DOMAIN, 'secure' => is_ssl(), 'httponly' => true]);
...
if (null === $result && isset($_COOKIE[ $activate_cookie ])) {
    ...
    $result = wpmu_activate_signup($key);
    setcookie($activate_cookie, ' ', ['expires' => time() - YEAR_IN_SECONDS, 'path' => $activate_path, 'domain' => COOKIE_DOMAIN, 'secure' => is_ssl(), 'httponly' => true]);
}
 ...
 * @since MU (3.0.0)
 */
function wpmu_activate_stylesheet(): void
{
    ?>
	<style>
