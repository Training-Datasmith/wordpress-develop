...
function twentytwenty_site_logo($args = [], $display = true)
{
    ...
    $contents = '';
    $classname = '';
    $defaults = ['logo' => '%1$s<span class="screen-reader-text">%2$s</span>', 'logo_class' => 'site-logo', 'title' => '<a href="%1$s" rel="home">%2$s</a>', 'title_class' => 'site-title', 'home_wrap' => '<h1 class="%1$s">%2$s</h1>', 'single_wrap' => '<div class="%1$s faux-heading">%2$s</div>', 'condition' => (is_front_page() || is_home()) && !is_page()];
    ...
    if (has_custom_logo()) {
        ...
        if ((is_front_page() || is_home() && (int) get_option('page_for_posts') !== get_queried_object_id()) && !is_paged() && $args['title'] === $defaults['title']) {
 ...
 */
function twentytwenty_filter_comment_reply_link($link): string
{
    return str_replace('class=\'', 'class=\'do-not-scroll ', $link);
}
...
function twentytwenty_get_post_meta($post_id = null, $location = 'single-top')
{
    ...
    if ('single-top' === $location) {
         ...
         * }
         */
        $post_meta = apply_filters('twentytwenty_post_meta_location_single_top', ['author', 'post-date', 'comments', 'sticky']);
         ...
         * }
         */
        $post_meta = apply_filters('twentytwenty_post_meta_location_single_bottom', ['tags']);
 ...
 */
function twentytwenty_filter_wp_list_pages_item_classes($css_class, $page, $depth, array $args)
{
...
function twentytwenty_nav_menu_social_icons($item_output, $item, $depth, $args)
{
    ...
    if ('social' === $args->theme_location) {
        $svg = Twenty_Twenty_svg_icons::get_social_link_svg($item->url);
        if (empty($svg)) {
...
function twentytwenty_get_the_archive_title($title)
{
     ...
     */
    $regex = apply_filters('twentytwenty_get_the_archive_title_regex', ['pattern' => '/(\A[^\:]+\:)/', 'replacement' => '<span class="color-accent">$1</span>']);
    if (empty($regex)) {
        return $title;
    }
 ...
 */
function twentytwenty_unique_id(string $prefix = '')
{
    static $id_counter = 0;
