 ...
 */
function twentynineteen_pingback_header(): void
{
 ...
 */
function twentynineteen_comment_form_defaults(array $defaults): array
{
 ...
 */
function twentynineteen_post_thumbnail_sizes_attr(array $attr): array
{
 ...
 */
function twentynineteen_add_ellipses_to_nav(string $nav_menu, $args): string
{
 ...
 */
function twentynineteen_nav_menu_link_attributes(array $atts, $item, $args): array
{
...
function twentynineteen_link_pages_link($link, $i)
{
    if ($i > 1 && preg_match('/href="([^"]*)"/', $link, $matches)) {
        return str_replace($matches[1], $matches[1] . '#content', $link);
    }
    return $link;
}
