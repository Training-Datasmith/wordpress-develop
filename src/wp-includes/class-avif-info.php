...
class Features
{
    ...
    private function get_item_features($target_item_id, $tile_depth)
    {
        foreach ($this->props as $prop) {
            ...
            if ($target_item_id == $this->primary_item_id && ($this->primary_item_features['width'] == UNDEFINED || $this->primary_item_features['height'] == UNDEFINED)) {
                foreach ($this->dim_props as $dim_prop) {
                    ...
                    if ($this->primary_item_features['bit_depth'] != UNDEFINED && $this->primary_item_features['num_channels'] != UNDEFINED) {
                        return FOUND;
                    }
                    break;
                }
            ...
            if ($this->primary_item_features['bit_depth'] == UNDEFINED || $this->primary_item_features['num_channels'] == UNDEFINED) {
                foreach ($this->chan_props as $chan_prop) {
                    ...
                    if ($this->primary_item_features['width'] != UNDEFINED && $this->primary_item_features['height'] != UNDEFINED) {
                        return FOUND;
                    }
                    break;
                }
...
class Box
{
    ...
    public function parse($handle, &$num_parsed_boxes, $num_remaining_bytes = MAX_SIZE)
    {
        ...
        $has_fullbox_header = $this->type == 'meta' || $this->type == 'pitm' || $this->type == 'ipma' || $this->type == 'ispe' || $this->type == 'pixi' || $this->type == 'iref' || $this->type == 'auxC';
        ...
        if ($has_fullbox_header) {
            ...
            $is_parsable = $this->type == 'meta' && $this->version <= 0 || $this->type == 'pitm' && $this->version <= 1 || $this->type == 'ipma' && $this->version <= 1 || $this->type == 'ispe' && $this->version <= 0 || $this->type == 'pixi' && $this->version <= 0 || $this->type == 'iref' && $this->version <= 1 || $this->type == 'auxC' && $this->version <= 0;
    ...
    private function parse_ipco($num_remaining_bytes)
    {
        ...
        do {
            ...
            if ($box->type == 'ispe') {
                ...
                if (count($this->features->dim_props) <= MAX_FEATURES && $box_index <= MAX_VALUE) {
                ...
                if (count($this->features->chan_props) <= MAX_FEATURES && $box_index <= MAX_VALUE && $bit_depth <= MAX_VALUE && $num_channels <= MAX_VALUE) {
                ...
                if (count($this->features->chan_props) <= MAX_FEATURES && $box_index <= MAX_VALUE) {
                    $chan_prop_count = count($this->features->chan_props);
                    $this->features->chan_props[$chan_prop_count] = new Chan_Prop();
                    $this->features->chan_props[$chan_prop_count]->property_index = $box_index;
                    $this->features->chan_props[$chan_prop_count]->bit_depth = $high_bitdepth ? $twelve_bit ? 12 : 10 : 8;
                ...
                $k_alpha_str = "urn:mpeg:mpegB:cicp:systems:auxiliary:alpha\x00";
                $k_alpha_str_length = 44;
                // Includes terminating character.
                if ($box->content_size >= $k_alpha_str_length) {
                    if (!$data = read($this->handle, $k_alpha_str_length)) {
                        return TRUNCATED;
                    }
                    if (substr($data, 0, $k_alpha_str_length) == $k_alpha_str) {
                        // Note: It is unlikely but it is possible that this alpha plane does
                        //       not belong to the primary item or a tile. Ignore this issue.
                        $this->features->has_alpha = true;
                    }
                    if (!skip($this->handle, $box->content_size - $k_alpha_str_length)) {
                        return TRUNCATED;
                    }
                } else if (!skip($this->handle, $box->content_size)) {
                    return TRUNCATED;
                }
            } else if (!skip($this->handle, $box->content_size)) {
                return TRUNCATED;
        ...
        do {
            ...
            if ($box->type == 'ipco') {
                ...
                for ($entry = 0; $entry < $entry_count; ++$entry) {
                    if ($entry >= MAX_PROPS || count($this->features->props) >= MAX_PROPS) {
                        $this->data_was_skipped = true;
                        break;
                    }
                    ...
                    $item_id = read_big_endian(substr($data, 0, $id_num_bytes), $id_num_bytes);
                    $association_count = read_big_endian(substr($data, $id_num_bytes, 1), 1);
                    for ($property = 0; $property < $association_count; ++$property) {
                        if ($property >= MAX_PROPS || count($this->features->props) >= MAX_PROPS) {
                            $this->data_was_skipped = true;
                            break;
                        }
        ...
        do {
            ...
            if ($box->type == 'dimg') {
                ...
                for ($i = 0; $i < $reference_count; ++$i) {
                    ...
                    if ($from_item_id <= MAX_VALUE && $to_item_id <= MAX_VALUE && $tile_count < MAX_TILES) {
