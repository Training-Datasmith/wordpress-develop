...
function wp_count_posts($type = 'post', $perm = '')
{
    ...
    if (
		...
		";
        $args  = [ $type, $type, get_current_user_id() ];
    } else {
        $query = "
			SELECT post_status, COUNT(*) AS num_posts
			FROM {$wpdb->posts}
...
function _update_term_count_on_transition_post_status($new_status, $old_status, $post)
{
    ...
    foreach ((array) get_object_taxonomies($post->post_type, 'objects') as $taxonomy) {
        ...
        $counted_statuses = apply_filters('update_post_term_count_statuses', [ 'publish' ], $taxonomy);
        ...
        $tt_ids = wp_get_object_terms($post->ID, $taxonomy->name, [ 'fields' => 'tt_ids' ]);
