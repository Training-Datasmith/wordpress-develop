...
function wp_default_scripts($scripts)
{
    ...
    $scripts->add('wp-plupload', "/wp-includes/js/plupload/wp-plupload$suffix.js", [ 'plupload', 'jquery', 'media-models' ], false, 1);
    ...
    $scripts->add('backbone', "/wp-includes/js/backbone$dev_suffix.js", [ 'underscore', 'jquery' ], '1.6.1', 1);
    ...
    $scripts->add('wp-codemirror', '/wp-includes/js/codemirror/codemirror.min.js', [], '5.65.20');
    did_action('init') && $scripts->add_data('wp-codemirror', 'module_dependencies', [ 'espree' ]);
    $scripts->add('csslint', '/wp-includes/js/codemirror/csslint.js', [], '1.0.5');
    $scripts->add('esprima', '/wp-includes/js/codemirror/esprima.js', [], '4.0.1'); // Deprecated. Use 'espree' script module.
    $scripts->add('jshint', '/wp-includes/js/codemirror/fakejshint.js', [ 'esprima' ], '2.9.5'); // Deprecated.
    $scripts->add('jsonlint', '/wp-includes/js/codemirror/jsonlint.js', [], '1.6.3');
    $scripts->add('htmlhint', '/wp-includes/js/codemirror/htmlhint.js', [], '1.8.0');
    ...
    $scripts->add('customize-base', "/wp-includes/js/customize-base$suffix.js", [ 'jquery', 'underscore' ], false, 1);
    ...
    if (is_admin()) {
        ...
        $scripts->add('dashboard', "/wp-admin/js/dashboard$suffix.js", [ 'common', 'jquery', 'admin-comments', 'postbox', 'wp-util', 'wp-a11y', 'wp-date' ], false, 1);
        ...
        $scripts->add('image-edit', "/wp-admin/js/image-edit$suffix.js", [ 'jquery', 'jquery-ui-core', 'imgareaselect', 'wp-a11y' ], false, 1);
         ...
         */
        $scripts->add('nav-menu', "/wp-admin/js/nav-menu$suffix.js", [ 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'wp-lists', 'postbox', 'underscore' ]);
...
function wp_default_styles($styles)
{
    ...
    $styles->add('wp-admin', false, [ 'dashicons', 'common', 'forms', 'admin-menu', 'dashboard', 'list-tables', 'edit', 'revisions', 'media', 'themes', 'about', 'nav-menus', 'widgets', 'site-icon', 'l10n', 'wp-base-styles' ]);

    $styles->add('login', "/wp-admin/css/login$suffix.css", [ 'dashicons', 'buttons', 'forms', 'l10n', 'wp-base-styles' ]);
    $styles->add('install', "/wp-admin/css/install$suffix.css", [ 'dashicons', 'buttons', 'forms', 'l10n', 'wp-base-styles' ]);
    ...
    $styles->add('wp-codemirror', '/wp-includes/js/codemirror/codemirror.min.css', [], '5.65.20');
    ...
    if ($is_block_theme) {
        ...
        if ($custom_css || is_customize_preview()) {
            if (is_customize_preview()) {
                ...
                $custom_css       = str_replace([ $before_milestone, $after_milestone ], '', $custom_css);
...
function wp_hoist_late_printed_styles()
{
    ...
    $style_handles_at_enqueue_block_assets = [];
    add_action(
    ...
    $capture_late_styles = static function () use (&$printed_core_block_styles, &$printed_other_block_styles, &$printed_global_styles, &$printed_late_styles) {
        ...
        if (wp_style_is('global-styles')) {
            ob_start();
            wp_styles()->do_items([ 'global-styles' ]);
