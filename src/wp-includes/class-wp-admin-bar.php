...
class WP_Admin_Bar
{
    ...
    public function add_node($args)
    {
        ...
        $back_compat_parents = ['my-account-with-avatar' => ['my-account', '3.3'], 'my-blogs' => ['my-sites', '3.3']];
        ...
        foreach ($this->_get_nodes() as $node) {
            ...
            if ('item' === $parent->type && 'item' === $node->type) {
                ...
                if (!$default) {
                     ...
                     */
                    $this->_set_node(['id' => $default_id, 'parent' => $parent->id, 'type' => 'group', 'children' => [], 'meta' => ['class' => $group_class], 'title' => false, 'href' => false]);
                    $default = $this->_get_node($default_id);
                ...
                if (!$container) {
                     ...
                     * Make sure to specify default settings for all properties.
                     */
                    $this->_set_node(['id' => $container_id, 'type' => 'container', 'children' => [$parent], 'parent' => false, 'title' => false, 'href' => false, 'meta' => []]);
                    $container = $this->_get_node($container_id);
