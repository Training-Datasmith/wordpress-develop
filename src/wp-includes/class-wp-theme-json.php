...
class WP_Theme_JSON
{
     ...
     * @var array
     */
    public const VALID_BLOCK_PSEUDO_SELECTORS = [
    ...
    protected function get_layout_styles($block_metadata, $options = [])
    {
        ...
        $node                     = $options['node'] ?? _wp_array_get($this->theme_json, $block_metadata['path'], []);
                ...
                if (isset(static::VALID_BLOCK_PSEUDO_SELECTORS[ $name ])) {
                    foreach (static::VALID_BLOCK_PSEUDO_SELECTORS[ $name ] as $pseudo_selector) {
                        if (isset($theme_json['styles']['blocks'][ $name ][ $pseudo_selector ])) {
                            $nodes[] = [
                                'name'       => $name,
                                'path'       => [ 'styles', 'blocks', $name, $pseudo_selector ],
    ...
    public function get_styles_for_block($block_metadata)
    {
        ...
        $style_variation_layout_metadata = [];
        if (! empty($block_metadata['variations'])) {
            foreach ($block_metadata['variations'] as $style_variation) {
                ...
                if (isset($style_variation_node['spacing']['blockGap'])) {
                    ...
                    $style_variation_layout_metadata[ $style_variation['selector'] ] = [
        ...
        if ($pseudo_selector && isset($node[ $pseudo_selector ]) &&
            ...
            $block_name  = $block_metadata['path'][2]; // 'core/button'
            $block_data  = _wp_array_get($this->theme_json, [ 'styles', 'blocks', $block_name ], []);
            $pseudo_data = $block_data[ $block_pseudo_selector ] ?? [];
        ...
        foreach ($style_variation_declarations as $style_variation_selector => $individual_style_variation_declarations) {
            ...
            if (isset($style_variation_layout_metadata[ $style_variation_selector ])) {
                ...
                $block_rules   .= $this->get_layout_styles($variation_data['metadata'], [ 'node' => $variation_data['node'] ]);
            }
        ...
        foreach ($nodes as $node) {
            ...
            foreach (static::PRESETS_METADATA as $preset_metadata) {
                ...
                if (is_array($prevent_override)) {
                    $global_path  = array_merge([ 'settings' ], $prevent_override);
