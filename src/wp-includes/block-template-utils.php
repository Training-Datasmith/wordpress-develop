...
{
    $default_area_definitions = [['area' => WP_TEMPLATE_PART_AREA_UNCATEGORIZED, 'label' => _x('General', 'template part area'), 'description' => __('General templates often perform a specific role like displaying post content, and are not tied to any particular area.'), 'icon' => 'layout', 'area_tag' => 'div'], ['area' => WP_TEMPLATE_PART_AREA_HEADER, 'label' => _x('Header', 'template part area'), 'description' => __('The Header template defines a page area that typically contains a title, logo, and main navigation.'), 'icon' => 'header', 'area_tag' => 'header'], ['area' => WP_TEMPLATE_PART_AREA_FOOTER, 'label' => _x('Footer', 'template part area'), 'description' => __('The Footer template defines a page area that typically contains site credits, social links, or any other combination of blocks.'), 'icon' => 'footer', 'area_tag' => 'footer'], ['area' => WP_TEMPLATE_PART_AREA_NAVIGATION_OVERLAY, 'label' => _x('Navigation Overlay', 'template part area'), 'description' => __('The Navigation Overlay template defines an overlay area that typically contains navigation links and can be toggled open and closed.'), 'icon' => 'navigation-overlay', 'area_tag' => 'div']];
    /**
...
{
    $default_template_types = ['index' => ['title' => _x('Index', 'Template name'), 'description' => __('Used as a fallback template for all pages when a more specific template is not defined.')], 'home' => ['title' => _x('Blog Home', 'Template name'), 'description' => __('Displays the latest posts as either the site homepage or as the "Posts page" as defined under reading settings. If it exists, the Front Page template overrides this template when posts are shown on the homepage.')], 'front-page' => ['title' => _x('Front Page', 'Template name'), 'description' => __('Displays your site\'s homepage, whether it is set to display latest posts or a static page. The Front Page template takes precedence over all templates.')], 'singular' => ['title' => _x('Single Entries', 'Template name'), 'description' => __('Displays any single entry, such as a post or a page. This template will serve as a fallback when a more specific template (e.g. Single Post, Page, or Attachment) cannot be found.')], 'single' => ['title' => _x('Single Posts', 'Template name'), 'description' => __('Displays a single post on your website unless a custom template has been applied to that post or a dedicated template exists.')], 'page' => ['title' => _x('Pages', 'Template name'), 'description' => __('Displays a static page unless a custom template has been applied to that page or a dedicated template exists.')], 'archive' => ['title' => _x('All Archives', 'Template name'), 'description' => __('Displays any archive, including posts by a single author, category, tag, taxonomy, custom post type, and date. This template will serve as a fallback when more specific templates (e.g. Category or Tag) cannot be found.')], 'author' => ['title' => _x('Author Archives', 'Template name'), 'description' => __('Displays a single author\'s post archive. This template will serve as a fallback when a more specific template (e.g. Author: Admin) cannot be found.')], 'category' => ['title' => _x('Category Archives', 'Template name'), 'description' => __('Displays a post category archive. This template will serve as a fallback when a more specific template (e.g. Category: Recipes) cannot be found.')], 'taxonomy' => ['title' => _x('Taxonomy', 'Template name'), 'description' => __('Displays a custom taxonomy archive. Like categories and tags, taxonomies have terms which you use to classify things. For example: a taxonomy named "Art" can have multiple terms, such as "Modern" and "18th Century." This template will serve as a fallback when a more specific template (e.g. Taxonomy: Art) cannot be found.')], 'date' => ['title' => _x('Date Archives', 'Template name'), 'description' => __('Displays a post archive when a specific date is visited (e.g., example.com/2023/).')], 'tag' => ['title' => _x('Tag Archives', 'Template name'), 'description' => __('Displays a post tag archive. This template will serve as a fallback when a more specific template (e.g. Tag: Pizza) cannot be found.')], 'attachment' => ['title' => __('Attachment Pages'), 'description' => __('Displays when a visitor views the dedicated page that exists for any media attachment.')], 'search' => ['title' => _x('Search Results', 'Template name'), 'description' => __('Displays when a visitor performs a search on your website.')], 'privacy-policy' => ['title' => __('Privacy Policy'), 'description' => __('Displays your site\'s Privacy Policy page.')], '404' => ['title' => _x('Page: 404', 'Template name'), 'description' => __('Displays when a visitor views a non-existent page, such as a dead link or a mistyped URL.')]];
    // Add a title and description to post format templates.
    $post_formats = get_post_format_strings();
    foreach ($post_formats as $post_format_slug => $post_format_name) {
        $default_template_types['taxonomy-post_format-post-format-' . $post_format_slug] = ['title' => sprintf(
...
function _get_block_templates_paths($base_directory)
{
    ...
    if (is_dir($base_directory)) {
        $nested_files = new Recursive_Iterator_Iterator(new Recursive_Directory_Iterator($base_directory));
        $nested_html_files = new Regex_Iterator($nested_files, '/^.+\.html$/i', Recursive_Regex_Iterator::GET_MATCH);
...
function _get_block_template_file($template_type, $slug)
{
    ...
    $themes = [get_stylesheet() => get_stylesheet_directory(), get_template() => get_template_directory()];
...
function _get_block_templates_files($template_type, $query = [])
{
    ...
    $themes = [$stylesheet => get_stylesheet_directory()];
...
function _build_block_template_result_from_file($template_file, $template_type)
{
    ...
    if ('wp_template_part' === $template->type) {
         ...
         */
        $content = get_comment_delimited_block_content('core/template-part', [], $template->content);
        $content = apply_block_hooks_to_content($content, $template, 'insert_hooked_blocks_and_set_ignored_hooked_blocks_metadata');
...
function _wp_build_title_and_description_for_single_post_type_block_template($post_type, $slug, WP_Block_Template $template)
{
    ...
    $default_args = ['post_type' => $post_type, 'post_status' => 'publish', 'posts_per_page' => 1, 'update_post_meta_cache' => false, 'update_post_term_cache' => false, 'ignore_sticky_posts' => true, 'no_found_rows' => true];
    $args = ['name' => $slug];
...
function _wp_build_title_and_description_for_taxonomy_block_template($taxonomy, $slug, WP_Block_Template $template)
{
    ...
    $default_args = ['taxonomy' => $taxonomy, 'hide_empty' => false, 'update_term_meta_cache' => false];
    $term_query = new WP_Term_Query();
    $args = ['number' => 1, 'slug' => $slug];
    $args = wp_parse_args($args, $default_args);
...
function _build_block_template_result_from_post($post)
{
    ...
    $meta = ['origin' => get_post_meta($parent_post->ID, 'origin', true), 'is_wp_suggestion' => get_post_meta($parent_post->ID, 'is_wp_suggestion', true)];
        ...
        if (preg_match('/(author|page|single|tag|category|taxonomy)-(.+)/', $template->slug, $matches)) {
            ...
            switch ($type) {
                case 'author':
                    ...
                    $users = get_users(['capability' => 'edit_posts', 'search' => $nice_name, 'search_columns' => ['user_nicename'], 'fields' => 'display_name']);
                    if (empty($users)) {
                        $template->title = sprintf(
                        ...
                        $users_with_same_name = get_users(['capability' => 'edit_posts', 'search' => $author_name, 'search_columns' => ['display_name'], 'fields' => 'display_name']);
    ...
    if ('wp_template' === $post->post_type) {
        ...
        if ($registered_template) {
            $template->plugin = $registered_template->plugin;
            $template->origin = 'theme' !== $template->origin && 'theme' !== $template->source ? 'plugin' : $template->origin;
            $template->title = empty($template->title) || $template->title === $template->slug ? $registered_template->title : $template->title;
            $template->description = empty($template->description) ? $registered_template->description : $template->description;
        }
    }
    if ('wp_template_part' === $template->type) {
         ...
         * we need to wrap its content a mock template part block and traverse it.
         */
        $content = get_comment_delimited_block_content('core/template-part', $attributes, $template->content);
        $content = apply_block_hooks_to_content($content, $template, 'insert_hooked_blocks_and_set_ignored_hooked_blocks_metadata');
        $template->content = remove_serialized_parent_block($content);
    } else {
        $template->content = apply_block_hooks_to_content($template->content, $template, 'insert_hooked_blocks_and_set_ignored_hooked_blocks_metadata');
...
function get_block_templates($query = [], $template_type = 'wp_template')
{
    ...
    $post_type = $query['post_type'] ?? '';
    $wp_query_args = ['post_status' => ['auto-draft', 'draft', 'publish'], 'post_type' => $template_type, 'posts_per_page' => -1, 'no_found_rows' => true, 'lazy_load_term_meta' => false, 'tax_query' => [['taxonomy' => 'wp_theme', 'field' => 'name', 'terms' => get_stylesheet()]]];
    if ('wp_template_part' === $template_type && isset($query['area'])) {
        $wp_query_args['tax_query'][] = ['taxonomy' => 'wp_template_part_area', 'field' => 'name', 'terms' => $query['area']];
    ...
    if (!isset($query['wp_id'])) {
        ...
        foreach ($template_files as $template_file) {
            ...
            if (!isset($query['post_type']) || isset($template_file['postTypes']) && in_array($query['post_type'], $template_file['postTypes'], true)) {
        ...
        if ('wp_template' === $template_type) {
            ...
            $matching_registered_templates = array_map(function ($template) {
                $template->content = apply_block_hooks_to_content($template->content, $template, 'insert_hooked_blocks_and_set_ignored_hooked_blocks_metadata');
                return $template;
...
function get_block_template($id, $template_type = 'wp_template')
{
    ...
    $wp_query_args = ['post_name__in' => [$slug], 'post_type' => $template_type, 'post_status' => ['auto-draft', 'draft', 'publish', 'trash'], 'posts_per_page' => 1, 'no_found_rows' => true, 'tax_query' => [['taxonomy' => 'wp_theme', 'field' => 'name', 'terms' => $theme]]];
    $template_query = new WP_Query($wp_query_args);
...
function get_block_file_template($id, $template_type = 'wp_template')
{
    ...
    if ($block_template) {
        $block_template->content = apply_block_hooks_to_content($block_template->content, $block_template, 'insert_hooked_blocks_and_set_ignored_hooked_blocks_metadata');
    }
    /**
...
function wp_generate_block_templates_export_file()
{
    ...
    $zip = new Zip_Archive();
    if (true !== $zip->open($filename, Zip_Archive::CREATE | Zip_Archive::OVERWRITE)) {
        return new WP_Error('unable_to_create_zip', __('Unable to open export file (archive) for writing.'));
    }
    $zip->add_empty_dir('templates');
    $zip->add_empty_dir('parts');
    // Get path of the theme.
    $theme_path = wp_normalize_path(get_stylesheet_directory());
    // Create recursive directory iterator.
    $theme_files = new Recursive_Iterator_Iterator(new Recursive_Directory_Iterator($theme_path), Recursive_Iterator_Iterator::LEAVES_ONLY);
    ...
    foreach ($templates as $template) {
        ...
        $zip->add_from_string('templates/' . $template->slug . '.html', $template->content);
    }
    // Load template parts into the zip file.
    $template_parts = get_block_templates([], 'wp_template_part');
    foreach ($template_parts as $template_part) {
        $zip->add_from_string('parts/' . $template_part->slug . '.html', $template_part->content);
    }
    ...
    $zip->add_from_string('theme.json', $theme_json_tabbed);
...
function inject_ignored_hooked_blocks_metadata_attributes($changes, $deprecated = null)
{
    ...
    if ('wp_template_part' === $post->post_type) {
        ...
        if (!empty($existing_ignored_hooked_blocks)) {
            $attributes['metadata'] = ['ignoredHookedBlocks' => json_decode($existing_ignored_hooked_blocks, true)];
        }
        $content = get_comment_delimited_block_content('core/template-part', $attributes, $changes->post_content);
