...
class WP_Comment_Query
{
    ...
    protected function get_comment_ids()
    {
        ...
        $raw_types = ['IN' => array_merge((array) $this->query_vars['type'], (array) $this->query_vars['type__in']), 'NOT IN' => (array) $this->query_vars['type__not_in']];
        // Exclude the 'note' comment type, unless 'all' types or the 'note' type explicitly are requested.
        if (!in_array('all', $raw_types['IN'], true) && !in_array('note', $raw_types['IN'], true) && !in_array('note', $raw_types['NOT IN'], true)) {
        ...
        if (isset($this->query_vars['search']) && strlen($this->query_vars['search'])) {
            $search_sql = $this->get_search_sql($this->query_vars['search'], ['comment_author', 'comment_author_email', 'comment_author_url', 'comment_author_IP', 'comment_content']);
            // Strip leading 'AND'.
    ...
    {
        $levels = [0 => wp_list_pluck($comments, 'comment_ID')];
    ...
    {
        global $wpdb;
        $allowed_keys = ['comment_agent', 'comment_approved', 'comment_author', 'comment_author_email', 'comment_author_IP', 'comment_author_url', 'comment_content', 'comment_date', 'comment_date_gmt', 'comment_ID', 'comment_karma', 'comment_parent', 'comment_post_ID', 'comment_type', 'user_id'];
