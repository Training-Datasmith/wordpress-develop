...
JS;
    ...
    {
        return in_array($priority, [ 'auto', 'low', 'high' ], true);
    }
     ...
     */
    private function filter_eligible_strategies($handle, $eligible_strategies = null, $checked = [], array &$stored_results = [])
    {
     ...
     */
    private function get_highest_fetchpriority_with_dependents(string $handle, array $checked = [], array &$stored_results = []): ?string
    {
        if (isset($stored_results[ $handle ])) {
            return $stored_results[ $handle ];
        }
