...
class WP_Date_Query
{
    ...
    {
        $cleaned_query = [];
        $defaults = ['column' => 'post_date', 'compare' => '=', 'relation' => 'AND'];
    ...
    {
        if (!empty($query['compare']) && in_array($query['compare'], ['=', '!=', '>', '>=', '<', '<=', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN'], true)) {
    ...
    public function validate_date_values($date_query = [])
    {
        ...
        $min_max_checks['dayofyear'] = ['min' => 1, 'max' => $max_days_of_year];
        // Days per week.
        $min_max_checks['dayofweek'] = ['min' => 1, 'max' => 7];
        // Days per week.
        $min_max_checks['dayofweek_iso'] = ['min' => 1, 'max' => 7];
        // Months per year.
        $min_max_checks['month'] = ['min' => 1, 'max' => 12];
        // Weeks per year.
        if (isset($_year)) {
            /*
        ...
        $min_max_checks['week'] = ['min' => 1, 'max' => $week_count];
        // Days per month.
        $min_max_checks['day'] = ['min' => 1, 'max' => 31];
        // Hours per day.
        $min_max_checks['hour'] = ['min' => 0, 'max' => 23];
        // Minutes per hour.
        $min_max_checks['minute'] = ['min' => 0, 'max' => 59];
        // Seconds per minute.
        $min_max_checks['second'] = ['min' => 0, 'max' => 59];
    ...
    public function validate_column($column)
    {
        ...
        $valid_columns = [
            ...
            // Part of $wpdb->posts.
            'comment_date',
            // Part of $wpdb->comments.
            'comment_date_gmt',
            // Part of $wpdb->comments.
            'user_registered',
        ];
        if (is_multisite()) {
            $valid_columns = array_merge($valid_columns, [
                'registered',
                // Part of $wpdb->blogs.
                'last_updated',
            ]);
        ...
        if (!str_contains($column, '.')) {
            ...
            $known_columns = [$wpdb->posts => ['post_date', 'post_date_gmt', 'post_modified', 'post_modified_gmt'], $wpdb->comments => ['comment_date', 'comment_date_gmt'], $wpdb->users => ['user_registered']];
            if (is_multisite()) {
                $known_columns[$wpdb->blogs] = ['registered', 'last_updated'];
            }
    ...
    protected function get_sql_for_clause($query, $parent_query)
    {
        ...
        $date_units = ['YEAR' => ['year'], 'MONTH' => ['month', 'monthnum'], '_wp_mysql_week' => ['week', 'w'], 'DAYOFYEAR' => ['dayofyear'], 'DAYOFMONTH' => ['day'], 'DAYOFWEEK' => ['dayofweek'], 'WEEKDAY' => ['dayofweek_iso']];
    ...
    public function build_mysql_datetime($datetime, $default_to_max = false)
    {
        if (!is_array($datetime)) {
            ...
            if (preg_match('/^(\d{4})$/', $datetime, $matches)) {
                // Y
                $datetime = ['year' => (int) $matches[1]];
            } elseif (preg_match('/^(\d{4})\-(\d{2})$/', $datetime, $matches)) {
                // Y-m
                $datetime = ['year' => (int) $matches[1], 'month' => (int) $matches[2]];
            } elseif (preg_match('/^(\d{4})\-(\d{2})\-(\d{2})$/', $datetime, $matches)) {
                // Y-m-d
                $datetime = ['year' => (int) $matches[1], 'month' => (int) $matches[2], 'day' => (int) $matches[3]];
            } elseif (preg_match('/^(\d{4})\-(\d{2})\-(\d{2}) (\d{2}):(\d{2})$/', $datetime, $matches)) {
                // Y-m-d H:i
                $datetime = ['year' => (int) $matches[1], 'month' => (int) $matches[2], 'day' => (int) $matches[3], 'hour' => (int) $matches[4], 'minute' => (int) $matches[5]];
            }
            ...
            if (!is_array($datetime)) {
                ...
                return $dt->set_timezone($wp_timezone)->format('Y-m-d H:i:s');
            }
        }
