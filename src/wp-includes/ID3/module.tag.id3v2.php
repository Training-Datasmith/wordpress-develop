...
class getid3_id3v2 extends getid3_handler
{
    ...
    public function Analyze(): bool
    {
        ...
        if ($sizeofframes > 0) {
            ...
            while (isset($framedata) && (strlen($framedata) > 0)) { // cycle through until no more frame data is left to parse
                if (strlen($framedata) <= static::ID3v2HeaderLength($id3v2_majorversion)) {
                ...
                if (($frame_size <= strlen($framedata)) && (static::IsValidID3v2FrameName($frame_name, $id3v2_majorversion))) {
    ...
    public function ParseID3v2Frame(array &$parsedFrame): bool
    {
        ...
        if (isset($parsedFrame['datalength']) && ($parsedFrame['datalength'] == 0)) {
            ...
            $parsedFrame['description'] = static::MakeUTF16emptyStringEmpty($parsedFrame['description']);
            ...
            $parsedFrame['data'] = static::RemoveStringTerminator($parsedFrame['data'], $frame_textencoding_terminator);
            ...
            $parsedFrame['data'] = static::RemoveStringTerminator($parsedFrame['data'], static::TextEncodingTerminatorLookup($frame_textencoding));
            ...
            $parsedFrame['description'] = static::RemoveStringTerminator($parsedFrame['description'], $frame_textencoding_terminator);
            $parsedFrame['description'] = static::MakeUTF16emptyStringEmpty($parsedFrame['description']);
            ...
            if (strlen($parsedFrame['data']) < 5) {
                ...
                $frame_text = static::RemoveStringTerminator($frame_text, $frame_textencoding_terminator);
            ...
            $parsedFrame['seller'] = static::RemoveStringTerminator($parsedFrame['seller'], static::TextEncodingTerminatorLookup($frame_textencoding));
            unset($parsedFrame['data']);
                    ...
                    switch ($subframe['name']) {
                        ...
                        case 'APIC':
                            if (preg_match('#^([^\\x00]+)*\\x00(.)([^\\x00]+)*\\x00(.+)$#s', $subframe['text'], $matches)) {
                                ...
                                $subframe['picture_type'] = static::APICPictureTypeLookup($subframe_apic_picturetype);
                                $subframe['description']  = trim(getid3_lib::iconv_fallback($subframe['encoding'], $info['encoding'], $subframe_apic_description));
                                if (strlen(static::TextEncodingTerminatorLookup($subframe['encoding'])) == 2) {
    ...
    public static function RemoveStringTerminator($string, $terminator)
    {
        ...
        if (substr($string, -strlen($terminator), strlen($terminator)) === $terminator) {
            return substr($string, 0, -strlen($terminator));
        }
        return $string;
    }
    ...
    public static function ID3v22iTunesBrokenFrameName($frame_name)
    {
        ...
        if (strlen($frame_name) != 4) {
            return $ID3v22_iTunes_BrokenFrames[substr($frame_name, 0, 3)] ?? false;
        }
        return $ID3v22_iTunes_BrokenFrames[substr($frame_name, 0, 3)] ?? false;
    }
