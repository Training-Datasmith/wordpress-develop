...
class getid3_matroska extends getid3_handler
{
    ...
    private array $unuseful_elements = [EBML_ID_CRC32, EBML_ID_VOID];
    ...
    public function Analyze(): bool
    {
        ...
        if (isset($info['matroska']['tags']) && is_array($info['matroska']['tags'])) {
            foreach ($info['matroska']['tags'] as $infoarray) {
        ...
        if (isset($info['matroska']['attachments']) && $this->getid3->option_save_attachments !== getID3::ATTACHMENTS_NONE) {
            foreach ($info['matroska']['attachments'] as $entry) {
        ...
        if (!empty($info['matroska']['tags'])) {
            ...
            foreach ($info['matroska']['tags'] as $value1) {
                if (!empty($value1['Targets']['TagTrackUID'][0]) && !empty($value1['SimpleTag'])) {
                    foreach ($value1['SimpleTag'] as $value2) {
    ...
    private function parseEBML(array &$info): void
    {
    ...
    private function unhandledElement(string $type, int $line, array $element): void
    {
    ...
    private function ExtractCommentsSimpleTag(array $SimpleTagArray): bool
    {
        if (!empty($SimpleTagArray['SimpleTag'])) {
            foreach ($SimpleTagArray['SimpleTag'] as $SimpleTagData) {
    ...
    private function HandleEMBLClusterBlock(array $element, int $block_type, array &$info): array
    {
