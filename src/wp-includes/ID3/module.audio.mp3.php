...
class getid3_mp3 extends getid3_handler
{
    ...
    public function GuessEncoderOptions()
    {
        ...
        if (empty($encoder_options) && !empty($info['audio']['bitrate']) && !empty($info['audio']['bitrate_mode'])) {
            //$encoder_options = strtoupper($info['audio']['bitrate_mode']).ceil($info['audio']['bitrate'] / 1000);
            return strtoupper($info['audio']['bitrate_mode']);
        }
     ...
     *
     */
    public function decodeMPEGaudioHeader($offset, array &$info, $recursivesearch = true, $ScanAsCBR = false, $FastMPEGheaderScan = false): bool
    {
    ...
    public function getOnlyMPEGaudioInfoBruteForce(): bool
    {
        ...
        self::MPEGaudioModeExtensionArray();
        self::MPEGaudioEmphasisArray();
            ...
            if (($header[$SynchSeekOffset] == "\xFF") && ($header[($SynchSeekOffset + 1)] > "\xE0")) { // possible synch detected
                ...
                if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
                    ...
                    if ($BitrateHistogram) {
                        ...
                        $distinct_bitrates = 0;
                        foreach ($info['mpeg']['audio']['bitrate_distribution'] as $bitrate_count) {
                            if ($bitrate_count > 0) {
                                $distinct_bitrates++;
                            }
                        }
     ...
     *
     */
    public static function MPEGaudioHeaderValid(array $rawarray, $echoerrors = false, $allowBitrate15 = false): bool
    {
    ...
    public static function LAMEpresetUsedLookup(array $LAMEtag): string
    {
