...
final class WP_Interactivity_API
{
     ...
     * @var array
     */
    private $derived_state_closures = [];
     ...
     */
    private $script_modules_that_can_load_on_client_navigation = [];
    ...
    public function add_hooks()
    {
        ...
        add_filter('wp_script_attributes', [ $this, 'add_load_on_client_navigation_attribute_to_script_modules' ], 10, 1);
    }
    ...
    public function add_load_on_client_navigation_attribute_to_script_modules($attributes)
    {
        if (
            ...
            $attributes['data-wp-router-options'] = wp_json_encode([ 'loadOnClientNavigation' => true ]);
        }
        return $attributes;
    }
    ...
    private function _process_directives(string $html)
    {
        ...
        while ($p->next_tag([ 'tag_closers' => 'visit' ])) {
            ...
            if ($p->is_tag_closer()) {
                list($opening_tag_name, $directives_prefixes) = ! empty($tag_stack) ? end($tag_stack) : [ null, null ];
        ...
        foreach ($path_segments as $index => $path_segment) {
            ...
            if ($current instanceof Closure) {
                ...
                try {
                     ...
                     * @since 6.9.0
                     */
                    $this->derived_state_closures[ $ns ] = $this->derived_state_closures[ $ns ] ?? [];
    ...
    private function data_wp_context_processor(WP_Interactivity_API_Directives_Processor $p, string $mode)
    {
        ...
        $context = end($this->context_stack) !== false ? end($this->context_stack) : [];
        foreach ($entries as $entry) {
            ...
            $context = array_replace_recursive(
                $context,
                [ $entry['namespace'] => is_array($entry['value']) ? $entry['value'] : [] ]
            );
        }
        ...
        if ('enter' === $mode && 'TEMPLATE' === $p->get_tag()) {
            ...
            foreach ($result as $item) {
                ...
                $this->context_stack[] = array_replace_recursive(
                    ...
                    [ $entry['namespace'] => [ $item_name => $item ] ]
                );
