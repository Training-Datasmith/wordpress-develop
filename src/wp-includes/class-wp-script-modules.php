...
class WP_Script_Modules
{
     ...
     * @var string[]
     */
    private $modules_with_missing_dependencies = [];
     ...
     * }
     */
    public function register(string $id, string $src, array $deps = [], $version = false, array $args = [])
    {
        if ('' === $id) {
     ...
     * }
     */
    public function enqueue(string $id, string $src = '', array $deps = [], $version = false, array $args = [])
    {
        if ('' === $id) {
            _doing_it_wrong(__METHOD__, __('Non-empty string required for id.'), '6.9.0');
            return;
    ...
    {
        $this->queue = array_values(array_diff($this->queue, [ $id ]));
    }
    ...
    public function print_head_enqueued_script_modules()
    {
        foreach ($this->get_sorted_dependencies($this->queue) as $id) {
            if (
                ...
                $dependencies = array_keys($this->get_dependencies([ $id ]));
    ...
    private function print_script_module(string $id)
    {
        ...
        $fetchpriority     = $this->get_highest_fetchpriority(array_merge([ $id ], $queued_dependents));
    ...
    {
        $dependency_ids = $this->get_sorted_dependencies($this->queue, [ 'static' ]);
     ...
     */
    private function get_dependencies(array $ids, array $import_types = [ 'static', 'dynamic' ]): array
    {
        $all_dependencies = [];
        $id_queue         = $ids;
     ...
     */
    private function get_sorted_dependencies(array $ids, array $import_types = [ 'static', 'dynamic' ]): array
    {
        $sorted = [];
