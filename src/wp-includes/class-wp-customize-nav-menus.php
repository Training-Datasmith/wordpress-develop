...
final class WP_Customize_Nav_Menus
{
    ...
    public function ajax_load_available_items()
    {
        ...
        if (isset($_POST['item_types']) && is_array($_POST['item_types'])) {
            ...
            // Back compat.
            $item_types[] = ['type' => wp_unslash($_POST['type']), 'object' => wp_unslash($_POST['object']), 'page' => empty($_POST['page']) ? 0 : absint($_POST['page'])];
        } else {
        ...
        if ('post_type' === $object_type) {
            ...
            if (0 === $page && 'page' === $object_name) {
                ...
                if (!empty($front_page)) {
                    ...
                    $items[] = ['id' => 'home', 'title' => _x('Home', 'nav menu home label'), 'type' => 'custom', 'type_label' => __('Custom Link'), 'object' => '', 'url' => home_url()];
                }
                // Insert Posts Page.
                ...
                $items[] = ['id' => $object_name . '-archive', 'title' => $title, 'original_title' => $title, 'type' => 'post_type_archive', 'type_label' => __('Post Type Archive'), 'object' => $object_name, 'url' => get_post_type_archive_link($object_name)];
            }
            ...
            $args = ['numberposts' => 10, 'offset' => 10 * $page, 'orderby' => 'date', 'order' => 'DESC', 'post_type' => $object_name];
            ...
            foreach ($posts as $post) {
                ...
                $items[] = ['id' => "post-{$post->ID}", 'title' => $title, 'original_title' => $title, 'type' => 'post_type', 'type_label' => $post_type_label, 'object' => $post->post_type, 'object_id' => (int) $post->ID, 'url' => get_permalink((int) $post->ID)];
            }
        } elseif ('taxonomy' === $object_type) {
            $terms = get_terms(['taxonomy' => $object_name, 'child_of' => 0, 'exclude' => '', 'hide_empty' => false, 'hierarchical' => 1, 'include' => '', 'number' => 10, 'offset' => 10 * $page, 'order' => 'DESC', 'orderby' => 'count', 'pad_counts' => false]);
            if (is_wp_error($terms)) {
                return $terms;
            }
            foreach ($terms as $term) {
                $title = html_entity_decode($term->name, ENT_QUOTES, get_bloginfo('charset'));
                $items[] = ['id' => "term-{$term->term_id}", 'title' => $title, 'original_title' => $title, 'type' => 'taxonomy', 'type_label' => get_taxonomy($term->taxonomy)->labels->singular_name, 'object' => $term->taxonomy, 'object_id' => (int) $term->term_id, 'url' => get_term_link((int) $term->term_id, $term->taxonomy)];
            }
        }
        /**
    ...
    public function search_available_items_query($args = [])
    {
        ...
        $query = ['post_type' => array_keys($post_type_objects), 'suppress_filters' => true, 'update_post_term_cache' => false, 'update_post_meta_cache' => false, 'post_status' => 'publish', 'posts_per_page' => 20];
        ...
        if (1 === $args['pagenum'] && $nav_menus_created_posts_setting && count($nav_menus_created_posts_setting->value()) > 0) {
            $stub_post_query = new WP_Query(array_merge($query, ['post_status' => 'auto-draft', 'post__in' => $nav_menus_created_posts_setting->value(), 'posts_per_page' => -1]));
        ...
        foreach ($posts as $post) {
            ...
            $items[] = ['id' => 'post-' . $post->ID, 'title' => html_entity_decode($post_title, ENT_QUOTES, get_bloginfo('charset')), 'type' => 'post_type', 'type_label' => $post_type_label, 'object' => $post->post_type, 'object_id' => (int) $post->ID, 'url' => get_permalink((int) $post->ID)];
        }
        // Query taxonomy terms.
        $taxonomies = get_taxonomies(['show_in_nav_menus' => true], 'names');
        $terms = get_terms(['taxonomies' => $taxonomies, 'name__like' => $args['s'], 'number' => 20, 'hide_empty' => false, 'offset' => 20 * ($args['pagenum'] - 1)]);
        // Check if any taxonomies were found.
        if (!empty($terms)) {
            foreach ($terms as $term) {
                $items[] = ['id' => 'term-' . $term->term_id, 'title' => html_entity_decode($term->name, ENT_QUOTES, get_bloginfo('charset')), 'type' => 'taxonomy', 'type_label' => get_taxonomy($term->taxonomy)->labels->singular_name, 'object' => $term->taxonomy, 'object_id' => (int) $term->term_id, 'url' => get_term_link((int) $term->term_id, $term->taxonomy)];
            }
        }
        ...
        if (isset($args['s'])) {
            ...
            if (empty($front_page)) {
                ...
                if ($matches) {
                    $items[] = ['id' => 'home', 'title' => $title, 'type' => 'custom', 'type_label' => __('Custom Link'), 'object' => '', 'url' => home_url()];
                }
            }
        }
        /**
    ...
    public function enqueue_scripts()
    {
        ...
        // Pass data to JS.
        $settings = ['allMenus' => wp_get_nav_menus(), 'itemTypes' => $this->available_item_types(), 'l10n' => [
        ...
        ], 'settingTransport' => 'postMessage', 'phpIntMax' => PHP_INT_MAX, 'defaultSettingValues' => ['nav_menu' => $temp_nav_menu_setting->default, 'nav_menu_item' => $temp_nav_menu_item_setting->default], 'locationSlugMappedToName' => get_registered_nav_menus()];
    ...
    public function filter_dynamic_setting_args($setting_args, $setting_id)
    {
        if (preg_match(WP_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id)) {
            $setting_args = ['type' => WP_Customize_Nav_Menu_Setting::TYPE, 'transport' => 'postMessage'];
        } elseif (preg_match(WP_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id)) {
            $setting_args = ['type' => WP_Customize_Nav_Menu_Item_Setting::TYPE, 'transport' => 'postMessage'];
        }
        return $setting_args;
    }
    /**
    ...
    public function customize_register()
    {
         ...
         */
        $this->manager->add_panel(new WP_Customize_Nav_Menus_Panel($this->manager, 'nav_menus', ['title' => __('Menus'), 'description' => $description, 'priority' => 100]));
        ...
        $this->manager->add_section('menu_locations', ['title' => 1 === $num_locations ? _x('View Location', 'menu locations') : _x('View All Locations', 'menu locations'), 'panel' => 'nav_menus', 'priority' => 30, 'description' => $description]);
        ...
        foreach ($locations as $location => $description) {
            ...
            if ($setting) {
                ...
                $this->manager->add_setting($setting_id, ['sanitize_callback' => [$this, 'intval_base10'], 'theme_supports' => 'menus', 'type' => 'theme_mod', 'transport' => 'postMessage', 'default' => 0]);
            }
            ...
            $this->manager->add_control(new WP_Customize_Nav_Menu_Location_Control($this->manager, $setting_id, ['label' => $description, 'location_id' => $location, 'section' => 'menu_locations', 'choices' => $choices]));
        }
        ...
        foreach ($menus as $menu) {
            ...
            $this->manager->add_section(new WP_Customize_Nav_Menu_Section($this->manager, $section_id, ['title' => html_entity_decode($menu->name, ENT_QUOTES, get_bloginfo('charset')), 'priority' => 10, 'panel' => 'nav_menus']));
            $nav_menu_setting_id = 'nav_menu[' . $menu_id . ']';
            $this->manager->add_setting(new WP_Customize_Nav_Menu_Setting($this->manager, $nav_menu_setting_id, ['transport' => 'postMessage']));
            ...
            foreach (array_values($menu_items) as $i => $item) {
                ...
                $this->manager->add_setting(new WP_Customize_Nav_Menu_Item_Setting($this->manager, $menu_item_setting_id, ['value' => $value, 'transport' => 'postMessage']));
                // Create a control for each menu item.
                $this->manager->add_control(new WP_Customize_Nav_Menu_Item_Control($this->manager, $menu_item_setting_id, ['label' => $item->title, 'section' => $section_id, 'priority' => 10 + $i]));
            }
            // Note: other controls inside of this section get added dynamically in JS via the MenuSection.ready() function.
        }
        // Add the add-new-menu section and controls.
        $this->manager->add_section('add_menu', ['type' => 'new_menu', 'title' => __('New Menu'), 'panel' => 'nav_menus', 'priority' => 20]);
        $this->manager->add_setting(new WP_Customize_Filter_Setting($this->manager, 'nav_menus_created_posts', [
    ...
    public function available_item_types()
    {
        ...
        if ($post_types) {
            foreach ($post_types as $slug => $post_type) {
                $item_types[] = ['title' => $post_type->labels->name, 'type_label' => $post_type->labels->singular_name, 'type' => 'post_type', 'object' => $post_type->name];
            }
        }
        ...
        if ($taxonomies) {
            foreach ($taxonomies as $slug => $taxonomy) {
                ...
                $item_types[] = ['title' => $taxonomy->labels->name, 'type_label' => $taxonomy->labels->singular_name, 'type' => 'taxonomy', 'object' => $taxonomy->name];
            }
        }
        /**
    ...
    public function ajax_insert_auto_draft_post()
    {
        ...
        $params = array_merge(['post_type' => '', 'post_title' => ''], $params);
        ...
        if (is_wp_error($r)) {
            ...
            $post = $r;
            $data = ['post_id' => $post->ID, 'url' => get_permalink($post->ID)];
    ...
    public function print_templates()
    {
		...
		<script type="text/html" id="tmpl-nav-menu-create-menu-section-title">
			<p class="add-new-menu-notice">
				<?php 
        _e('It does not look like your site has any menus yet. Want to build one? Click the button to start.');
        ?>
			</p>
			<p class="add-new-menu-notice">
				<?php 
        ...
        ?>
			...
			<div id="available-menu-items-search" class="accordion-section cannot-expand">
				<div class="accordion-section-title">
					<label for="menu-items-search"><?php 
    ...
    protected function print_post_type_container($available_item_type)
    {
        ...
        ?>" class="accordion-section">
			<h4 class="accordion-section-title">
				<button type="button" class="accordion-trigger" aria-expanded="false" aria-controls="<?php 
        echo esc_attr($id);
        ?>-content">
					<?php 
        ...
        ?></span>
			...
			<div class="accordion-section-content" id="<?php 
        echo esc_attr($id);
        ?>-content">
				<?php 
        if ('post_type' === $available_item_type['type']) {
            ...
            if (current_user_can($post_type_obj->cap->create_posts) && current_user_can($post_type_obj->cap->publish_posts)) {
                ...
                ?></button>
							...
							<span id="create-input-<?php 
                echo esc_attr($available_item_type['object']);
                ?>-error" class="create-item-error error-message" style="display: none;"><?php 
                _e('Please enter an item title');
                ?></span>
        ...
        echo esc_attr($available_item_type['object']);
        ?>" data-type_label="<?php 
        echo esc_attr($available_item_type['type_label'] ?? $available_item_type['type']);
        ?>"></ul>
			</div>
		</div>
		<?php 
    ...
    protected function print_custom_links_available_menu_item()
    {
        ...
        ?></label>
					...
					<span id="custom-url-error" class="error-message" style="display: none;"><?php 
        _e('Please provide a valid link.');
        ?></span>
				</p>
        ...
        ?></label>
					...
					<span id="custom-name-error" class="error-message" style="display: none;"><?php 
        _e('The link text cannot be empty.');
        ?></span>
				</p>
    ...
    public function save_nav_menus_created_posts($setting)
    {
        ...
        if (!empty($post_ids)) {
            foreach ($post_ids as $post_id) {
                ...
                $args = ['ID' => $post_id, 'post_status' => $target_status];
    ...
    public function filter_wp_nav_menu_args($args)
    {
         ...
         */
        $can_partial_refresh = !empty($args['echo']) && (empty($args['fallback_cb']) || is_string($args['fallback_cb'])) && (empty($args['walker']) || is_string($args['walker'])) && (!empty($args['theme_location']) || !empty($args['menu']) && (is_numeric($args['menu']) || is_object($args['menu']))) && (!empty($args['container']) || isset($args['items_wrap']) && str_starts_with($args['items_wrap'], '<'));
    ...
    public function export_preview_data()
    {
        ...
        $exports = ['navMenuInstanceArgs' => $this->preview_nav_menu_instance_args];
