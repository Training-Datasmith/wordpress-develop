...
class WP_Block
{
    ...
    private function process_block_bindings()
    {
        ...
        if (empty($supported_block_attributes) || empty($parsed_block['attrs']['metadata']['bindings']) || !is_array($parsed_block['attrs']['metadata']['bindings'])) {
        ...
        if (isset($bindings['__default']['source']) && 'core/pattern-overrides' === $bindings['__default']['source']) {
            ...
            foreach ($supported_block_attributes as $attribute_name) {
                ...
                $updated_bindings[$attribute_name] = $bindings[$attribute_name] ?? ['source' => 'core/pattern-overrides'];
            }
             ...
             */
            $computed_attributes['metadata'] = array_merge($parsed_block['attrs']['metadata'], ['bindings' => $bindings]);
        }
        ...
        switch ($block_type->attributes[$attribute_name]['source']) {
            ...
            case 'rich-text':
                ...
                foreach ($selectors as $selector) {
                    ...
                    if (strcasecmp($block_reader->get_tag(), $selector) === 0 || $block_reader->next_tag(['tag_name' => $selector])) {
    ...
    private static function get_block_bindings_processor(string $block_content)
    {
        $internal_processor_class = new class('', WP_HTML_Processor::CONSTRUCTOR_UNLOCK_CODE) extends WP_HTML_Processor
        {
            ...
            public function replace_rich_text($rich_text)
            {
                ...
                $this->lexical_updates[] = new WP_HTML_Text_Replacement($start, $end - $start, $rich_text);
                return true;
            }
        };
    ...
    public function render($options = [])
    {
        ...
        if ($interactivity_process_directives_enabled && null === $root_interactive_block && (isset($this->block_type->supports['interactivity']) && true === $this->block_type->supports['interactivity'] || !empty($this->block_type->supports['interactivity']['interactive']))) {
        ...
        if (!$just_did_wp_enqueue_scripts && ($has_new_styles || $has_new_scripts || $has_new_script_modules) && (trim($block_content) === '' && !(bool) apply_filters('enqueue_empty_block_content_assets', false, $this->name))) {
