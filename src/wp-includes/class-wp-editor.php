...
final class _WP_Editors
{
    ...
    public static function editor($content, $editor_id, $settings = [])
    {
        ...
        if (self::$this_tinymce) {
            ...
            if (self::$this_quicktags) {
                ...
                $buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce"' . $html_active . ' class="wp-switch-editor switch-tmce"' . ' data-wp-editor-id="' . $editor_id_attr . '">' . _x('Visual', 'Name for the Visual editor tab') . "</button>\n";
        ...
        if (self::$this_tinymce) {
            if (empty(self::$first_init)) {
                ...
                if ($set['teeny']) {
                     ...
                     * @param string $editor_id Unique editor identifier, e.g. 'content'.
                     */
                    $plugins = apply_filters('teeny_mce_plugins', ['colorpicker', 'lists', 'fullscreen', 'image', 'wordpress', 'wpeditimage', 'wplink'], $editor_id);
                } else {
                    /**
            ...
            if ($set['teeny']) {
                $mce_buttons = ['bold', 'italic', 'underline', 'blockquote', 'strikethrough', 'bullist', 'numlist', 'alignleft', 'aligncenter', 'alignright', 'undo', 'redo', 'link', 'fullscreen'];
                /**
            ...
            $mce_init = ['selector' => "#{$editor_id}", 'wpautop' => (bool) $set['wpautop'], 'indent' => !$set['wpautop'], 'toolbar1' => implode(',', $mce_buttons), 'toolbar2' => implode(',', $mce_buttons_2), 'toolbar3' => implode(',', $mce_buttons_3), 'toolbar4' => implode(',', $mce_buttons_4), 'tabfocus_elements' => $set['tabfocus_elements'], 'body_class' => $body_class];
    ...
    public static function enqueue_scripts($default_scripts = false)
    {
         ...
         */
        do_action('wp_enqueue_editor', ['tinymce' => $default_scripts || self::$has_tinymce, 'quicktags' => $default_scripts || self::$has_quicktags]);
    }
    /**
    ...
    public static function print_default_editor_scripts()
    {
        ...
        if ($user_can_richedit) {
             ...
             */
            $settings['plugins'] = implode(',', ['charmap', 'colorpicker', 'hr', 'lists', 'paste', 'tabfocus', 'textcolor', 'fullscreen', 'wordpress', 'wpautoresize', 'wpeditimage', 'wpemoji', 'wpgallery', 'wplink', 'wptextpattern']);
    ...
    private static function get_translation()
    {
        if (empty(self::$translation)) {
            self::$translation = [
                ...
                'Code|tab' => _x('Code', 'Name for the Code editor tab (formerly Text)'),
                // Editor switch tab label.
    ...
    public static function editor_js()
    {
        ...
        $ref = ['plugins' => implode(',', self::$plugins), 'theme' => 'modern', 'language' => self::$mce_locale];
        $suffix = SCRIPT_DEBUG ? '' : '.min';
        $baseurl = self::get_baseurl();
    ...
    public static function wp_link_query($args = [])
    {
        ...
        $query = ['post_type' => $pt_names, 'suppress_filters' => true, 'update_post_term_cache' => false, 'update_post_meta_cache' => false, 'post_status' => 'publish', 'posts_per_page' => 20];
        ...
        foreach ($posts as $post) {
            ...
            $results[] = ['ID' => $post->ID, 'title' => trim(esc_html(strip_tags(get_the_title($post)))), 'permalink' => get_permalink($post->ID), 'info' => $info];
        }
        /**
