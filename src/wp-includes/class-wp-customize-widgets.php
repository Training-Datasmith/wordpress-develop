...
final class WP_Customize_Widgets
{
     ...
     * @var array
     */
    protected $core_widget_id_bases = ['archives', 'calendar', 'categories', 'custom_html', 'links', 'media_audio', 'media_image', 'media_video', 'meta', 'nav_menu', 'pages', 'recent-comments', 'recent-posts', 'rss', 'search', 'tag_cloud', 'text'];
    /**
     * @since 3.9.0
     * @var array
     */
     ...
     * @var array
     */
    protected $setting_id_patterns = ['widget_instance' => '/^widget_(?P<id_base>.+?)(?:\[(?P<widget_number>\d+)\])?$/', 'sidebar_widgets' => '/^sidebars_widgets\[(?P<sidebar_id>.+?)\]$/'];
    /**
     * Initial loader.
     *
     * @since 3.9.0
    ...
    public function customize_register()
    {
        ...
        if (!$this->manager->is_theme_active()) {
            ...
            $setting_args = $this->get_setting_args($setting_id, ['type' => 'global_variable', 'dirty' => true]);
            $this->manager->add_setting($setting_id, $setting_args);
        }
        $this->manager->add_panel('widgets', ['type' => 'widgets', 'title' => __('Widgets'), 'description' => __('Widgets are independent sections of content that can be placed into widgetized areas provided by your theme (commonly called sidebars).'), 'priority' => 110, 'active_callback' => [$this, 'is_panel_active'], 'auto_expand_sole_section' => true, 'theme_supports' => 'widgets']);
        foreach ($sidebars_widgets as $sidebar_id => $sidebar_widget_ids) {
            ...
            if ($is_registered_sidebar || $is_inactive_widgets) {
                ...
                if ($is_active_sidebar) {
                    $section_args = ['title' => $wp_registered_sidebars[$sidebar_id]['name'], 'priority' => array_search($sidebar_id, array_keys($wp_registered_sidebars), true), 'panel' => 'widgets', 'sidebar_id' => $sidebar_id];
                    ...
                    if ($use_widgets_block_editor) {
                        $control = new WP_Sidebar_Block_Editor_Control($this->manager, $setting_id, ['section' => $section_id, 'sidebar_id' => $sidebar_id, 'label' => $section_args['title'], 'description' => $section_args['description']]);
                    } else {
                        $control = new WP_Widget_Area_Customize_Control($this->manager, $setting_id, ['section' => $section_id, 'sidebar_id' => $sidebar_id, 'priority' => count($sidebar_widget_ids)]);
                    }
            ...
            if (!$use_widgets_block_editor) {
                ...
                foreach ($sidebar_widget_ids as $i => $widget_id) {
                    ...
                    $control = new WP_Widget_Form_Customize_Control($this->manager, $setting_id, ['label' => $registered_widget['name'], 'section' => $section_id, 'sidebar_id' => $sidebar_id, 'widget_id' => $widget_id, 'widget_id_base' => $id_base, 'priority' => $i, 'width' => $wp_registered_widget_controls[$widget_id]['width'], 'height' => $wp_registered_widget_controls[$widget_id]['height'], 'is_wide' => $this->is_wide_widget($widget_id)]);
                    $this->manager->add_control($control);
                }
            }
        }
    ...
    {
        $parsed = ['number' => null, 'id_base' => null];
    ...
    public function enqueue_scripts()
    {
        ...
        $move_widget_area_tpl = str_replace(['{description}', '{btn}'], [__('Select an area to move this widget into:'), _x('Move', 'Move widget')], '<div class="move-widget-area">
        ...
        for ($non_rendered_count = 2; $non_rendered_count < $registered_sidebar_count; $non_rendered_count++) {
            $some_non_rendered_areas_messages[$non_rendered_count] = html_entity_decode(sprintf(
                ...
                _n('Your theme has %s other widget area, but this particular page does not display it.', 'Your theme has %s other widget areas, but this particular page does not display them.', $non_rendered_count),
        ...
        if (1 === $registered_sidebar_count) {
            ...
            $no_areas_shown_message = html_entity_decode(sprintf(
                ...
                _n('Your theme has %s widget area, but this particular page does not display it.', 'Your theme has %s widget areas, but this particular page does not display them.', $registered_sidebar_count),
        ...
        $settings = [
            ...
            'tpl' => ['widgetReorderNav' => $widget_reorder_nav_tpl, 'moveWidgetArea' => $move_widget_area_tpl],
        ...
        $wp_scripts->add_data('customize-widgets', 'data', sprintf('var _wpCustomizeWidgetsSettings = %s;', wp_json_encode($settings, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES)));
        /*
        ...
        if (wp_use_widgets_block_editor()) {
            $block_editor_context = new WP_Block_Editor_Context(['name' => 'core/customize-widgets']);
            $editor_settings = get_block_editor_settings(get_legacy_widget_block_editor_settings(), $block_editor_context);
            wp_add_inline_script('wp-customize-widgets', sprintf('wp.domReady( function() {
            ...
            wp_add_inline_script('wp-blocks', 'wp.blocks.unstable__bootstrapServerSideBlockDefinitions(' . wp_json_encode(get_block_editor_server_block_settings(), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES) . ');');
            // Preload server-registered block bindings sources.
            $registered_sources = get_all_registered_block_bindings_sources();
            if (!empty($registered_sources)) {
                $filtered_sources = [];
                foreach ($registered_sources as $source) {
                    $filtered_sources[] = ['name' => $source->name, 'label' => $source->label, 'usesContext' => $source->uses_context];
                }
            ...
            wp_add_inline_script('wp-blocks', sprintf('wp.blocks.setCategories( %s );', wp_json_encode(get_block_categories($block_editor_context), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES)), 'after');
    ...
    {
        $args = ['type' => 'option', 'capability' => 'edit_theme_options', 'default' => []];
    ...
    public function export_preview_data()
    {
        ...
        $l10n = ['widgetTooltip' => __('Shift-click to edit this widget.')];
        if ($switched_locale) {
    ...
    public function sanitize_widget_js_instance($value, $id_base = null)
    {
        ...
        if (empty($value['is_widget_customizer_js_value'])) {
            ...
            $js_value = ['encoded_serialized_instance' => base64_encode($serialized), 'title' => empty($value['title']) ? '' : $value['title'], 'is_widget_customizer_js_value' => true, 'instance_hash_key' => $this->get_instance_hash_key($serialized)];
    ...
    public function customize_dynamic_partial_args($partial_args, $partial_id)
    {
        ...
        if (preg_match('/^widget\[(?P<widget_id>.+)\]$/', $partial_id, $matches)) {
            ...
            $partial_args = array_merge($partial_args, ['type' => 'widget', 'render_callback' => [$this, 'render_widget_partial'], 'container_inclusive' => true, 'settings' => [$this->get_setting_id($matches['widget_id'])], 'capability' => 'edit_theme_options']);
        }
        return $partial_args;
    }
    /**
    ...
    {
        $sidebar_args = array_merge(['before_widget' => '', 'after_widget' => ''], $params[0]);
        ...
        $context = ['sidebar_id' => $sidebar_args['id']];
    ...
    public function filter_wp_kses_allowed_data_attributes($allowed_html)
    {
        foreach (array_keys($this->before_widget_tags_seen) as $tag_name) {
            ...
            $allowed_html[$tag_name] = array_merge($allowed_html[$tag_name], array_fill_keys(['data-customize-partial-id', 'data-customize-partial-type', 'data-customize-partial-placement-context', 'data-customize-partial-widget-id', 'data-customize-partial-options'], true));
        }
        return $allowed_html;
    }
    /**
