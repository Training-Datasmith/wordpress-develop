...
class WP_Http_Streams
{
    ...
    public function request($url, $args = [])
    {
        ...
        if ($parsed_args['stream']) {
            ...
            while (!feof($handle) && $keep_reading) {
                ...
                if (!$body_started) {
                    $response .= $block;
                    if (strpos($response, "\r\n\r\n")) {
                        $processed_response = WP_Http::process_response($response);
                        $body_started = true;
        ...
        fclose($handle);
        $processed_headers = WP_Http::process_headers($processed_response['headers'], $url);
        $response = [
        ...
        if (!empty($processed_response['body']) && isset($processed_headers['headers']['transfer-encoding']) && 'chunked' === $processed_headers['headers']['transfer-encoding']) {
            $processed_response['body'] = WP_Http::chunk_transfer_decode($processed_response['body']);
        }
