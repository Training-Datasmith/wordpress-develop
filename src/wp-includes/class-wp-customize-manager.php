...
final class WP_Customize_Manager
{
    ...
    {
        $args = array_merge(array_fill_keys(['changeset_uuid', 'theme', 'messenger_channel', 'settings_previewed', 'autosaved', 'branching'], null), $args);
        ...
        add_action('customize_register', [$this, 'register_dynamic_settings'], 11);
    ...
    protected function wp_die($ajax_message, $message = null)
    {
        ...
        if ($this->messenger_channel) {
            ...
            $settings = ['messengerArgs' => ['channel' => $this->messenger_channel, 'url' => wp_customize_url()], 'error' => $ajax_message];
            ...
            echo wp_json_encode($settings, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES);
            ?> );
			</script>
			<?php 
    ...
    public function establish_loaded_changeset()
    {
        ...
        if (empty($this->_changeset_uuid)) {
            ...
            if (!$this->branching() && $this->is_theme_active()) {
                $unpublished_changeset_posts = $this->get_changeset_posts(['post_status' => array_diff(get_post_stati(), ['auto-draft', 'publish', 'trash', 'inherit', 'private']), 'exclude_restore_dismissed' => false, 'author' => 'any', 'posts_per_page' => 1, 'order' => 'DESC', 'orderby' => 'date']);
    ...
    public function find_changeset_post_id($uuid)
    {
        ...
        $changeset_post_query = new WP_Query(['post_type' => 'customize_changeset', 'post_status' => get_post_stati(), 'name' => $uuid, 'posts_per_page' => 1, 'no_found_rows' => true, 'cache_results' => true, 'update_post_meta_cache' => false, 'update_post_term_cache' => false, 'lazy_load_term_meta' => false]);
    ...
    {
        $default_args = ['exclude_restore_dismissed' => true, 'posts_per_page' => -1, 'post_type' => 'customize_changeset', 'post_status' => 'auto-draft', 'order' => 'DESC', 'orderby' => 'date', 'no_found_rows' => true, 'cache_results' => true, 'update_post_meta_cache' => false, 'update_post_term_cache' => false, 'lazy_load_term_meta' => false];
    ...
    {
        $changeset_autodraft_posts = $this->get_changeset_posts(['post_status' => 'auto-draft', 'exclude_restore_dismissed' => true, 'posts_per_page' => -1]);
        $dismissed = 0;
    ...
    public function import_theme_starter_content($starter_content = [])
    {
            ...
            foreach ($widgets as $widget) {
                ...
                if (!isset($max_widget_numbers[$id_base])) {
                    ...
                    if ($settings instanceof ArrayObject || $settings instanceof ArrayIterator) {
                        $settings = $settings->get_array_copy();
                    }
        ...
        if (!empty($starter_content_auto_draft_post_ids)) {
            $existing_posts_query = new WP_Query(['post__in' => $starter_content_auto_draft_post_ids, 'post_status' => 'auto-draft', 'post_type' => $post_types, 'posts_per_page' => -1]);
        ...
        if (!empty($all_post_slugs)) {
            $existing_posts_query = new WP_Query(['post_name__in' => $all_post_slugs, 'post_status' => array_diff(get_post_stati(), ['auto-draft']), 'post_type' => 'any', 'posts_per_page' => -1]);
            foreach ($existing_posts_query->posts as $existing_post) {
        ...
        if (!empty($attachments)) {
            ...
            foreach ($attachments as $symbol => $attachment) {
                ...
                if (!$attachment_id) {
                    ...
                    $attachment_post_data = array_merge(wp_array_slice_assoc($attachment, ['post_title', 'post_content', 'post_excerpt']), ['post_status' => 'auto-draft']);
        ...
        foreach ($theme_mods as $name => $value) {
            ...
            if ('header_image' === $name) {
                ...
                $value = ['attachment_id' => $value, 'url' => wp_get_attachment_url($value), 'height' => $metadata['height'], 'width' => $metadata['width']];
    ...
    public function _save_starter_content_changeset()
    {
        ...
        $this->save_changeset_post(['data' => array_fill_keys($this->pending_starter_content_settings_ids, ['starter_content' => true]), 'starter_content' => true]);
    ...
    {
        $args = array_merge(['exclude_changeset' => false, 'exclude_post_data' => !current_user_can('customize')], $args);
        $values = [];
    ...
    public function customize_preview_settings()
    {
        ...
        $l10n = ['shiftClickToEdit' => __('Shift-click to edit this element.'), 'linkUnpreviewable' => __('This link is not live-previewable.'), 'formUnpreviewable' => __('This form is not live-previewable.')];
        if ($switched_locale) {
            restore_previous_locale();
        }
        $settings = ['changeset' => ['uuid' => $this->changeset_uuid(), 'autosaved' => $this->autosaved()], 'timeouts' => ['selectiveRefresh' => 250, 'keepAliveSend' => 1000], 'theme' => ['stylesheet' => $this->get_stylesheet(), 'active' => $this->is_theme_active(), 'isBlockTheme' => wp_is_block_theme()], 'url' => ['self' => $self_url, 'allowed' => array_map('sanitize_url', $this->get_allowed_urls()), 'allowedHosts' => array_unique($allowed_hosts), 'isCrossDomain' => $this->is_cross_domain()], 'channel' => $this->messenger_channel, 'activePanels' => [], 'activeSections' => [], 'activeControls' => [], 'settingValidities' => $exported_setting_validities, 'nonce' => current_user_can('customize') ? $this->get_nonces() : [], 'l10n' => $l10n, '_dirty' => array_keys($post_values)];
        ...
        foreach ($this->settings as $id => $setting) {
            if ($setting->check_capabilities()) {
                printf("v[%s] = %s;\n", wp_json_encode($id, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES), wp_json_encode($setting->js_value(), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES));
            }
        }
        ?>
			})( _wpCustomizeSettings.values );
    ...
    {
        $options = wp_parse_args($options, ['validate_capability' => false, 'validate_existence' => false]);
        $validities = [];
    ...
    public function prepare_setting_validity_for_js($validity)
    {
        if (is_wp_error($validity)) {
            ...
            foreach ($validity->errors as $error_code => $error_messages) {
                $notification[$error_code] = ['message' => implode(' ', $error_messages), 'data' => $validity->get_error_data($error_code)];
            }
            return $notification;
        } else {
            return true;
    ...
    public function save()
    {
        ...
        } else if (!current_user_can(get_post_type_object('customize_changeset')->cap->edit_post, $changeset_post_id)) {
        ...
        $autosaved = false;
        $r = $this->save_changeset_post(['status' => $changeset_status, 'title' => $changeset_title, 'date_gmt' => $changeset_date_gmt, 'data' => $input_changeset_data, 'autosave' => $autosave]);
        ...
        if ($lock_user_id && !is_wp_error($r)) {
            $r = new WP_Error('changeset_locked', __('Changeset is being edited by other user.'), ['lock_user' => $this->get_lock_user_data($lock_user_id)]);
        }
        if (is_wp_error($r)) {
    ...
    {
        $args = array_merge(['status' => null, 'title' => null, 'data' => [], 'date_gmt' => null, 'user_id' => get_current_user_id(), 'starter_content' => false, 'autosave' => false], $args);
        $changeset_post_id = $this->changeset_post_id();
        $existing_changeset_data = [];
        if ($changeset_post_id) {
            $existing_status = get_post_status($changeset_post_id);
            if ('publish' === $existing_status || 'trash' === $existing_status) {
                return new WP_Error('changeset_already_published', __('The previous set of changes has already been published. Please try saving your current set of changes again.'), ['next_changeset_uuid' => wp_generate_uuid4()]);
            }
        ...
        foreach ($args['data'] as $setting_id => $setting_params) {
            ...
            if (null === $setting_params) {
                ...
                $data[$changeset_setting_id] = array_merge($merged_setting_params, ['type' => $setting->type, 'user_id' => $args['user_id'], 'date_modified_gmt' => current_time('mysql', true)]);
        ...
        $filter_context = ['uuid' => $this->changeset_uuid(), 'title' => $args['title'], 'status' => $args['status'], 'date_gmt' => $args['date_gmt'], 'post_id' => $changeset_post_id, 'previous_data' => is_wp_error($original_changeset_data) ? [] : $original_changeset_data, 'manager' => $this];
        /**
    ...
    public function handle_changeset_trash_request()
    {
        ...
        if (!check_ajax_referer('trash_customize_changeset', 'nonce', false)) {
            wp_send_json_error(['code' => 'invalid_nonce', 'message' => __('There was an authentication problem. Please reload and try again.')]);
        }
        $changeset_post_id = $this->changeset_post_id();
        if (!$changeset_post_id) {
            wp_send_json_error(['message' => __('No changes saved yet, so there is nothing to trash.'), 'code' => 'non_existent_changeset']);
            return;
        }
        if (!current_user_can(get_post_type_object('customize_changeset')->cap->delete_post, $changeset_post_id)) {
            wp_send_json_error(['code' => 'changeset_trash_unauthorized', 'message' => __('Unable to trash changes.')]);
        }
        $lock_user = (int) wp_check_post_lock($changeset_post_id);
        if ($lock_user && get_current_user_id() !== $lock_user) {
            wp_send_json_error(['code' => 'changeset_locked', 'message' => __('Changeset is being edited by other user.'), 'lockUser' => $this->get_lock_user_data($lock_user)]);
        }
        if ('trash' === get_post_status($changeset_post_id)) {
            wp_send_json_error(['message' => __('Changes have already been trashed.'), 'code' => 'changeset_already_trashed']);
            return;
        }
        $r = $this->trash_changeset_post($changeset_post_id);
        if (!$r instanceof WP_Post) {
            wp_send_json_error(['code' => 'changeset_trash_failure', 'message' => __('Unable to trash changes.')]);
        }
        wp_send_json_success(['message' => __('Changes trashed successfully.')]);
    }
    /**
    ...
    protected function get_lock_user_data($user_id)
    {
        ...
        $user_details = ['id' => $lock_user->ID, 'name' => $lock_user->display_name];
    ...
    public function handle_override_changeset_lock_request()
    {
        ...
        if (!check_ajax_referer('customize_override_changeset_lock', 'nonce', false)) {
            wp_send_json_error(['code' => 'invalid_nonce', 'message' => __('Security check failed.')]);
        }
        $changeset_post_id = $this->changeset_post_id();
        if (empty($changeset_post_id)) {
            wp_send_json_error(['code' => 'no_changeset_found_to_take_over', 'message' => __('No changeset found to take over')]);
        }
        if (!current_user_can(get_post_type_object('customize_changeset')->cap->edit_post, $changeset_post_id)) {
            wp_send_json_error(['code' => 'cannot_remove_changeset_lock', 'message' => __('Sorry, you are not allowed to take over.')]);
        }
    ...
    public function _publish_changeset_values($changeset_post_id)
    {
        ...
        $changeset_setting_values = $this->unsanitized_post_values(['exclude_post_data' => true, 'exclude_changeset' => false]);
        ...
        foreach ($revisions as $revision) {
            if (str_contains($revision->post_name, "{$changeset_post_id}-autosave")) {
                $wpdb->update($wpdb->posts, ['post_status' => 'auto-draft', 'post_type' => 'customize_changeset', 'post_name' => wp_generate_uuid4(), 'post_parent' => 0], ['ID' => $revision->ID]);
    ...
    public function render_control_templates()
    {
        ...
        foreach ($this->registered_control_types as $control_type) {
            $control = new $control_type($this, 'temp', ['settings' => []]);
    ...
    {
        $controls = [];
        $this->controls = wp_list_sort($this->controls, ['priority' => 'ASC', 'instance_number' => 'ASC'], 'ASC', true);
        foreach ($this->controls as $id => $control) {
        ...
        // Prepare sections.
        $this->sections = wp_list_sort($this->sections, ['priority' => 'ASC', 'instance_number' => 'ASC'], 'ASC', true);
        $sections = [];
        foreach ($this->sections as $section) {
            if (!$section->check_capabilities()) {
                continue;
            }
            $section->controls = wp_list_sort($section->controls, ['priority' => 'ASC', 'instance_number' => 'ASC']);
            if (!$section->panel) {
                // Top-level section.
                $sections[$section->id] = $section;
            } else if (isset($this->panels[$section->panel])) {
                $this->panels[$section->panel]->sections[$section->id] = $section;
            }
        }
        $this->sections = $sections;
        // Prepare panels.
        $this->panels = wp_list_sort($this->panels, ['priority' => 'ASC', 'instance_number' => 'ASC'], 'ASC', true);
        $panels = [];
        foreach ($this->panels as $panel) {
            if (!$panel->check_capabilities()) {
                continue;
            }
            $panel->sections = wp_list_sort($panel->sections, ['priority' => 'ASC', 'instance_number' => 'ASC'], 'ASC', true);
            $panels[$panel->id] = $panel;
        }
        $this->panels = $panels;
        // Sort panels and top-level sections together.
        $this->containers = array_merge($this->panels, $this->sections);
        $this->containers = wp_list_sort($this->containers, ['priority' => 'ASC', 'instance_number' => 'ASC'], 'ASC', true);
    }
    /**
    ...
    public function enqueue_control_scripts()
    {
        ...
        if (!is_multisite() && (current_user_can('install_themes') || current_user_can('update_themes') || current_user_can('delete_themes'))) {
            ...
            wp_localize_script('updates', '_wpUpdatesItemCounts', ['totals' => wp_get_update_data()]);
        }
    }
    /**
    ...
    public function customize_pane_settings()
    {
        ...
        if (!$this->saved_starter_content_changeset && !$this->autosaved()) {
            if ($changeset_post_id) {
                ...
                $autosave_autodraft_posts = $this->get_changeset_posts(['posts_per_page' => 1, 'post_status' => 'auto-draft', 'exclude_restore_dismissed' => true]);
        ...
        if ($current_user_can_publish) {
            $status_choices[] = ['status' => 'publish', 'label' => __('Publish')];
        }
        $status_choices[] = ['status' => 'draft', 'label' => __('Save Draft')];
        if ($current_user_can_publish) {
            $status_choices[] = ['status' => 'future', 'label' => _x('Schedule', 'customizer changeset action/button label')];
        }
        ...
        $settings = [
            ...
            'timeouts' => ['windowRefresh' => 250, 'changesetAutoSave' => AUTOSAVE_INTERVAL * 1000, 'keepAliveCheck' => 2500, 'reflowPaneContents' => 100, 'previewFrameSensitivity' => 2000],
            'theme' => ['stylesheet' => $this->get_stylesheet(), 'active' => $this->is_theme_active(), '_canInstall' => current_user_can('install_themes')],
            'url' => ['preview' => sanitize_url($this->get_preview_url()), 'return' => sanitize_url($this->get_return_url()), 'parent' => sanitize_url(admin_url()), 'activated' => sanitize_url(home_url('/')), 'ajax' => sanitize_url(admin_url('admin-ajax.php', 'relative')), 'allowed' => array_map('sanitize_url', $this->get_allowed_urls()), 'isCrossDomain' => $this->is_cross_domain(), 'home' => sanitize_url(home_url('/')), 'login' => sanitize_url($login_url)],
            'browser' => ['mobile' => wp_is_mobile(), 'ios' => $this->is_ios()],
            'panels' => [],
            'sections' => [],
        ...
        foreach ($this->settings() as $setting) {
            if ($setting->check_capabilities()) {
                printf("s[%s] = %s;\n", wp_json_encode($setting->id, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES), wp_json_encode($setting->json(), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES));
            }
        }
        ...
        foreach ($this->controls() as $control) {
            if ($control->check_capabilities()) {
                printf("c[%s] = %s;\n", wp_json_encode($control->id, JSON_HEX_TAG | JSON_UNESCAPED_SLASHES), wp_json_encode($control->json(), JSON_HEX_TAG | JSON_UNESCAPED_SLASHES));
            }
        }
    ...
    {
        $devices = ['desktop' => ['label' => __('Enter desktop preview mode'), 'default' => true], 'tablet' => ['label' => __('Enter tablet preview mode')], 'mobile' => ['label' => __('Enter mobile preview mode')]];
        /**
    ...
    public function register_controls()
    {
        ...
        $this->add_panel(new WP_Customize_Themes_Panel($this, 'themes', ['title' => $this->theme()->display('Name'), 'description' => '<p>' . __('Looking for a theme? You can search or browse the WordPress.org theme directory, install and preview themes, then activate them right here.') . '</p>' . '<p>' . __('While previewing a new theme, you can continue to tailor things like widgets and menus, and explore theme-specific options.') . '</p>', 'capability' => 'switch_themes', 'priority' => 0]));
        $this->add_section(new WP_Customize_Themes_Section($this, 'installed_themes', ['title' => __('Installed themes'), 'action' => 'installed', 'capability' => 'switch_themes', 'panel' => 'themes', 'priority' => 0]));
        if (!is_multisite()) {
            $this->add_section(new WP_Customize_Themes_Section($this, 'wporg_themes', ['title' => __('WordPress.org themes'), 'action' => 'wporg', 'filter_type' => 'remote', 'capability' => 'install_themes', 'panel' => 'themes', 'priority' => 5]));
        }
        // Themes Setting (unused - the theme is considerably more fundamental to the Customizer experience).
        $this->add_setting(new WP_Customize_Filter_Setting($this, 'active_theme', ['capability' => 'switch_themes']));
        /* Site Identity */
        $this->add_section('title_tagline', ['title' => __('Site Identity'), 'priority' => 20]);
        $this->add_setting('blogname', ['default' => get_option('blogname'), 'type' => 'option', 'capability' => 'manage_options']);
        $this->add_control('blogname', ['label' => __('Site Title'), 'section' => 'title_tagline']);
        $this->add_setting('blogdescription', ['default' => get_option('blogdescription'), 'type' => 'option', 'capability' => 'manage_options']);
        $this->add_control('blogdescription', ['label' => __('Tagline'), 'section' => 'title_tagline']);
        // Add a setting to hide header text if the theme doesn't support custom headers.
        if (!current_theme_supports('custom-header', 'header-text')) {
            $this->add_setting('header_text', ['theme_supports' => ['custom-logo', 'header-text'], 'default' => 1, 'sanitize_callback' => 'absint']);
            $this->add_control('header_text', ['label' => __('Display Site Title and Tagline'), 'section' => 'title_tagline', 'settings' => 'header_text', 'type' => 'checkbox']);
        }
        $this->add_setting('site_icon', ['type' => 'option', 'capability' => 'manage_options', 'transport' => 'postMessage']);
        $this->add_control(new WP_Customize_Site_Icon_Control($this, 'site_icon', ['label' => __('Site Icon'), 'description' => sprintf(
            ...
            512,
            512
        ), 'section' => 'title_tagline', 'priority' => 60, 'height' => 512, 'width' => 512]));
        $this->add_setting('custom_logo', ['theme_supports' => ['custom-logo'], 'transport' => 'postMessage']);
        $custom_logo_args = get_theme_support('custom-logo');
        $this->add_control(new WP_Customize_Cropped_Image_Control($this, 'custom_logo', ['label' => __('Logo'), 'section' => 'title_tagline', 'priority' => 8, 'height' => $custom_logo_args[0]['height'] ?? null, 'width' => $custom_logo_args[0]['width'] ?? null, 'flex_height' => $custom_logo_args[0]['flex-height'] ?? null, 'flex_width' => $custom_logo_args[0]['flex-width'] ?? null, 'button_labels' => ['select' => __('Select logo'), 'change' => __('Change logo'), 'remove' => __('Remove'), 'default' => __('Default'), 'placeholder' => __('No logo selected'), 'frame_title' => __('Select logo'), 'frame_button' => __('Choose logo')]]));
        $this->selective_refresh->add_partial('custom_logo', ['settings' => ['custom_logo'], 'selector' => '.custom-logo-link', 'render_callback' => [$this, '_render_custom_logo_partial'], 'container_inclusive' => true]);
        /* Colors */
        $this->add_section('colors', ['title' => __('Colors'), 'priority' => 40]);
        $this->add_setting('header_textcolor', ['theme_supports' => ['custom-header', 'header-text'], 'default' => get_theme_support('custom-header', 'default-text-color'), 'sanitize_callback' => [$this, '_sanitize_header_textcolor'], 'sanitize_js_callback' => 'maybe_hash_hex_color']);
        // Input type: checkbox, with custom value.
        $this->add_control('display_header_text', ['settings' => 'header_textcolor', 'label' => __('Display Site Title and Tagline'), 'section' => 'title_tagline', 'type' => 'checkbox', 'priority' => 40]);
        $this->add_control(new WP_Customize_Color_Control($this, 'header_textcolor', ['label' => __('Header Text Color'), 'section' => 'colors']));
        // Input type: color, with sanitize_callback.
        $this->add_setting('background_color', ['default' => get_theme_support('custom-background', 'default-color'), 'theme_supports' => 'custom-background', 'sanitize_callback' => 'sanitize_hex_color_no_hash', 'sanitize_js_callback' => 'maybe_hash_hex_color']);
        $this->add_control(new WP_Customize_Color_Control($this, 'background_color', ['label' => __('Background Color'), 'section' => 'colors']));
        /* Custom Header */
        ...
        $this->add_setting('header_video', ['theme_supports' => ['custom-header', 'video'], 'transport' => 'postMessage', 'sanitize_callback' => 'absint', 'validate_callback' => [$this, '_validate_header_video']]);
        $this->add_setting('external_header_video', ['theme_supports' => ['custom-header', 'video'], 'transport' => 'postMessage', 'sanitize_callback' => [$this, '_sanitize_external_header_video'], 'validate_callback' => [$this, '_validate_external_header_video']]);
        $this->add_setting(new WP_Customize_Filter_Setting($this, 'header_image', ['default' => sprintf(get_theme_support('custom-header', 'default-image'), get_template_directory_uri(), get_stylesheet_directory_uri()), 'theme_supports' => 'custom-header']));
        $this->add_setting(new WP_Customize_Header_Image_Setting($this, 'header_image_data', ['theme_supports' => 'custom-header']));
        /*
        ...
        $this->add_control(new WP_Customize_Media_Control($this, 'header_video', ['theme_supports' => ['custom-header', 'video'], 'label' => __('Header Video'), 'description' => $control_description, 'section' => 'header_image', 'mime_type' => 'video', 'active_callback' => 'is_header_video_active']));
        $this->add_control('external_header_video', ['theme_supports' => ['custom-header', 'video'], 'type' => 'url', 'description' => __('Or, enter a YouTube URL:'), 'section' => 'header_image', 'active_callback' => 'is_header_video_active']);
        ...
        /* Custom Background */
        $this->add_section('background_image', ['title' => __('Background Image'), 'theme_supports' => 'custom-background', 'priority' => 80]);
        $this->add_setting('background_image', ['default' => get_theme_support('custom-background', 'default-image'), 'theme_supports' => 'custom-background', 'sanitize_callback' => [$this, '_sanitize_background_setting']]);
        $this->add_setting(new WP_Customize_Background_Image_Setting($this, 'background_image_thumb', ['theme_supports' => 'custom-background', 'sanitize_callback' => [$this, '_sanitize_background_setting']]));
        $this->add_control(new WP_Customize_Background_Image_Control($this));
        $this->add_setting('background_preset', ['default' => get_theme_support('custom-background', 'default-preset'), 'theme_supports' => 'custom-background', 'sanitize_callback' => [$this, '_sanitize_background_setting']]);
        $this->add_control('background_preset', ['label' => _x('Preset', 'Background Preset'), 'section' => 'background_image', 'type' => 'select', 'choices' => ['default' => _x('Default', 'Default Preset'), 'fill' => __('Fill Screen'), 'fit' => __('Fit to Screen'), 'repeat' => _x('Repeat', 'Repeat Image'), 'custom' => _x('Custom', 'Custom Preset')]]);
        $this->add_setting('background_position_x', ['default' => get_theme_support('custom-background', 'default-position-x'), 'theme_supports' => 'custom-background', 'sanitize_callback' => [$this, '_sanitize_background_setting']]);
        $this->add_setting('background_position_y', ['default' => get_theme_support('custom-background', 'default-position-y'), 'theme_supports' => 'custom-background', 'sanitize_callback' => [$this, '_sanitize_background_setting']]);
        $this->add_control(new WP_Customize_Background_Position_Control($this, 'background_position', ['label' => __('Image Position'), 'section' => 'background_image', 'settings' => ['x' => 'background_position_x', 'y' => 'background_position_y']]));
        $this->add_setting('background_size', ['default' => get_theme_support('custom-background', 'default-size'), 'theme_supports' => 'custom-background', 'sanitize_callback' => [$this, '_sanitize_background_setting']]);
        $this->add_control('background_size', ['label' => __('Image Size'), 'section' => 'background_image', 'type' => 'select', 'choices' => ['auto' => _x('Original', 'Original Size'), 'contain' => __('Fit to Screen'), 'cover' => __('Fill Screen')]]);
        $this->add_setting('background_repeat', ['default' => get_theme_support('custom-background', 'default-repeat'), 'sanitize_callback' => [$this, '_sanitize_background_setting'], 'theme_supports' => 'custom-background']);
        $this->add_control('background_repeat', ['label' => __('Repeat Background Image'), 'section' => 'background_image', 'type' => 'checkbox']);
        $this->add_setting('background_attachment', ['default' => get_theme_support('custom-background', 'default-attachment'), 'sanitize_callback' => [$this, '_sanitize_background_setting'], 'theme_supports' => 'custom-background']);
        $this->add_control('background_attachment', ['label' => __('Scroll with Page'), 'section' => 'background_image', 'type' => 'checkbox']);
        /*
         ...
         */
        $this->add_section('static_front_page', ['title' => __('Homepage Settings'), 'priority' => 120, 'description' => __('You can choose what&#8217;s displayed on the homepage of your site. It can be posts in reverse chronological order (classic blog), or a fixed/static page. To set a static homepage, you first need to create two Pages. One will become the homepage, and the other will be where your posts are displayed.'), 'active_callback' => [$this, 'has_published_pages']]);
        $this->add_setting('show_on_front', ['default' => get_option('show_on_front'), 'capability' => 'manage_options', 'type' => 'option']);
        $this->add_control('show_on_front', ['label' => __('Your homepage displays'), 'section' => 'static_front_page', 'type' => 'radio', 'choices' => ['posts' => __('Your latest posts'), 'page' => __('A static page')]]);
        $this->add_setting('page_on_front', ['type' => 'option', 'capability' => 'manage_options']);
        $this->add_control('page_on_front', ['label' => __('Homepage'), 'section' => 'static_front_page', 'type' => 'dropdown-pages', 'allow_addition' => true]);
        $this->add_setting('page_for_posts', ['type' => 'option', 'capability' => 'manage_options']);
        $this->add_control('page_for_posts', ['label' => __('Posts page'), 'section' => 'static_front_page', 'type' => 'dropdown-pages', 'allow_addition' => true]);
        /* Custom CSS */
        ...
        $section_description .= '</p>';
        $this->add_section('custom_css', ['title' => __('Additional CSS'), 'priority' => 200, 'description_hidden' => true, 'description' => $section_description]);
        $custom_css_setting = new WP_Customize_Custom_CSS_Setting($this, sprintf('custom_css[%s]', get_stylesheet()), ['capability' => 'edit_css', 'default' => '']);
        $this->add_setting($custom_css_setting);
        $this->add_control(new WP_Customize_Code_Editor_Control($this, 'custom_css', ['label' => __('CSS code'), 'section' => 'custom_css', 'settings' => ['default' => $custom_css_setting->id], 'code_type' => 'text/css', 'input_attrs' => ['aria-describedby' => 'editor-keyboard-trap-help-1 editor-keyboard-trap-help-2 editor-keyboard-trap-help-3 editor-keyboard-trap-help-4']]));
    }
    /**
    ...
    public function has_published_pages()
    {
        ...
        return 0 !== count(get_pages(['number' => 1, 'hierarchical' => 0]));
    }
    /**
    ...
    public function handle_load_themes_request()
    {
        ...
        if ('installed' === $theme_action) {
            ...
            $wporg_args = ['per_page' => 100, 'fields' => ['reviews_url' => true]];
            ...
            foreach ($themes->themes as &$theme) {
                $theme->install_url = add_query_arg(['theme' => $theme->slug, '_wpnonce' => wp_create_nonce('install-theme_' . $theme->slug)], $update_php);
                ...
                $theme->author_and_uri = wp_kses($theme->author['display_name'], $themes_allowedtags);
