...
class WP_REST_Server
{
    ...
    public function get_index($request)
    {
        ...
        if (wp_is_client_side_media_processing_enabled() && current_user_can('upload_files')) {
            ...
            $available['image_sizes'] = [];
            foreach (wp_get_registered_image_subsizes() as $name => $size) {
                $available['image_sizes'][ $name ] = $size;
            }

            /** This filter is documented in wp-admin/includes/image.php */
            $available['image_size_threshold'] = (int) apply_filters('big_image_size_threshold', 2560, [ 0, 0 ], '', 0);

            // Image output formats.
            $input_formats  = [ 'image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/avif', 'image/heic' ];
            $output_formats = [];
