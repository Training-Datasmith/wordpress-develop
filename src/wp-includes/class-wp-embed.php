...
<script>
	jQuery( function($) {
		$.get("<?php 
        echo esc_url(admin_url('admin-ajax.php', 'relative')) . '?action=oembed-cache&post=' . $post->ID;
        ?>");
	} );
</script>
		<?php 
    ...
    public function shortcode($attr, $url = '')
    {
        ...
        if ($post_id) {
            ...
            $insert_post_args = ['post_name' => $key_suffix, 'post_status' => 'publish', 'post_type' => 'oembed_cache'];
            if ($html) {
                if ($cached_post_id) {
                    wp_update_post(wp_slash(['ID' => $cached_post_id, 'post_content' => $html]));
                } else {
                    wp_insert_post(wp_slash(array_merge($insert_post_args, ['post_content' => $html])));
                }
            } elseif (!$cache) {
                wp_insert_post(wp_slash(array_merge($insert_post_args, ['post_content' => '{{unknown}}'])));
            }
            if ($has_kses) {
                kses_init_filters();
            }
    ...
    public function find_oembed_post_id($cache_key)
    {
        ...
        $oembed_post_query = new WP_Query(['post_type' => 'oembed_cache', 'post_status' => 'publish', 'name' => $cache_key, 'posts_per_page' => 1, 'no_found_rows' => true, 'cache_results' => true, 'update_post_meta_cache' => false, 'update_post_term_cache' => false, 'lazy_load_term_meta' => false]);
