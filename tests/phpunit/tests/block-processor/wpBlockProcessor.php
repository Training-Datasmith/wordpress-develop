...
class Tests_Blocks_BlockProcessor extends WP_UnitTestCase
{
    ...
    {
        return [
            'Shortest HTML comment'         => [ '<!-->' ],
            'Span-of-dashes'                => [ '<!------>' ],
            'Empty HTML comment'            => [ '<!-- -->' ],
            'HTML comment with exclamation' => [ '<!-- --! is not the end -->' ],
            'No spaces, minimal info'       => [ '<!--wp:block-->' ],
            'No spaces, minimal info, void' => [ '<!--wp:block/-->' ],
            'No spaces, empty JSON'         => [ '<!--wp:block{}-->' ],
            'No spaces, empty JSON, void'   => [ '<!--wp:block{}/-->' ],
            'No space before wp:'           => [ '<!--wp:block -->' ],
            'No space after name'           => [ '<!-- wp:block-->' ],
            'No space before JSON'          => [ '<!-- wp:block{} -->' ],
            'No space after JSON'           => [ '<!-- wp:block {}-->' ],
            'Missing wp:'                   => [ '<!-- core/paragraph -->' ],
            'Malformed wp:'                 => [ '<!-- wordpress:core/paragraph -->' ],
            'Malformed block namespace'     => [ '<!-- wp:3more/block -->' ],
            'Malformed block name'          => [ '<!-- wp:core/paragraph/variation -->' ],
            'Invalid block name characters' => [ '<!-- wp:core/32-block -->' ],
        ];
    }
    ...
    {
        return [
            'Opening and non-whitespace' => [ '<!--[' ],
            'Non wp: token'              => [ '<!-- this is not a block' ],
            'Non-: after wp'             => [ '<!-- wpm' ],
            'Invalid namespace'          => [ '<!-- wp:)' ],
            'Invalid name'               => [ '<!-- wp:core//test' ],
            'Invalid full name'          => [ '<!-- wp:core/test/variation' ],
            'Missing final ->'           => [ '<!-- -' ],
            'Missing final > (has !)'    => [ '<!-- --!' ],
            'Missing final >'            => [ '<!-- --' ],
        ];
    ...
    public static function data_invalid_block_json()
    {
        return [
            'Extra opening {' => [ '{{}' ],
            'Extra closing }' => [ '{}}' ],
            'Unquoted string' => [ '{"name": block}' ],
            'Invalid number'  => [ '{"level": 14e6e7-3}' ],
        ];
    }
    ...
    public static function data_unrecognizable_block_json()
    {
        return [
            ...
            'Nested list'        => [ '[{"type": "broken"}]' ],
            'True'               => [ 'true' ],
            'False'              => [ 'false' ],
            'null'               => [ 'null' ],
            'Number (36)'        => [ '36' ],
            'Number (3.141e0)'   => [ '3.141e0' ],
            'Letters'            => [ 'not_even_json' ],
            'Parentheses'        => [ '()' ],
            'Emoji'              => [ '🥳' ],
            'Canadian Syllabics' => [ 'ᐸint32ᐳ' ],
        ];
    }
    ...
    {
        return [
            'Void'                         => [ '<!-- wp:void /-->', WP_Block_Processor::VOID ],
            'Void, full name'              => [ '<!-- wp:core/void /-->', WP_Block_Processor::VOID ],
            'Opener'                       => [ '<!-- wp:paragraph -->', WP_Block_Processor::OPENER ],
            'Opener, full name'            => [ '<!-- wp:core/list -->', WP_Block_Processor::OPENER ],
            'Unexpected Closer'            => [ '<!-- /wp:paragraph -->', WP_Block_Processor::CLOSER ],
            'Unexpected Closer, full name' => [ '<!-- /wp:core/list -->', WP_Block_Processor::CLOSER ],
            'Closer'                       => [ '<!-- wp:paragraph --><!-- /wp:paragraph -->', WP_Block_Processor::CLOSER, 1 ],
            'Closer, full name'            => [ '<!-- wp:core/list --><!-- /wp:core/list -->', WP_Block_Processor::CLOSER, 1 ],
        ];
    }
    ...
    {
        return [
            'Opener, core/group' => [ '<!-- wp:core/group -->', 'core/group' ],
            'Void, core/group'   => [ '<!-- wp:core/group /-->', 'core/group' ],
            'Closer, core/group' => [ '<!-- /wp:core/group -->', 'core/group' ],
            'Opener, group'      => [ '<!-- wp:group -->', 'core/group' ],
            'Void, group'        => [ '<!-- wp:group /-->', 'core/group' ],
            'Closer, group'      => [ '<!-- /wp:group -->', 'core/group' ],
            'Opener, my/group'   => [ '<!-- wp:my/group -->', 'my/group' ],
            'Void, thy/group'    => [ '<!-- wp:thy/group /-->', 'thy/group' ],
            'Closer, the5/group' => [ '<!-- /wp:the-5/group -->', 'the-5/group' ],
            'Freeform content'   => [ 'Not a block, but at the top level.', 'core/freeform' ],
        ];
    }
    ...
    public static function data_delimiters_and_sets_of_block_types()
    {
        return [
            // Positive matches.
            'type "test", first in set'                    => [ '<!-- wp:test -->', [ 'test', 'tossed', 'tess' ], true ],
            'type "core/test", first in set'               => [ '<!-- wp:core/test -->', [ 'test', 'tossed', 'tess' ], true ],
            'type "test", middle of set'                   => [ '<!-- wp:test -->', [ 'tust', 'test', 'tossed', 'tess' ], true ],
            'type "core/test", middle of set'              => [ '<!-- wp:core/test -->', [ 'tust', 'test', 'tossed', 'tess' ], true ],
            'type "test", last in set'                     => [ '<!-- wp:test -->', [ 'tust', 'tossed', 'tess', 'test' ], true ],
            'type "core/test", last in set'                => [ '<!-- wp:core/test -->', [ 'tust', 'tossed', 'tess', 'core/test' ], true ],
            'type "test", core/test first in set'          => [ '<!-- wp:test -->', [ 'core/test', 'tossed', 'tess' ], true ],
            'type "core/test", core/test first in set'     => [ '<!-- wp:core/test -->', [ 'core/test', 'tossed', 'tess' ], true ],
            'type "test", core/test in middle of set'      => [ '<!-- wp:test -->', [ 'tust', 'core/test', 'tossed', 'tess' ], true ],
            'type "core/test", core/test in middle of set' => [ '<!-- wp:core/test -->', [ 'tust', 'core/test', 'tossed', 'tess' ], true ],
            'type "test", core/test last in set'           => [ '<!-- wp:test -->', [ 'tust', 'tossed', 'tess', 'core/test' ], true ],
            'type "core/test", core/test last in set'      => [ '<!-- wp:core/test -->', [ 'tust', 'tossed', 'tess', 'core/test' ], true ],
            'non-core, fully-qualified'                    => [ '<!-- wp:test/block -->', [ 'test/ship', 'test/block', 'test/wheel' ], true ],

            // Negative matches.
            'type "test", not in set'                      => [ '<!-- wp:test -->', [ 'text', 'core/text', 'my/test' ], false ],
            'type "core/test", not in set'                 => [ '<!-- wp:core/test -->', [ 'text', 'core/text', 'my/test' ], false ],
            'type "next-dev/code", not in set'             => [ '<!-- wp:next-dev/code -->', [ 'code', 'new/code', 'dev/code' ], false ],

            // Top-level freeform HTML content.
            'Freeform content'                             => [ 'Just some HTML', [ '*', 'core/freeform', 'freeform' ], true ],
        ];
    }
    ...
    {
        return [
            'Only non-block content (one freeform)'     => [ 'this is not inside a block', 0 ],
            'Freeform before a block'                   => [ 'before the block<!-- wp:suffix /-->', 0 ],
            'Freeform after a block'                    => [ '<!-- wp:prefix /-->after the block', 1 ],
            'Freeform between blocks'                   => [ '<!-- wp:prefix /-->after the block<!-- wp:suffix /-->', 1 ],
            'Visits only freeform HTML, not inner HTML' => [ '<!-- wp:block -->this is innerHTML<!-- /wp:block -->this is freeform', 3 ],
        ];
    }
...
HTML
    ...
    public function test_returns_appropriate_span_for_delimiters($html, ...$spans)
    {
        ...
        foreach ($spans as $i => $span) {
            ...
            $this->assertSame(
                $span,
                [ $reported->start, $reported->length ],
    ...
    {
        return [
            'Before matching' => [ 'Blocks <!-- wp:will/not --> advance yet' ],
            'Only freeform'   => [ 'Have a lovely day.', [ 0, 18 ] ],
            'Only void'       => [ '<!-- wp:into/abyss /-->', [ 0, 23 ] ],
            'Mixed'           => [ '<!-- wp:pw --><><!-- /wp:pw -->', [ 0, 14 ], [ 14, 2 ], [ 16, 15 ] ],
        ];
    }
    ...
    public static function data_various_block_posts()
    {
        ...
        yield 'Void block' => [ '<!-- wp:void /-->' ];

        yield 'Empty block' => [ '<!-- wp:empty --><!-- /wp:empty -->' ];

        yield 'Paragraph block' => [ '<!-- wp:paragraph --><p>Test</p><!-- /wp:paragraph -->' ];

        yield 'Paragraph block with attributes' => [
            '<!-- wp:paragraph {"dropCaps": true} --><p>Test</p><!-- /wp:paragraph -->',
        ];

        yield 'Group with void inner' => [
...
HTML
        ...
        yield 'Attributes on nested blocks' => [
            <<<HTML
			<!-- wp:b1 -->
			<!-- wp:b2 {} -->
			<!-- wp:b3 {"id":"going"} -->
...
HTML
    ...
    {
        return [
            'At start of HTML'    => [ '<!-- wp:target -->', 'target' ],
            'After freeform text' => [ 'prefix<!-- wp:target -->', 'target' ],
            'After outer block'   => [ 'prefix<!-- wp:group --><!-- wp:target -->', 'target' ],
            'After innerHTML'     => [ 'prefix<!-- wp:group -->inner<!-- wp:target -->', 'target' ],
        ];
    }
