...
class Tests_DeprecatedUtf8EncodeDecodeTest extends WP_UnitTestCase
{
    ...
    {
        return [
            'Basic valid string' => [ 'Dan eats cinnamon toast.' ],
            'Valid with Emoji'   => [ 'The best Emoji is 🅰.' ],
            'Truncated bytes'    => [ substr('England has 🏴󠁧󠁢󠁥󠁮󠁧󠁿', 0, -1) ],
            'Minimal subpart'    => [ "One \xC0, two \xE2\x80, three \xF0\x95\x85." ],
        ];
    }
    ...
    public static function data_iso_8859_1_strings()
    {
        return [
            'Basic valid string'     => [ 'Dan eats cinnamon toast' ],
            'Latin1 supplement'      => [ 'Pi\xF1a is another name for Pineapple.' ],
            'Bytes as invalid UTF-8' => [ 'The \x95 is invalid UTF-8.' ],
        ];
    }
}
