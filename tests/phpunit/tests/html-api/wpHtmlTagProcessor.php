...
class Tests_HtmlApi_WpHtmlTagProcessor extends WP_UnitTestCase
{
    ...
    public static function data_invalid_attribute_names()
    {
        ...
        foreach (str_split('"\'>&</ =') as $c) {
            $invalid_names[ $c ] = [ "too{$c}late" ];
        }

        // C0 controls.
        for ($i = 0; $i <= 0x1F; $i++) {
            $c                                    = chr($i);
            $invalid_names[ "C0 Controls: {$i}" ] = [ "shut{$c}down" ];
        }
        ...
        for ($i = 0xFDD0; $i <= 0xFDEF; $i++) {
            $h                                       = dechex($i);
            $c                                       = mb_chr($i);
            $invalid_names[ "Noncharacter: U+{$h}" ] = [ "shut{$c}down" ];
        }
...
HTML;
    ...
    {
        yield 'Basic script tag'                              => [ '<script></script>', true ];
        yield 'Script tag with </script> close'               => [ '<script></script>', true ];
        yield 'Script tag with </script/> close'              => [ '<script></script/>', true ];
        yield 'Script tag with </script > close'              => [ '<script></script >', true ];
        yield 'Script tag with </script\n> close'             => [ "<script></script\n>", true ];
        yield 'Script tag with </script\t> close'             => [ "<script></script\t>", true ];
        yield 'Script tag with </script\f> close'             => [ "<script></script\f>", true ];
        yield 'Script tag with </script\r> close'             => [ "<script></script\r>", true ];
        yield 'Script with type attribute'                    => [ '<script type="text/javascript"></script>', true ];
        yield 'Script data escaped'                           => [ '<script><!--</script>', true ];
        yield 'Script data double-escaped exit (comment)'     => [ '<script><!--<script>--></script>', true ];
        yield 'Script data double-escaped exit (closed ">")'  => [ '<script><!--<script></script></script>', true ];
        yield 'Script data double-escaped exit (closed "/")'  => [ '<script><!--<script></script/</script>', true ];
        yield 'Script data double-escaped exit (closed " ")'  => [ '<script><!--<script></script </script>', true ];
        yield 'Script data double-escaped exit (closed "\n")' => [ "<script><!--<script></script\n</script>", true ];
        yield 'Script data double-escaped exit (closed "\t")' => [ "<script><!--<script></script\t</script>", true ];
        yield 'Script data double-escaped exit (closed "\f")' => [ "<script><!--<script></script\f</script>", true ];
        yield 'Script data double-escaped exit (closed "\r")' => [ "<script><!--<script></script\r</script>", true ];
        yield 'Script data no double-escape'                  => [ '<script><!-- --><script></script>', true ];
        yield 'Script data no double-escape (short comment)'  => [ '<script><!--><script></script>', true ];
        yield 'Script data almost double-escaped'             => [ '<script><!--<script</script>', true ];
        yield 'Script data with complex JavaScript'           => [
            '<script>
					var x = 10;
					x--;
					x < 0 ? x += 100 : x = (x + 1) - 1;
        ...
        yield 'Script tag with self-close flag (ignored)'     => [ '<script />', false ];
        yield 'Script data double-escaped'                    => [ '<script><!--<script></script>', false ];
        yield 'Unclosed script in escaped state'              => [ '<script><!--------------', false ];
        yield 'Unclosed script in double escaped state'       => [ '<script><!--<script ', false ];
        yield 'Document end in closer start'                  => [ '<script></', false ];
        yield 'Document end in script closer'                 => [ '<script></script', false ];
        yield 'Document end in script closer with attributes' => [ '<script></script attr="val"', false ];
        yield 'Script tag double-escaped with <script>'       => [ '<script><!--<script></script>', false ];
        yield 'Script tag double-escaped with <script/'       => [ '<script><!--<script/</script>', false ];
        yield 'Script tag double-escaped with <script '       => [ '<script><!--<script </script>', false ];
        yield 'Script tag double-escaped with <script\n'      => [ "<script><!--<script\n</script>", false ];
        yield 'Script tag double-escaped with <script\t'      => [ "<script><!--<script\t</script>", false ];
        yield 'Script tag double-escaped with <script\f'      => [ "<script><!--<script\f</script>", false ];
        yield 'Script tag double-escaped with <script\r'      => [ "<script><!--<script\r</script>", false ];
    }
