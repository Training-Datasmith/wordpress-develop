...
HTML
    ...
    public static function data_unallowed_modifiable_text_updates()
    {
        return [
            ...
            'Non-JS SCRIPT with <script>'             => [ '<script type="text/html">Replace me</script>', '<!-- Just a <script>' ],
            'Non-JS SCRIPT with </script>'            => [ '<script type="text/plain">Replace me</script>', 'Just a </script>' ],
            'Non-JS SCRIPT with <script attributes>'  => [ '<script language="text">Replace me</script>', '<!-- <script sneaky>after' ],
            'Non-JS SCRIPT with </script attributes>' => [ '<script language="text">Replace me</script>', 'before</script sneaky>after' ],
        ];
    }
    ...
    public static function data_script_tag_text_updates(): array
    {
        return [
            ...
            'var script;1<script>0'                 => [ '<script></script>', 'var script;1<script>0', '<script>var script;1<\u0073cript>0</script>' ],
            '1</script>/'                           => [ '<script></script>', '1</script>/', '<script>1</\u0073cript>/</script>' ],
            'var SCRIPT;1<SCRIPT>0'                 => [ '<script></script>', 'var SCRIPT;1<SCRIPT>0', '<script>var SCRIPT;1<\u0053CRIPT>0</script>' ],
            '1</SCRIPT>/'                           => [ '<script></script>', '1</SCRIPT>/', '<script>1</\u0053CRIPT>/</script>' ],
            '"</script>"'                           => [ '<script></script>', '"</script>"', '<script>"</\u0073cript>"</script>' ],
            '"</ScRiPt>"'                           => [ '<script></script>', '"</ScRiPt>"', '<script>"</\u0053cRiPt>"</script>' ],
            'Tricky script open tag with \r'        => [ '<script></script>', "<!-- <script\r>", "<script><!-- <\\u0073cript\r></script>" ],
            'Tricky script open tag with \r\n'      => [ '<script></script>', "<!-- <script\r\n>", "<script><!-- <\\u0073cript\r\n></script>" ],
            'Tricky script close tag with \r'       => [ '<script></script>', "// </script\r>", "<script>// </\\u0073cript\r></script>" ],
            'Tricky script close tag with \r\n'     => [ '<script></script>', "// </script\r\n>", "<script>// </\\u0073cript\r\n></script>" ],
            'Module tag'                            => [ '<script type="module"></script>', '"<script>"', '<script type="module">"<\u0073cript>"</script>' ],
            'Tag with type'                         => [ '<script type="text/javascript"></script>', '"<script>"', '<script type="text/javascript">"<\u0073cript>"</script>' ],
            'Tag with language'                     => [ '<script language="javascript"></script>', '"<script>"', '<script language="javascript">"<\u0073cript>"</script>' ],
            'Non-JS script, save HTML-like content' => [ '<script type="text/html"></script>', '<h1>This & that</h1>', '<script type="text/html"><h1>This & that</h1></script>' ],
        ];
    }
...
HTML;
    ...
    public static function data_set_modifiable_fails_non_atomic_tags(): array
    {
        return [
            ...
            'svg PATH (self-closing)'    => [ '<path />', 'svg', 'PATH' ],
            'math MTEXT'                 => [ '<mtext></mtext>', 'math', 'MTEXT' ],
            'math MSPACE (self-closing)' => [ '<mspace />', 'math', 'MSPACE' ],

            // Foreign elements with atomic-like tags.
            'svg TEXTAREA'               => [ '<textarea></textarea>', 'svg', 'TEXTAREA' ],
            'svg TITLE'                  => [ '<title></title>', 'svg', 'TITLE' ],
            'svg STYLE'                  => [ '<style></style>', 'svg', 'STYLE' ],
            'svg SCRIPT'                 => [ '<script></script>', 'svg', 'SCRIPT' ],
            'math TEXTAREA'              => [ '<textarea></textarea>', 'math', 'TEXTAREA' ],
            'math TITLE'                 => [ '<title></title>', 'math', 'TITLE' ],
            'math STYLE'                 => [ '<style></style>', 'math', 'STYLE' ],
            'math SCRIPT'                => [ '<script></script>', 'math', 'SCRIPT' ],
        ];
    }
}
