...
class Tests_Interactivity_API_WpInteractivityAPI extends WP_UnitTestCase
{
    ...
    public function test_get_directive_entries()
    {
        ...
        $p->next_tag();
        $results = $get_directive_entries->invoke($this->interactivity, $p, 'test');
        $this->assertSame([ 'key' => 'value' ], $results[0]['value']);
    ...
    private function evaluate($entry)
    {
        ...
        $result = $evaluate->invokeArgs($this->interactivity, [ $entry ]);
...
HTML;
    ...
    public function test_add_client_navigation_support_to_script_module()
    {
        ...
        $output = get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]);

        $p = new WP_HTML_Tag_Processor($output);

        // First module: marked-module should have the attribute.
        $p->next_tag([ 'tag_name' => 'SCRIPT' ]);
