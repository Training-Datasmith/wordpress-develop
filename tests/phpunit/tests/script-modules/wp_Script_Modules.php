...
class Tests_Script_Modules_WpScriptModules extends WP_UnitTestCase
{
    ...
    public function get_enqueued_script_modules(): array
    {
                ...
                $modules[ $id ] = array_merge(
                    ...
                    ...array_map(
                        static function ($attribute_name) use ($p) {
                            return [ $attribute_name => $p->get_attribute($attribute_name) ];
                        },
        ...
        $modules = array_merge(
            $get_modules(get_echo([ $this->script_modules, 'print_head_enqueued_script_modules' ]), false),
            $get_modules(get_echo([ $this->script_modules, 'print_enqueued_script_modules' ]), true)
        );
    ...
    {
        $p = new WP_HTML_Tag_Processor(get_echo([ $this->script_modules, 'print_import_map' ]));
        if ($p->next_tag([ 'tag' => 'SCRIPT' ])) {
    ...
    public function get_preloaded_script_modules(): array
    {
        ...
        $p = new WP_HTML_Tag_Processor(get_echo([ $this->script_modules, 'print_script_module_preloads' ]));
    ...
    public function test_comprehensive_methods(bool $use_global_function, bool $only_enqueue)
    {
        ...
        $this->assertSame([ 'a' ], wp_script_modules()->get_queue(), 'Expected queue to match.');
        ...
        $this->assertSame([], $get_dependencies->invoke(wp_script_modules(), [ 'a' ]));

        // One Dependency.
        $register('b-dep', '/b-dep.js');
        $register_and_enqueue('b', '/b.js', [ 'b-dep' ]);
        $this->assertTrue(wp_script_modules()->set_fetchpriority('b', 'low'));
        $this->assertSame([ 'a', 'b' ], wp_script_modules()->get_queue());
        $this->assertSame(
            [
                'b-dep' => [
                    'src'           => '/b-dep.js',
            ...
            $get_dependencies->invoke(wp_script_modules(), [ 'b' ])
        );

        // Two dependencies with different formats and a false version.
        $register('c-dep', '/c-static.js', [], false, [ 'fetchpriority' => 'low' ]);
        $register('c-static-dep', '/c-static-dep.js', [], false, [ 'fetchpriority' => 'high' ]);
        ...
        $this->assertSame([ 'a', 'b', 'c' ], wp_script_modules()->get_queue());

        // Two dependencies, one imported statically and the other dynamically, with a null version.
        $register('d-static-dep', '/d-static-dep.js', [], false, [ 'fetchpriority' => 'auto' ]);
        $register('d-dynamic-dep', '/d-dynamic-dep.js', [], false, [ 'fetchpriority' => 'high' ]); // Because this is a dynamic import dependency, the fetchpriority will not be reflected in the markup since no SCRIPT tag and no preload LINK are printed, and the importmap SCRIPT does not support designating a priority.
        $register_and_enqueue(
            'd',
            '/d.js',
            [
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd' ], wp_script_modules()->get_queue());
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd', 'e' ], wp_script_modules()->get_queue());
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd', 'e', 'f' ], wp_script_modules()->get_queue());
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd', 'e', 'f', 'g' ], wp_script_modules()->get_queue());
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h' ], wp_script_modules()->get_queue());
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i' ], wp_script_modules()->get_queue());
        ...
        $this->assertSame([ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j' ], wp_script_modules()->get_queue());
    ...
    public function data_test_register_and_enqueue_script_module(): array
    {
        ...
        foreach ([ true, false ] as $use_global_function) {
            foreach ([ true, false ] as $only_enqueue) {
    ...
    {
        wp_enqueue_script_module('with-src', '/src.js', [], null);
        ...
        $this->assertSame([ 'with-src', 'without-src', 'without-src-but-filtered' ], wp_script_modules()->get_queue());
        add_filter(
        ...
        $actual   = get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]);
        $expected = <<<'HTML'
...
HTML;
    ...
    */
    ...
    public function test_version_is_propagated_correctly()
    {
        ...
        $this->script_modules->register('dep', '/dep.js', [], '2.0', [ 'fetchpriority' => 'high' ]);
        $this->script_modules->enqueue('foo');
    ...
    {
        $this->script_modules->enqueue('foo', '/foo.js', [ 'dep' ], '1.0', [ 'fetchpriority' => 'low' ]);
        $this->script_modules->register('dep', '/dep.js');
...
HTML;
    ...
    public static function data_special_chars_script_encoding(): array
    {
        return [
            ...
            // Non UTF-8
            'Solidus non-utf8'                       => [ '/', '/', 'iso-8859-1' ],
            'Less than non-utf8'                     => [ '<', '\u003C', 'iso-8859-1' ],
            'Greater than non-utf8'                  => [ '>', '\u003E', 'iso-8859-1' ],
            'Ampersand non-utf8'                     => [ '&', '&', 'iso-8859-1' ],
            'Newline non-utf8'                       => [ "\n", '\\n', 'iso-8859-1' ],
            'Tab non-utf8'                           => [ "\t", '\\t', 'iso-8859-1' ],
            'Form feed non-utf8'                     => [ "\f", '\\f', 'iso-8859-1' ],
            'Carriage return non-utf8'               => [ "\r", '\\r', 'iso-8859-1' ],
            'Line separator non-utf8'                => [ "\u{2028}", "\u2028", 'iso-8859-1' ],
            'Paragraph separator non-utf8'           => [ "\u{2029}", "\u2029", 'iso-8859-1' ],
            /*
             * The following is the Flag of England emoji
             * PHP: "\u{1F3F4}\u{E0067}\u{E0062}\u{E0065}\u{E006E}\u{E0067}\u{E007F}"
             */
            'Flag of england non-utf8'               => [ '🏴󠁧󠁢󠁥󠁮󠁧󠁿', "\ud83c\udff4\udb40\udc67\udb40\udc62\udb40\udc65\udb40\udc6e\udb40\udc67\udb40\udc7f", 'iso-8859-1' ],
            'Malicious script closer non-utf8'       => [ '</script>', '\u003C/script\u003E', 'iso-8859-1' ],
            'Entity-encoded malicious script closer non-utf8' => [ '&lt;/script&gt;', '&lt;/script&gt;', 'iso-8859-1' ],
    ...
    {
        $this->script_modules->register('test-script', '/test-script.js', [], null, [ 'fetchpriority' => $fetchpriority ]);
    ...
    {
        wp_register_script_module('default', '/default.js', [], null);
        wp_enqueue_script_module('default');

        wp_register_script_module('in-footer-via-register', '/in-footer-via-register.js', [], null, [ 'in_footer' => true ]);
        wp_enqueue_script_module('in-footer-via-register');

        wp_enqueue_script_module('in-footer-via-enqueue', '/in-footer-via-enqueue.js', [], null, [ 'in_footer' => true ]);

        wp_enqueue_script_module('not-in-footer-via-enqueue', '/not-in-footer-via-enqueue.js', [], null, [ 'in_footer' => false ]);

        wp_enqueue_script_module('in-footer-via-override', '/in-footer-via-override.js', [], null);
        wp_script_modules()->set_in_footer('in-footer-via-override', true);

        wp_enqueue_script_module('not-in-footer-via-override', '/not-in-footer-via-override.js', [], null, [ 'in_footer' => true ]);
        wp_script_modules()->set_in_footer('not-in-footer-via-override', false);

        $actual_head   = get_echo([ wp_script_modules(), 'print_head_enqueued_script_modules' ]);
        $actual_footer = get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]);
...
HTML;
    ...
    {
        $this->script_modules->register('foo', '/foo.js', [], false, [ 'fetchpriority' => 'silly' ]);
        $registered_modules = $this->get_registered_script_modules($this->script_modules);
    ...
    {
        $this->script_modules->register('foo', '/foo.js', [], false, [ 'fetchpriority' => [ 'WHY AM I NOT A STRING???' ] ]);
        $registered_modules = $this->get_registered_script_modules($this->script_modules);
        $this->assertSame('auto', $registered_modules['foo']['fetchpriority']);
        $this->assertArrayHasKey('WP_Script_Modules::register', $this->caught_doing_it_wrong);
    ...
    public function test_fetchpriority_bumping(array $enqueues, array $expected)
    {
        $this->script_modules->register(
            ...
            [],
            null,
            [ 'fetchpriority' => 'low' ] // This won't show up anywhere since it is a dynamic import dependency.
        );
    ...
    {
        wp_register_script_module('a', '/a.js', [ 'b' ], null, [ 'fetchpriority' => 'low' ]);
        wp_register_script_module('b', '/b.js', [ 'c' ], null, [ 'fetchpriority' => 'auto' ]);
        wp_register_script_module('c', '/c.js', [ 'd', 'e' ], null, [ 'fetchpriority' => 'auto' ]);
        wp_register_script_module('d', '/d.js', [ 'z' ], null, [ 'fetchpriority' => 'high' ]);
        wp_register_script_module('e', '/e.js', [], null, [ 'fetchpriority' => 'auto' ]);

        wp_register_script_module('x', '/x.js', [ 'd', 'y' ], null, [ 'fetchpriority' => 'high' ]);
        wp_register_script_module('y', '/y.js', [ 'z' ], null, [ 'fetchpriority' => 'auto' ]);
        wp_register_script_module('z', '/z.js', [], null, [ 'fetchpriority' => 'auto' ]);

        // The fetch priorities are derived from these enqueued dependents.
        wp_enqueue_script_module('a');
        wp_enqueue_script_module('x');

        $actual   = get_echo([ wp_script_modules(), 'print_script_module_preloads' ]);
        $actual  .= get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]);
        $expected = <<<'HTML'
...
HTML;
    ...
    public function test_fetchpriority_propagation()
    {
        ...
        wp_register_script_module('a', '/a.js', [ 'd', 'e' ], null, [ 'fetchpriority' => 'high' ]);
        wp_register_script_module('b', '/b.js', [ 'e' ], null);
        wp_register_script_module('c', '/c.js', [ 'e', 'f' ], null);
        wp_register_script_module('d', '/d.js', [], null);
        // The low fetchpriority for this module will be disregarded because its enqueued dependent has a non-low priority.
        wp_register_script_module('e', '/e.js', [], null, [ 'fetchpriority' => 'low' ]);
        wp_register_script_module('f', '/f.js', [], null);

        wp_register_script_module('x', '/x.js', [ 'a' ], null, [ 'fetchpriority' => 'low' ]);
        wp_register_script_module('y', '/y.js', [ 'b' ], null, [ 'fetchpriority' => 'auto' ]);
        wp_register_script_module('z', '/z.js', [ 'c' ], null, [ 'fetchpriority' => 'high' ]);
...
HTML;
    ...
    public function test_default_script_modules()
    {
        ...
        $actual_preloads = $this->normalize_markup_for_snapshot(get_echo([ wp_script_modules(), 'print_script_module_preloads' ]));
        $this->assertEqualHTML(
        ...
        $actual_head_script_modules = $this->normalize_markup_for_snapshot(get_echo([ wp_script_modules(), 'print_head_enqueued_script_modules' ]));
        $this->assertEqualHTML(
            '',
            $actual_head_script_modules
        );

        $actual_footer_script_modules = $this->normalize_markup_for_snapshot(get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]));
        $expected                     = <<<'HTML'
...
HTML;
    ...
    {
        wp_default_script_modules();
        wp_register_script_module('not-enqueued', 'https://example.com/not-enqueued.js', [ '@wordpress/a11y' ], null, [ 'priority' => 'high' ]);
        wp_enqueue_script_module('@wordpress/a11y');

        $actual = $this->normalize_markup_for_snapshot(get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]));
        $this->assertEqualHTML(
            "<script type='module' src='/wp-includes/js/dist/script-modules/a11y/index.min.js' id='@wordpress/a11y-js-module' fetchpriority='low'></script>\n",
            $actual
        );
    ...
    public function test_dependent_of_default_script_modules()
    {
        ...
        wp_enqueue_script_module(
            ...
            [ '@wordpress/a11y', '@wordpress/block-library/navigation/view' ],
            null,
            [ 'fetchpriority' => 'high' ]
        );
...
HTML;
    ...
    public function test_direct_queue_manipulation()
    {
        ...
        $this->script_modules->register('bar', '/bar.js');
        $this->script_modules->register('baz', '/baz.js');
        $this->assertSame([], $this->script_modules->get_queue(), 'Expected queue to be empty.');
        $this->script_modules->enqueue('foo');
        $this->script_modules->enqueue('foo');
        $this->script_modules->enqueue('bar');
        $this->assertSame([ 'foo', 'bar' ], $this->script_modules->get_queue(), 'Expected two deduplicated queued items.');
        $this->script_modules->dequeue('foo');
        $this->script_modules->dequeue('foo');
        $this->script_modules->enqueue('baz');
        $this->assertSame([ 'bar', 'baz' ], $this->script_modules->get_queue(), 'Expected items tup be updated after dequeue and enqueue.');
        $this->script_modules->dequeue('baz');
        $this->script_modules->dequeue('bar');
        $this->assertSame([], $this->script_modules->get_queue(), 'Expected queue to be empty after dequeueing both items.');
    }
    ...
    public function test_included_module_appears_in_importmap()
    {
        ...
        $this->script_modules->register('example', '/example.js', [ 'dependency' ]);
        $this->script_modules->register('example2', '/example2.js');

        // Nothing printed now.
        $this->assertSame([], $this->get_enqueued_script_modules(), 'Initial enqueued script modules was wrong.');
        $this->assertSame([], $this->get_preloaded_script_modules(), 'Initial module preloads was wrong.');
        $this->assertSame([], $this->get_import_map(), 'Initial import map was wrong.');
        ...
        $this->assertSame([], $this->get_enqueued_script_modules(), 'Final enqueued script modules was wrong.');
        $this->assertSame([], $this->get_preloaded_script_modules(), 'Final module preloads was wrong.');
        $this->assertEqualSets(
            [ 'example', 'example2', 'dependency' ],
    ...
    public function test_import_map_includes_dependencies_of_classic_scripts_recursive()
    {
        ...
        $this->script_modules->register('not-enqueued', '/not-enqueued.js', [ 'dependency-of-not-enqueued' ]);
        ...
        wp_register_script(
            ...
            false,
            [
                'module_dependencies' => [ 'classic-transitive-dependency' ],
            ]
        );
        wp_enqueue_script(
            'classic',
            ...
            false,
            [
                'module_dependencies' => [ 'not-enqueued' ],
            ]
        );
    ...
    public function test_wp_scripts_doing_it_wrong_for_missing_script_module_dependencies()
    {
        ...
        wp_enqueue_script(
            ...
            null,
            [
                'module_dependencies' => [ 'does-not-exist' ],
            ]
        );

        $import_map = $this->get_import_map();
        $this->assertSame([], $import_map, 'Expected importmap to be empty.');
    ...
    public function test_script_module_printing_and_dependency_ordering(bool $use_global_function, bool $only_enqueue)
    {
        ...
        $register_and_enqueue('a', '/a.js', [], '1.0.0', [ 'in_footer' => true ]);
        ...
        $register_and_enqueue('b', '/b.js', [], '1.0.0', [ 'in_footer' => true ]);
        $register_and_enqueue('c', '/c.js', [ 'b' ], '1.0.0');
        ...
        $register('d', '/d.js', [], '1.0.0', [ 'in_footer' => true ]);
        $register_and_enqueue('e', '/e.js', [ 'd' ], '1.0.0');
        ...
        $register_and_enqueue('f', '/f.js', [], '1.0.0');
        $register_and_enqueue('g', '/g.js', [ 'f' ], '1.0.0', [ 'in_footer' => true ]);
        $register_and_enqueue('h', '/h.js', [ 'g' ], '1.0.0');
        $register_and_enqueue('i', '/i.js', [ 'h' ], '1.0.0');
        ...
        $register_and_enqueue('m', '/m.js', [ 'j', 'l' ], '1.0.0');
        $register_and_enqueue('k', '/k.js', [ 'j' ], '1.0.0');
        $register_and_enqueue('l', '/l.js', [ 'k' ], '1.0.0');
        $register_and_enqueue('j', '/j.js', [], '1.0.0');
        ...
        $register_and_enqueue('n', '/n.js', [ 'q' ], '1.0.0');
        $register_and_enqueue('q', '/q.js', [], '1.0.0');
        $register_and_enqueue('o', '/o.js', [ 'n' ], '1.0.0', [ 'in_footer' => true ]);
        $register_and_enqueue('r', '/r.js', [ 'q', 'o', 'p' ], '1.0.0');
        $register_and_enqueue('p', '/p.js', [], '1.0.0', [ 'in_footer' => true ]);
    ...
    public function test_static_import_dependency_with_dynamic_imports_depending_on_static_import_dependency()
    {
        ...
        wp_register_script_module('enqueued', '/enqueued.js', [ $get_dependency('static1', 'static') ], null);
        wp_register_script_module('static1', '/static1.js', [ $get_dependency('dynamic1', 'dynamic') ], null);
        wp_register_script_module('dynamic1', '/dynamic1.js', [ $get_dependency('static2', 'static') ], null);
        wp_register_script_module('static2', '/static2.js', [], null);

        wp_enqueue_script_module('enqueued');
        $import_map     = $this->get_import_map();
        $preload_links  = get_echo([ wp_script_modules(), 'print_script_module_preloads' ]);
        $script_modules = get_echo([ wp_script_modules(), 'print_enqueued_script_modules' ]);
    ...
    public function test_missing_script_module_dependency_triggers_incorrect_usage()
    {
        ...
        $this->script_modules->enqueue('main-module', '/main-module.js', [ 'missing-mod-dep' ]);

        $markup = get_echo([ $this->script_modules, 'print_enqueued_script_modules' ]);
