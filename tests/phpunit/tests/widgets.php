...
class Tests_Widgets extends WP_UnitTestCase
{
    ...
    public function test_retrieve_widgets_converts_null_sidebar_to_empty_array()
    {
        global $sidebars_widgets;
        wp_widgets_init();
        $this->register_sidebars([ 'primary', 'secondary', 'wp_inactive_widgets' ]);

        $sidebars_widgets = [
            'primary'             => null,
            'secondary'           => [ 'text-1' ],
            'extra_sidebar'       => [ 'unregistered_widget-1' ],
            'wp_inactive_widgets' => [],
        ];

        $result = retrieve_widgets(true);
        $this->assertArrayHasKey('primary', $result);
        $this->assertSame([], $result['primary'], 'Primary sidebar should be an empty array after normalization.');
    ...
    {
        $this->register_sidebars([ 'primary', 'wp_inactive_widgets' ]);
        ...
        $this->assertSame([], $new_sidebars['primary'], 'Primary sidebar should be an empty array after normalization.');
    }
