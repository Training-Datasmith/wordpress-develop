...
HTML
    ...
    public function test_update_block_with_value_from_source($bound_attribute, $block_content, $expected_result)
    {
        ...
        $parsed_blocks[0]['attrs']['metadata'] = [
            'bindings' => [
                $bound_attribute => [
...
HTML;
        ...
        $parsed_blocks    = parse_blocks($block_content);
        $block            = new WP_Block($parsed_blocks[0], [ 'pattern/overrides' => [ 'Test' => [ 'myAttribute' => $expected_content ] ] ]);
        ...
        $expected_bindings_metadata = [
            'myAttribute' => [ 'source' => 'core/pattern-overrides' ],
        ];
        $this->assertSame(
            $expected_bindings_metadata,
