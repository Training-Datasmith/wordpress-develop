...
class WP_Test_REST_Comments_Controller extends WP_Test_REST_Controller_Testcase
{
    ...
    public function test_cannot_read_note_without_post_type_support()
    {
        ...
        $post_id = self::factory()->post->create([ 'post_type' => 'no-notes' ]);
        $request = new WP_REST_Request('GET', '/wp/v2/comments');
        $request->set_param('post', $post_id);
    ...
    public function test_cannot_create_note_without_post_type_support()
    {
        ...
        wp_set_current_user(self::$admin_id);
        $post_id = self::factory()->post->create([ 'post_type' => 'no-note' ]);
        $params  = [
            'post'         => $post_id,
            'author_name'  => 'Ishmael',
            'author_email' => 'herman-melville@earthlink.net',
    ...
    public function test_create_note_status()
    {
        ...
        $post_id = self::factory()->post->create([ 'post_author' => self::$author_id ]);
    ...
    public function test_note_get_items_permissions_mixed_post_authors()
    {
        ...
        $request = new WP_REST_Request('GET', '/wp/v2/comments');
        $request->set_param('post', [ $author_post_id, $editor_post_id ]);
        $request->set_param('type', 'note');
        $request->set_param('status', 'all');
        $request->set_param('per_page', 100);
        $request->set_param('context', 'edit');
    ...
    {
        return [
            'Administrator can see note on other posts'  => [ 'administrator', 'author', true ],
            'Editor can see note on other posts'         => [ 'editor', 'contributor', true ],
            'Author cannot see note on other posts'      => [ 'author', 'editor', false ],
            'Contributor cannot see note on other posts' => [ 'contributor', 'author', false ],
            'Subscriber cannot see note'                 => [ 'subscriber', 'author', false ],
            'Author can see note on own post'            => [ 'author', 'author', true ],
            'Contributor can see note on own post'       => [ 'contributor', 'contributor', true ],
        ];
    }
    ...
    {
        return [
            'comment type'    => [ 'comment', 5 ],
            'annotation type' => [ 'annotation', 5 ],
            'discussion type' => [ 'discussion', 9 ],
            'note type'       => [ 'note', 3 ],
        ];
    }
}
