...
class WP_Test_REST_Sync_Server extends WP_Test_REST_Controller_Testcase
{
    ...
    public static function wpSetUpBeforeClass(WP_UnitTest_Factory $factory)
    {
        ...
        self::$post_id       = $factory->post->create([ 'post_author' => self::$editor_id ]);
    }
     ...
     */
    private function build_room($room, $client_id = 1, $cursor = 0, $awareness = [], $updates = [])
    {
        if (empty($awareness)) {
            $awareness = [ 'user' => 'test' ];
        }
    ...
    private function dispatch_sync($rooms)
    {
        ...
        $request->set_body_params([ 'rooms' => $rooms ]);
    ...
    public function test_create_item()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room($this->get_post_room()) ]);
    ...
    public function test_sync_post_type_collection_requires_edit_posts_capability()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room('postType/post') ]);
    ...
    public function test_sync_root_collection_allowed()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room('root/site') ]);
    ...
    public function test_sync_taxonomy_collection_allowed()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room('taxonomy/category') ]);
    ...
    public function test_sync_unknown_collection_kind_rejected()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room('unknown/entity') ]);
    ...
    public function test_sync_non_posttype_entity_with_object_id_rejected()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room('root/site:123') ]);
    ...
    public function test_sync_nonexistent_post_rejected()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room('postType/post:999999') ]);
    ...
    public function test_sync_response_room_matches_request()
    {
        ...
        $response = $this->dispatch_sync([ $this->build_room($room) ]);
    ...
    public function test_sync_update_delivered_to_other_client()
    {
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'user' => 'client1' ], [ $update ]),
            ]
        );
    ...
    public function test_sync_multiple_updates_in_single_request()
    {
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'user' => 'client1' ], $updates),
            ]
        );
    ...
    public function test_sync_total_updates_increments()
    {
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'user' => 'c1' ], [ $update ]),
            ]
        );
        $this->dispatch_sync(
            [
                $this->build_room($room, 2, 0, [ 'user' => 'c2' ], [ $update ]),
            ]
        );
        $this->dispatch_sync(
            [
                $this->build_room($room, 3, 0, [ 'user' => 'c3' ], [ $update ]),
            ]
        );
    ...
    public function test_sync_should_compact_is_true_above_threshold_for_compactor()
    {
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'user' => 'c1' ], $updates),
            ]
        );
        ...
        $response = $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'user' => 'c1' ]),
            ]
        );
    ...
    public function test_sync_should_compact_is_false_for_non_compactor()
    {
        ...
        $response = $this->dispatch_sync(
            [
                $this->build_room($room, 2, 0, [ 'user' => 'c2' ]),
            ]
        );
    ...
    public function test_sync_awareness_shows_multiple_clients()
    {
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'name' => 'Client 1' ]),
            ]
        );
        ...
        $response = $this->dispatch_sync(
            [
                $this->build_room($room, 2, 0, [ 'name' => 'Client 2' ]),
            ]
        );
        ...
        $this->assertSame([ 'name' => 'Client 1' ], $awareness[1]);
        $this->assertSame([ 'name' => 'Client 2' ], $awareness[2]);
    }
    ...
    public function test_sync_awareness_updates_existing_client()
    {
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'cursor' => 'start' ]),
            ]
        );
        ...
        $response = $this->dispatch_sync(
            [
                $this->build_room($room, 1, 0, [ 'cursor' => 'updated' ]),
            ]
        );
        ...
        $this->assertSame([ 'cursor' => 'updated' ], $awareness[1]);
    }
    ...
    public function test_sync_rooms_are_isolated()
    {
        ...
        $post_id_2 = self::factory()->post->create([ 'post_author' => self::$editor_id ]);
        $room1     = $this->get_post_room();
        ...
        $this->dispatch_sync(
            [
                $this->build_room($room1, 1, 0, [ 'user' => 'client1' ], [ $update ]),
            ]
        );
