...
class Tests_Feed_FetchFeed extends WP_UnitTestCase
{
    ...
    public function test_fetch_feed_returns_error_for_404_response()
    {
        ...
        add_filter('pre_http_request', [ $this, 'mocked_rss_404_error_response' ], 15);
    ...
    public function test_fetch_feed_multiple_returns_error_if_any_feed_errors()
    {
        ...
        add_filter(
             ...
             */
            function ($response) {
                remove_filter('pre_http_request', [ $this, 'mocked_rss_404_error_response' ], 15);
        ...
        $feed = fetch_feed([ 'https://example.org/news/feed/', 'https://wordpress.org/news/feed/' ]);
    ...
    public function test_fetch_feed_multiple_returns_error_if_all_feeds_error()
    {
        // Priority 15 to ensure this runs after the mocked_rss_response filter.
        add_filter('pre_http_request', [ $this, 'mocked_rss_404_error_response' ], 15);
        $feed = fetch_feed([ 'https://example.org/news/feed/', 'https://example.com/news/feed/' ]);
    ...
    {
        $feed    = fetch_feed([ 'https://wordpress.org/news/feed/', 'https://wordpress.org/news/feed/atom/' ]);
        $content = [];
    ...
    public function test_fetch_feed_cached()
    {
        ...
        add_filter('pre_http_request', [ $filter, 'filter' ]);
