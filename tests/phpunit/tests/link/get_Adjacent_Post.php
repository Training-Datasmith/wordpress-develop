...
class Tests_Link_GetAdjacentPost extends WP_UnitTestCase
{
    ...
    public function test_get_adjacent_post_returns_empty_string_when_wp_get_object_terms_returns_wp_error()
    {
        ...
        wp_set_post_terms($post1_id, [ $term1_id ], 'wptests_error_tax');
        wp_set_post_terms($post2_id, [ $term1_id ], 'wptests_error_tax');
    ...
    public function test_get_adjacent_post_empty_term_array_after_exclusions()
    {
        ...
        wp_set_post_terms($post1_id, [ $term1_id ], 'wptests_tax');
        wp_set_post_terms($post2_id, [ $term1_id ], 'wptests_tax');

        $this->go_to(get_permalink($post2_id));
        $result = get_adjacent_post(true, [ $term1_id ], true, 'wptests_tax');
        $this->assertSame('', $result);
    }
    ...
    public function test_get_adjacent_post_term_array_processing_order()
    {
        ...
        wp_set_post_terms($post1_id, [ $term1_id ], 'wptests_tax');
        wp_set_post_terms($post2_id, [ $term1_id, $term2_id ], 'wptests_tax');
        wp_set_post_terms($post3_id, [ $term1_id ], 'wptests_tax');

        // Set the current post to post_two.
        $this->go_to(get_permalink($post2_id));

        // When we exclude term2, we should still get adjacent posts that share term1.
        $result = get_adjacent_post(true, [ $term2_id ], true, 'wptests_tax');
        ...
        // Test next post.
        $result = get_adjacent_post(true, [ $term2_id ], false, 'wptests_tax');
